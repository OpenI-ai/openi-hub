...
**Version:** 5.31
**Last Updated:** 31 May 2026 — **Vercel stale-bundle root-cause fix + retroactive token-rotation documentation.** Shipped `9bc03fa` fix(vercel): added a `headers` block to `vercel.json` so content-hashed `/assets/*` are served `Cache-Control: public, max-age=31536000, immutable` while the `/index.html` shell is `public, max-age=0, must-revalidate`. Root cause of the recurring stale-bundle `ae` TDZ false-alarm: Vercel's default served BOTH the shell and the hashed assets with `must-revalidate`, allowing stale bundles through. Verified live this session via deployment `dpl_91dTWrik3VxpKbQ967cyVcvb6D5p` @ `9bc03fa` (state READY) — curl confirmed shell `/` → `max-age=0, must-revalidate` and hashed asset `/assets/index-D_qB7PVq.js` → `max-age=31536000, immutable`, with the asset hash changed (`index-CvSeNjHY.js` → `index-D_qB7PVq.js`) confirming a fresh build. Also documented retroactively: backend git auth + GitHub token rotation (done a prior session, never written to memory) — the exposed over-scoped classic PAT was rotated to a fine-grained repo-scoped PAT (Contents r/w) and the old classic PAT deleted; backend fetch/push verified working. Backend HEAD `0439bbc` (unchanged — frontend-only session). Frontend HEAD `9bc03fa`. DOCUMENTATION.md v5.30 → v5.31.
