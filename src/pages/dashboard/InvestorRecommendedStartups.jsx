/**
 * s36 — Investor Recommended Startups (Stage B Discovery surface).
 *
 * Consumes GET /api/investor/recommended-startups (existing s28 endpoint).
 * Backend uses cluster boost on the investor's focus_sectors + looking_for
 * to lift relevant startups. Same card pattern as Student/Academia surfaces
 * with cluster-match badge, boost_lift indicator, and click-through tracking.
 */
...
export default function InvestorRecommendedStartups() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const data = await investorAPI.recommendedStartups();
  ...
  const handleCardClick = (s) => {
    // Fire-and-forget click telemetry. Don't block navigation.
    recommendationsAPI.trackClick({
      persona_role: 'investor',
    ...
    <div style={{ padding: 24, maxWidth: 1100, margin: '0 auto' }}>
      <div id="tour-page-recs-header" style={{ marginBottom: 20 }}>
        ...
        <p style={{ fontSize: 12, color: '#888', margin: '4px 0 0' }}>
          Startups matching your focus sectors and investment thesis, ranked by topical alignment.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ ...card, textAlign: 'center', padding: 40, cursor: 'auto' }}>
          ...
          <p style={{ color: '#666', fontSize: 13, margin: '0 0 18px', lineHeight: 1.5 }}>
            Add <b>focus sectors</b> and <b>looking for</b> tags to your investor profile.<br />
            We'll match you with startups in adjacent topical clusters.
          </p>
          <Link
