...
**Version:** 4.8
**Last Updated:** 18 May 2026 evening (Phase 89.3 → 89.9 cohort batch + Phase 97 session hardening, all SHIPPED end-to-end on prod). Phase 89.3 / T27 — IPR leak on Startup + Lab Dashboards closed (`7985f88`, frontend-only, 4 surgical line removals in `PersonaDashboard.jsx`). Phase 89.4 / T28 — DeepTech Submit gate hint copy widened to 3 variants + `setSubmitted` moved into `.then()` (`1d6e15c`). Phase 89.5 / T28 follow-up — prominent gold-bordered required startup-name card + auto-fill from `user.organization_name` for startup persona + smooth-scroll-to-focus on disabled click (`0be51de`). Phase 89.6 / T29 — DeepTech score math fixed (full-denominator) + "Provisional — N of M answered" amber subtitle (`444960c`). Phase 89.7 / T30 — backend rewritten to accept free-text `startup_name` + `verdict` (no FK required); ALTER TABLE deeptech_assessments: `startup_id DROP NOT NULL` + ADD COLUMN startup_name VARCHAR(200) + ADD COLUMN verdict VARCHAR(200); COALESCE bridging columns at every read site. Phase 89.8 / T31 — Watchlist "Add Startup" modal made search-driven via existing `/startups?search=` endpoint (`62efb85`). Phase 89.9 / T33 — Messaging "New Conversation" button wired with full modal (Direct/Group toggle, typeahead via `meetingAPI.searchUsers` w/ debounce + race-guard, validation, auto-refresh + open). Three commits to fully land due to apply-script idempotency-check substring-match trap (`497cc46` → `0ca5b27` → final sub-5 hotfix). Phase 97 — 24h JWT TTL + silent refresh (`POST /auth/refresh` w/ authMiddleware gate) + idle-logout with `role="dialog"` modal-exclusion (backend `b7fadc2`, frontend `d8045e6`). Smoke test confirmed `{ttl_seconds: 86400}`. Yesterday: Phase 96 — Projects + Tasks UI wiring (`8e68374` + `d7afbc0` AuthContext hotfix).
