...
**Previous Updated:** 1 Jun 2026 — **Post-hang recovery session: TourWrapper hoist + login-redirect-path fix recovered and shipped.** The prior (31 May) session hung mid-edit, leaving the frontend repo with one committed-but-unpushed commit (`4f73c0b`) plus 3 uncommitted dashboard files. This session recovered cleanly from real git state (not memory): verified the dirty diff was a coherent TourWrapper consolidation, confirmed `navRole` in scope in DashboardLayout.jsx (line 173), ran `npm run build` (0 errors, 2648 modules, pre-existing lint warnings only), committed as `085cc07`, and pushed `4f73c0b..085cc07`. Two ships now landed: (1) `4f73c0b` fix(auth): corrected the login redirect path — the 401 interceptor + idle-logout were redirecting to `/login?reason=expired`, but the real route is `/dashboard/login`; React Router's auth guard caught the null user first and `Navigate`-replaced to `/dashboard/login`, **stripping the query string** so the amber session-expired/idle banners never rendered. Pointed all three redirects at `/dashboard/login` and updated the pathname guards — this is the real fix that makes the carried "401 interceptor" banners actually show, **closing that pending-verification item.** (2) `085cc07` refactor(tours): hoisted `TourWrapper` from `CorporateDashboard.jsx` + `PersonaDashboard.jsx` up to the single canonical mount in `DashboardLayout.jsx` — `{navRole && <TourWrapper role={navRole} />}` after `<Outlet />`, co-located with the "Tour this page" topbar button. Page tours now fire on EVERY dashboard route that has a `PAGE_TOURS` entry, incl. admin sub-routes that previously had no mount. **This effectively closes Tour Batches 6/7/8** (the 3 standing P0 items) — one mount instead of per-page wiring. **Vercel deploy verified:** `dpl_5D3e45cjEifV7rDwH6mU42kocvpi` @ `085cc07`, target production, state READY (confirmed via MCP + user dashboard screenshot — source `main` @ `085cc07`, domains openi.ai + app.openi.ai + 4 more). Live prod shell `/` → `cache-control: public, max-age=0, must-revalidate` (cache-header fix from `9bc03fa` holding); serving Vercel-built bundle `/assets/index-B9BVAiL3.js`. Both repos clean + synced `0 0`. Backend HEAD `0439bbc` (unchanged — frontend-only session). Frontend HEAD `085cc07`. DOCUMENTATION.md v5.31 → v5.32.
