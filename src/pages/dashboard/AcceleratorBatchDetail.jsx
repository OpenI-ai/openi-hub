...
import {
  ...
  Users, Clock, Zap, MapPin, DollarSign, BarChart3
} from 'lucide-react';
...
export default function AcceleratorBatchDetail() {
  ...
  const load = async () => {
    try {
      const [b, s, m] = await Promise.all([
        acceleratorAPI.getBatch(id),
        acceleratorAPI.listBatchStartups(id),
        acceleratorAPI.listBatchMilestones(id),
      ]);
      setBatch(b);
      setStartups(s);
      setMilestones(m);
    } catch (err) { toast.error(err.message || 'Failed to load batch'); }
    finally { setLoading(false); }
  };

  const moveStartup = async (sid, newStatus) => {
    try {
      await acceleratorAPI.updateBatchStartup(id, sid, { application_status: newStatus });
  ...
  const handleAddStartup = async (e) => {
    ...
    try {
      await acceleratorAPI.addBatchStartup(id, {
        ...startupForm,
        pitch_order: startupForm.pitch_order || null,
      });
  ...
  const removeStartup = async (sid) => {
    if (!window.confirm('Remove this startup from the batch?')) return;
    try {
      await acceleratorAPI.removeBatchStartup(id, sid);
  ...
  const toggleMilestone = async (mid, currentStatus) => {
    ...
    try {
      await acceleratorAPI.updateBatchMilestone(id, mid, { status: newStatus });
      load();
    } catch (err) { toast.error(err.message || 'Failed'); }
  };
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1300, margin: '0 auto' }}>
      <button onClick={() => navigate('/dashboard/accelerator/batches')}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
              <h1 style={{ fontSize: 21, fontWeight: 700, color: '#1a1a1a', margin: 0 }}>{batch.name}</h1>
          ...
          <div style={{ ...card, width: '100%', maxWidth: 500, padding: 22 }}>
            ...
            <form onSubmit={handleAddStartup} style={{ display: 'grid', gap: 12 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 10 }}>
                ...
                <div>
                  <label style={lbl}>Pitch Order</label>
                  <input type="number" value={startupForm.pitch_order} onChange={e => setStartupForm({ ...startupForm, pitch_order: e.target.value })} style={inp} />
                </div>
              </div>
              <div>
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 6 }}>
                <button type="button" onClick={() => setShowAddStartup(false)}
                  style={{ padding: '9px 16px', background: '#fff', border: '1px solid #ddd', borderRadius: 8, fontSize: 13, fontWeight: 600, color: '#666', cursor: 'pointer' }}>Cancel</button>
                <button type="submit"
                  style={{ padding: '9px 18px', background: G, color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Add</button>
              </div>
            </form>
          </div>
        </div>
