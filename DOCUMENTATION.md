...
**Version:** 5.0
**Last Updated:** 19 May 2026 evening (Phase 101 — Application review enhancements, 5 sub-ships all SHIPPED end-to-end on prod same evening as Phase 99/100). Sub-A (backend) — `corporateController.getChallenge` JOINs `challenge_invites` to expose `is_invited` + `invite_status` + `invited_at` per applicant; sort invited-first via `ORDER BY (ci.id IS NOT NULL) DESC`. NEW `inviteController.remindInvitee` route POST `/challenges/:cid/invites/:iid/remind`, same `isOrgCoInviter` gate as send/revoke, re-uses `sendInviteEmail` helper. Sub-B (frontend `e9170c5`) — gold `★ Invited` pill chip next to applicant name on application card when `app.is_invited === true`. Sub-C (backend + frontend `9a55626`) — Empty-stub amber hint gains action guidance line. NEW Remind button (amber ✉ icon) next to Shortlist/Reject when application is empty AND `app.invite_id` is set. Backend also exposes `ci.id AS invite_id` per applicant. Sub-D (frontend `69861c3`) — Wired the existing notification bell stub on `DashboardLayout.jsx` to live `notificationAPI` (was `useState([])` and never updated since pre-T32 — fifth instance of the Phase 66/T9 bug pattern). 60s refresh interval. Click-row marks read + navigates to `n.link`. Mark-all-read button. Empty-state copy. Per-user notifications now visible to invitees on next login. Sub-E (frontend `6a1a494`) — Marketplace `★ Invited` pill chip next to challenge title on Marketplace.jsx cards when `ch.visibility === 'invite_only'`. Backend `c.*` already includes visibility column (Phase 99a). Pairs with Phase 99b sub5 OR-clause that exposes invite_only challenges to invitees on the authed marketplace. Cloudinary upgraded from Free → Image and Video Small PAYG ($29/mo, 60 credits/mo, $0.55 overflow) same evening — triggered by free-tier 25 GB storage cap reached following Phase 73 + Phase 93a/b uploads. Next bill 19 Jun 2026. Earlier today: Phase 99a/b/c/d/e (T32 private challenges schema → invitee inbox → Invite Startups modal → Accept-creates-application) + Phase 100 (cross-persona application review UX with profile link + per-persona vocab + investor financials + empty-stub hint, 3 hotfixes). Yesterday (18 May): Phase 89.3 → 89.9 cohort batch + Phase 97 session hardening + Phase 96 Projects/Tasks wiring.
