...
**Pending verification:** Invite-email persona framing (`2f5d11d`) + **99j/99k COPY narrowing** — testing-team inbox checks in a REAL external gmail: Path A (registered user → standard challenge-invite); Path B (unregistered email per inviter role → persona-framed magic-link); **plus reviewer/watchlist/IPR/collaboration invites must read as collaboration/opportunity NOT "lead."** 99g role-aware challenge access end-to-end (reviewer can open+evaluate, cannot mutate). Spot-check tours fire on the moved mount (`085cc07`). 401/idle banners in prod (`4f73c0b`). Phase 122 + 123 mobile dashboard layout on real device. Phase 120 mobile Back button.
