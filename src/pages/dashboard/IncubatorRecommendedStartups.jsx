/**
 * s36 — Incubator Recommended Startups (Stage B Discovery surface).
 *
 * Consumes GET /api/incubator/recommended-startups (existing s28 endpoint).
 * Same card pattern as Investor/Student/Academia surfaces with cluster-match
 * badge, boost_lift indicator, and click-through tracking.
 */
import { useState, useEffect } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import toast from 'react-hot-toast';
import { Sparkles, Cpu, MapPin, TrendingUp, Loader2, ArrowRight } from 'lucide-react';
import { incubatorAPI, recommendationsAPI } from '../../services/api';

const G = '#D5AA5B';
const SOURCE = 'incubator_recommended_startups';
const card = {
  background: '#fff',
...
export default function IncubatorRecommendedStartups() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const data = await incubatorAPI.recommendedStartups();
    ...
    <div style={{ padding: 24, maxWidth: 1100, margin: '0 auto' }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ fontSize: 12, color: '#888', margin: '4px 0 0' }}>
          Startups matching your incubator's focus sectors, ranked for cohort fit.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ ...card, textAlign: 'center', padding: 40, cursor: 'auto' }}>
          ...
          <p style={{ color: '#666', fontSize: 13, margin: '0 0 18px', lineHeight: 1.5 }}>
            Add <b>focus sectors</b> to your incubator profile.<br />
            We'll match you with startups working in those areas, excluding ones already in your programs.
          </p>
          <Link
