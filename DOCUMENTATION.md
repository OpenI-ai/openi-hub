...
**Version:** 5.20
**Last Updated:** 26 May 2026 — **Session shipped 5 things end-to-end. 3 backend commits (`04c26b1` Ship A invoice BCC + `84de5f3` Phase 113-renewals service + `ecb73e4` Phase 113b idempotency), 0 frontend commits, 0 schema migrations. Plus 2 ops changes (MAIL_FROM env var change + 1 stale notification deleted) and 1 verified backfill (2 historical invoices re-sent with BCC).** **Naming clash to remember:** "Phase 113" today is the SUBSCRIPTION RENEWAL REMINDER SERVICE — totally distinct from the "Phase 113 ladder" of 25 May (org-by-domain detection at signup). Same number, different scope. Both live. Both repos clean. Backend HEAD `ecb73e4`. Frontend HEAD `6db5266` (unchanged from 25 May v5.19). **Ship A — BCC vanessa@openi.ai + rajeev@openi.ai on every paid invoice email** (`04c26b1`, emailService.js only): added `bcc` param to `sendEmail({to,subject,html,attachments,bcc})` and threaded through Resend body + Nodemailer opts. `paymentConfirmationEmail()` template now returns `bcc: ['vanessa@openi.ai', 'rajeev@openi.ai']` so every caller inherits Finance visibility automatically. Closes Govt-compliance gap raised by Vanessa for paid customer invoice archival. **Ship B — Backfill all past paid invoices** (operational `/tmp/shipB_backfill_invoice_bcc_v2.js` via railway ssh, no commit): enumerated `SELECT FROM payment_history WHERE status='captured' AND amount > 0` JOINed users + user_subscriptions + subscription_plans. Re-generated PDFs via existing `generateInvoicePdf` shape (same as `subscriptionController.downloadInvoice`). Sent 2 historical invoices: Vanessa's OPENI/FY26-27/0001 + Karuna Dagur's OPENI/FY26-27/0002 (Dentsu first paid customer, paid 24 May). Subject format for backfill emails (per locked Option Q2): `"OpenI Tax Invoice - {invoice_number}"` with "Re-sent for compliance archival" subtitle in body — distinct from forward-going `"Payment Confirmed - {plan} Plan"`. Customer sees verbatim invoice; vanessa+rajeev BCC'd silently. End-to-end verified — rajeev@openi.ai inbox received both Dentsu BCC with correct GST PDF showing IGST @ 18% Rs. 449.82 (inter-state Maharashtra→Karnataka) + Total Rs. 2,948.82 + total-in-words. **MAIL_FROM env var fix** (Railway prod): was `OpenI Hub <openihub@send.openi.ai>` — `send.openi.ai` subdomain was NEVER verified in Resend (Resend free plan caps at 1 verified domain; Resend's existing verified domain is `openi.ai` on `ap-northeast-1`). Every transactional email was hitting Resend → 403 `domain not verified` → silently falling through to Nodemailer/Gmail SMTP fallback. Changed to `OpenI Hub <noreply@openi.ai>` → smoke-tested via `/tmp/smoke_resend.js` → `[EMAIL] Sent via Resend to rajeev@openi.ai (id: 4b22dbde-...)`. Resend now properly delivers all transactional + invoice + notification + reminder emails. **Phase 113 — Subscription renewal reminder service** (`84de5f3`, 3 files: NEW `renewalReminderService.js` ~180 lines + 5 templates appended to `emailService.js` + cron wire in `cronService.js`). 5 fire windows mapped to subscription lifecycle for the MANUAL-renewal Razorpay model: (1) `fireRenewal30` 30 days before `period_end` for active subs; (2) `fireRenewal7` 7 days before; (3) `fireRenewal3` 3 days before; (4) `fireExpired` 1 day after expiry (silent-expiry catch); (5) `fireCancellationEnding` 7 days before for cancelled subs winding down. Each fires email + bell notification. Each email template returns `bcc: ['vanessa@openi.ai', 'rajeev@openi.ai']` for Finance churn-risk visibility. Cron registered via new `startRenewalReminder()` function in cronService.js, scheduled at `30 21 * * *` (21:30 UTC = 03:00 IST daily, same tick as `startDeadlineReminder` + `startIntegrityCheck`). Each registers via its own try/catch-isolated function — independent failure handling. Smoke-test invocation via railway ssh fired CORRECTLY: Karuna's row matched the 30-day window (her `current_period_end` = 25 June 2026, 30 days from today), received email with subject "Renewal reminder: Your Pro Plan expires in 30 days" verified end-to-end in rajeev@openi.ai BCC inbox with personalized body, correct date format, gold CTA button to `/dashboard/settings?tab=billing&focus=plans`. Important architectural finding (verified via grep + read): Razorpay subscriptions on OpenI Hub are MANUAL not auto-renew — `subscriptionController.verifyPayment` creates one-shot Razorpay orders + manually sets `user_subscriptions.current_period_end = now + 1 month`. Webhook only handles `payment.captured` / `payment.failed`, no `subscription.charged`. Customer MUST proactively renew or access silently expires. Phase 113 is the critical lifeline for protecting renewal revenue. **Phase 113b — Idempotency guard prevents duplicate sends** (`ecb73e4`, single-file 36ins/10del): same-window matching across consecutive cron ticks would have spammed Karuna with the same 30-day reminder 1-2 mornings in a row (her period_end is fixed; the `[NOW+29.5d, NOW+30.5d]` window doesn't drift fast enough). NEW `wasFiredRecently(userId, kind, relatedId, daysAgo)` helper queries `user_notifications` for prior delivery within lookback window; if found, skip silently. Lookback per window: 7d for renewal_30/renewal_7 (cover 1-day window + 6-day buffer), 1d for renewal_3/expired/cancellation_ending (narrower windows + tighter cadence acceptable). Telemetry surfaced via updated log line `[renewal_30] fired N reminders (skipped M already-fired)` and return shape `{category, count: rows.length - skipped, skipped}`. Re-invocation of runAll smoke-test post-Railway-redeploy confirmed: `[renewal_30] fired 0 reminders (skipped 1 already-fired)` — Karuna's row matched the 30-day window query, but the wasFiredRecently helper found her prior `user_notifications` row from earlier today and correctly skipped. She will NOT receive duplicate reminders. Forward compatibility: any future fire helper in this service should include the same guard at top of for loop. Pattern is local to file (no shared abstraction extracted yet — wait until 3rd fire helper requires different lookback per CLAUDE.md "premature generalization" lesson). **Integrity-drift fix** (operational, two scripts via railway ssh: `refresh-cluster-subgroups.js --apply` + `prune-stale-notifications.js --apply`): Phase 102d watchdog from 25 May LATER alerted this morning at 03:30 IST with "51 stale cluster_subgroups + 1 orphan notification". Dry-run via `refresh-cluster-subgroups.js` revealed cluster_subgroups was ALREADY clean (0 stale, 0 deletes, 0 updates — watchdog over-alerted OR self-healed between alert and triage). Notification dry-run found exactly 1 stale challenge_invite pointing at deleted challenge — applied (--apply), 1 row removed. Idempotency re-verified via dry-run = `TOTAL stale notifications pruned: 0`. **5 NEW carry-forward items added to queue** based on session-level user requests: (A) Admin platform-health dashboard (signed-up users, paid vs free, persona breakdown, revenue MTD/YTD, churn signals from `current_period_end` proximity) — high strategic priority, ~300 lines, reuses Phase 102 AdminCosts.jsx pattern; (B) Ship C — "New paid customer" Finance notification on signup/upgrade/renewal — closes "how do I know about paid customers" gap (Razorpay was accidental signal), Q1=A (single email TO vanessa+rajeev, not BCC). 3 cohort-asked items already in queue from earlier sessions: Restore Drill dashboard, Messaging button on Challenge+Watchlist, Pricing overhaul (8-Vector + team collaboration to startup plan + 1m/12m billing cycles), Mobile responsive design pass, Joy Ride per-page tour extensions, Landing page rehash. Previously (v5.19, 25 May 2026 LATER) — **Phase 112 + 113 ladder (Ships 1+2+3) + 113b + Tier-B5 sidebar badge + Tier-B4 integrity watchdog + Tier-C6 Phase 114 schema prep ALL SHIPPED end-to-end on prod.** 8 commits in second half of 25 May, including 1 backend batch (`61a2525`: Tier-B5 endpoint + Tier-B4 watchdog + Tier-C6 schema), 5 frontend ships (`d7f47d1` Phase 112 Edit modal, `6f9e964` Phase 113 Ship 2 signup card, `1e376eb` Phase 113 Ship 3 post-verify replay, `3bfff72` email lock at magic-link signup, `e116391` Phase 113b precedence + Tier-B5 badge), and 2 backend ships (`03b3d2c` Phase 112 Edit gate, `e9186c7` Phase 113 Ship 1 org-by-domain + request-join). **Phase 112 — Document Repository Edit modal:** cohort feedback "Edit option is required to change the Access" closed. PUT /documents/:id gate extended to include same-org members (mirrors Phase 99 isOrgCoInviter EXISTS subquery on org_members.org_id JOIN). Frontend: pencil icon (Edit2) added as FIRST action button in list-view row, opens modal with editable Name/Folder/Access/Tags. **Phase 113 — Multi-stakeholder organization detection at signup:** Ship 1 backend (`e9186c7`) added 2 routes — `GET /api/public/org-by-domain?domain=X` (no-auth lookup of orgs by email domain) + `POST /api/org/members/request-join` (authed insertion of org_members row with status='invited'). Ship 2 frontend (`6f9e964`) added gold suggestion card below email input in Register.jsx — debounced 500ms domain lookup, shows "<OrgName> is already on OpenI Hub" with Request to join + Create my own buttons; localStorage.openi_pending_org_join stash for post-verify replay. Ship 3 frontend (`1e376eb`) added flushPendingOrgJoin() helper in VerifyEmail.jsx that fires POST /org/members/request-join after completeVerification() issues token — mirrors Phase 60.10 flushPendingProfile pattern with terminal-vs-transient error handling. **Phase 113b** (in `e116391`): when arriving via magic-link `/invite/accept/<token>` (Phase 108 flow), domain-match card suppressed via early-return + inviteToken in useEffect deps (no competing prompts). **Email lock follow-up** (`3bfff72`): Phase 108-related Register.jsx email field becomes readOnly + grey when inviteToken URL param present — prevents accidental edit that would break the Phase 108 consume flow. **Tier B #5 sidebar badge** (backend in `61a2525` + frontend in `e116391`): NEW `GET /api/org/pending-invites-count` returns count of `status='invited'` rows where current user is org admin. Sidebar Organization link shows gold pill chip when count > 0 (mirrors Phase 74 What's New unread pattern). 60s poll. Closes UX gap where org admin previously had to navigate to OrgAdmin to discover pending join requests. **Tier B #4 — daily integrity drift cron** (in `61a2525`): NEW `src/services/integrityCheckService.js` (~120 lines, mirrors Phase 105b deadlineReminder pattern). Schedule: `0 22 * * *` (22:00 UTC = 03:30 IST, daily). Two categories audited — (A) `cluster_subgroups.member_count` drift vs actual count (>5% triggers email alert); (B) `user_notifications.related_id` orphans across 7 polymorphic kinds (challenge_invite / challenge_deadline_soon / challenge_applied / event_published / event_tomorrow / event_registered / ipr_linked). Email via existing `ALERT_EMAIL_RECIPIENTS` env var. Catches the 24 May class of stale-snapshot bug 11 days earlier than a cohort screenshot would. **Tier C #6 — Phase 114 schema prep** (in `61a2525`, migrated to prod via migrate-bootstrap.js): all ALTERs idempotent. Added 9 new columns total — `org_id INTEGER REFERENCES organizations(id)` on challenges/events/ipr_records/documents + `applicant_org_id` on challenge_applications + `visibility_in_org VARCHAR(20) DEFAULT 'creator_admin' CHECK ('creator_admin','org_wide')` on challenges/events/ipr_records/documents. Extended `org_members.role` CHECK from `('admin','member')` to `('admin','editor','member','viewer')` for 4-tier role model. 5 new partial indexes on the org_id columns (WHERE org_id IS NOT NULL). NO controller changes, NO backfill — groundwork only so Phase 114 proper (dual-write controllers + backfill) ships later without separate migration step. **PHASE_113_116_DESIGN.md** saved to `/Users/rajeevbanduni/CoPilot/` covering the full multi-stakeholder organization model: Phase 113 (surface existing org infra) → Phase 114 (org-aware ownership of challenges/applications/documents/IPR/events) → Phase 115 (auto-create 1-person org at signup + per-persona profile linking + public `/orgs/:slug` page) → Phase 116 (drop legacy single-user columns + multi-org topbar switcher; ONE-WAY DOOR). Locked design decisions: multi-org membership (Slack/Notion model with JWT carrying active_org_id), auto-create 1-person org at signup for every persona, dual-write migration shape, self-service domain match (lenient verification, org admin approves), restricted-by-default content visibility (creator_admin default with explicit org_wide opt-in). **Test team verification guide** saved to `/Users/rajeevbanduni/CoPilot/TEST_TEAM_GUIDE_PHASE_112_113.md` covering 6 verification tests with expected/actual/network-trace checkpoints — share with cohort to unblock verification. Memory hygiene: marked Phase 107c + 3 stale "Ship #N follow-up" carry-forward entries as SHIPPED in CLAUDE.md (memory was lying about deferred status; lines 978-980 had correct shipped record). Both repos clean. Backend HEAD `61a2525`. Frontend HEAD `e116391`. Previously (v5.18, 25 May SESSION CLOSE earlier today) - **Phase 108-repair + 109 + 110 + 111 ALL SHIPPED end-to-end.** Net: 4 new shareable entity types (DeepTech / IPR / 8-Vector Self / Program-eval) wired backend + frontend + public pages, alongside Phase 109 watchlist collaboration and Phase 110 startup profile sharing. Cohort can now share 7 entity types across 11 sharable surfaces (Challenges + Watchlist + Startup Profile + DeepTech + IPR + 8-Vector Self + Program-eval). Commit ledger: `371cf10` Phase 108-repair backend (createInvites email branch ReferenceError fix via bucket-by-email pattern), `8a33545` Phase 109 Ship 1 backend (watchlist_collaborators table + 3 access helpers + 4 routes + addStartup/removeStartup gates that closed pre-existing zero-check bug), `caba06f` Phase 109 Ship 2 prep (patch HTTP helper + 4 watchlist collaborator API methods), `2136006` Phase 109 Ship 2 frontend (Manage Collaborators tab in StartupWatchlist modal), `7cc55d9` Phase 110 Ship 1 backend (startup_profile_shares + 5 routes + generateStartupProfilePdf + redaction logic Full/Public-safe/Pitch-only), `12931e8` Phase 110 Ship 2 frontend (Share modal + SharedStartupProfile.jsx public page), `dceaf03` Phase 111 Ship 1a backend (DeepTech + IPR sharing - deeptech_assessment_shares table, NO ipr_record_shares per locked D3 - IPR magic-link only because legal exposure of patents/trademarks/filings), `b3094ab` Phase 111 Ship 1b backend (8-Vector self-assessment persistence + sharing + Program-eval sharing - 3 new tables, NEW eightVectorSelfController.js ~250 lines, 2 new PDF generators, 13 new routes), `36d5062` Phase 111 Ship 2a frontend (DeepTech Share modal + SharedDeepTech.jsx), `4873f8f` Phase 111 Ship 2b frontend (IPR Share modal magic-link only, 2 hotfixes), `ce91a37` Phase 111 Ship 2c frontend (StartupEvaluation Save + Share + SharedEightVectorSelf.jsx, 1 hotfix - was local-state only pre-ship, now persists assessment then shares), `d33d4b1` Phase 111 Ship 2d prep (portfolioEvalsAPI extension), `5d7939d` Phase 111 Ship 2d frontend (Program-eval Share + SharedProgramEval.jsx, 2 hotfixes). 5 hotfixes shipped along the way - all banked as new lessons in CLAUDE.md Don'ts section. Locked design decisions: D1 strict serial ship (later abandoned mid-session for parallel build per user explicit ask "let us not wait for 109 cohort signoff, let us build parallel"); D2 startup profile public link gets per-share redaction mode picker (Full/Public-safe/Pitch-only) - owner picks at mint time; D3 IPR records get magic-link signup ONLY (no public token mode) - legal exposure too high for "anyone with the URL" access; Option C for 8-Vector ships both self-assessment + program-evals sharing. 8 new lessons banked covering apply-script anchor traps: (1) `return (` is universally unsafe in files with sub-components/closures/useMemo - use last useState as anchor instead; (2) `useAuth()` and `useParams()` also unsafe when multiple sub-components exist - anchor on `export default function COMPONENTNAME()` literally; (3) `useParams` destructured names vary per file - always grep the actual destructure before referencing param name; (4) modal JSX regex anchor `\n  </div>\n);\n}` matches root closing but insertion-with-leading-newline can shift JSX AFTER `</div>` making it sibling-of-root (invalid JSX); (5) handler-insertion + JSX-insertion are same trap class - narrow anchor to structurally unique 1-2 line literal; (6) grep `Mail|Loader2|<NEW_ICON>` BEFORE adding to lucide imports - duplicate-import lint trap; (7) lucide regex with trailing commas + line-end comments breaks naive append (parse-dedupe-emit instead); (8) multi-line literal anchors break on 1-space whitespace mismatches - narrow to 1-2 lines or whitespace-tolerant regex. All-entity share matrix now LIVE on prod: Challenges (PDF + invite-only public + magic-link), Watchlist (PDF + public token + collaborators magic-link), Startup Profile (PDF + 3 redaction modes + magic-link), DeepTech (PDF + public + magic-link), IPR (PDF + magic-link ONLY per D3), 8-Vector Self (PDF + public + magic-link), Program-eval (PDF + public + magic-link, feature-gated). All 4 backend ships passed 3 verification gates each (schema/boot/routes); all 6 frontend ships clean on Vercel post-hotfix. Test team verification deferred to mailinator-based testing of all 11 sharable surfaces. Both repos clean. Backend HEAD `b3094ab`. Frontend HEAD `5d7939d`. Deferred follow-ups (not blocking): lucide warnings cleanup on Phase 110 + Phase 111 Ship 2c (unused Mail/Copy icons imported but never referenced in modal JSX - cosmetic only); 12 entries in /tmp/ apply-scripts safe to delete (every shipped change is in git history). Previously (v5.14, 24 May LATE EVENING): **Phase 108 SHIPPED end-to-end + hotfix + migration verified live.** Initial backend ship (commit `387cd20`) deployed but crashed on boot with "argument handler must be a function" at routes/index.js:113. Root cause: Phase 108 apply-script appended `exports.pendingInviteByToken = ...` AFTER the existing `module.exports = { ... }` block in publicController.js - the explicit module.exports object overwrote the `exports` reference, so the appended `exports.X = ...` assignments were silently lost. Same bug in collaboratorController.js. Hotfix (commit `4bb4c32`, 2 files / +6/-5): converted `exports.X = async (req, res) => { ... };` to `async function X(req, res) { ... }`, stripped trailing semicolon, added `X` name to the existing `module.exports = { ... }` object. Verified node -c syntax-check on both files before push. **Post-hotfix Railway redeploy:** new deployment ID `6e544e73-a23a-4ac0-89dd-055d3502585a` ACTIVE. **Migration:** ran `railway ssh "cd /app && node src/scripts/migrate-bootstrap.js"` per project convention (RUN_MIGRATIONS_ON_BOOT disabled in prod). Completed in 4.9s. **Verified live:** (a) frontend `/invite/accept/test_token` returns HTTP 200 (AcceptInvite page loads); (b) backend `GET /api/public/invite/accept/abc` returns HTTP 404 + `{"message":"Invite not found"}` (handler running, correct response shape); (c) `pending_email_invites` table exists with all 13 designed columns + 0 rows (clean state, ready for cohort). **Frontend ship unchanged** (commit `f063916`): AcceptInvite.jsx + Register.jsx pre-fill + CorporateChallenges email-invite UI + App.jsx route. **NEW lesson banked:** When apply-scripts append new handlers to controllers that use explicit `module.exports = { X, Y, Z }`, ALWAYS add the handler name to the exports object - bare `exports.NewHandler = ...` appends silently get lost because the explicit assignment overwrote the `exports` reference earlier in the file. Pattern reusable for any future controller extension. Better future practice: detect `module.exports = {` pattern in apply-script + abort with a clear "append handler then update module.exports" message OR auto-extend the existing exports object. **Phase 108 cohort verification path** (share with testing team): as corporate, open invite modal on a private challenge, type a non-OpenI email like `test123@mailinator.com` in the "Or invite by email" field, press Enter to add as chip, click Send Invites -> toast "1 email invite sent to non-OpenI users". Check mailinator inbox -> "<inviter> invited you to collaborate on OpenI Hub" email -> click magic link -> `/invite/accept/<token>` -> auto-redirects to `/register?email=test123@mailinator.com&invite_token=...` -> email pre-filled -> complete signup -> verify email -> pending invite auto-materializes into `challenge_invites` -> log in -> see invitation in Invitations Inbox -> Accept -> Phase 108 Option B deep-link lands on Apply form with pitch field focused. Previously (v5.13, 24 May LATE EVENING): **Phase 108: Invite non-OpenI users by email** (4-ship: backend `<108abc>` + frontend `<108d>`). Closes cohort design question: "If I share a challenge with an email that is not part of OpenI Hub or any share (e.g. watchlist), how will we handle this?" Pre-change: every invite/share surface required an existing user_id - emails of non-OpenI people were rejected. Post-change: every email invite goes through `pendingInviteService.findUserByEmail()` - if user exists, in-app invite fires immediately; if not, a `pending_email_invites` row is created with a 30-day-expiring magic-link token + branded email is sent ("<inviter> invited you to collaborate on OpenI Hub"). Recipient clicks link -> `/invite/accept/:token` -> verifies invite -> redirects to `/login?invite_token=...&email=...` (if account exists) or `/register?invite_token=...&email=...` (if not, with email pre-filled + locked). On email verification, `pendingInviteService.consumePendingInvitesForUser()` runs inside the same txn as `markVerified()` and materializes every pending invite for that email into the real per-entity table (challenge_invites for challenge invites, entity_collaborators for collaboration invites). **Schema:** NEW `pending_email_invites` table (id, email, entity_type, entity_id, invited_by_user_id, role, token VARCHAR(64) UNIQUE, message, status CHECK in ('pending','consumed','expired','revoked'), expires_at, consumed_at, consumed_by_user_id) + UNIQUE on (email, entity_type, entity_id) for idempotent re-invites + 3 indexes (email/status, token-partial, inviter-status). **Backend ship:** NEW `src/services/pendingInviteService.js` (~210 lines: findUserByEmail / createPendingInvite / consumePendingInvitesForUser / findByToken / revokePendingInvite + SUPPORTED_ENTITY_TYPES Set + EXPIRY_DAYS=30). NEW `pendingInviteEmail()` template in emailService.js (subject "<inviter> invited you to collaborate on OpenI Hub", from "OpenI Hub <hello@openi.ai>", body explains entity_type with friendly per-type labels). Wired into `inviteController.createInvites` (challenges - accepts `emails: [string]` alongside `user_ids: [int]`), `collaboratorController.inviteByEmail` (NEW POST /collaborators/invite-by-email - generic Phase 40 collaboration invites by email), `emailVerificationController.markVerified` (consume hook fires after email-verification UPDATE inside the same transaction - non-fatal if it throws), `publicController.pendingInviteByToken` (NEW GET /api/public/invite/accept/:token, no auth required, returns invite + has_account boolean). **Frontend ship:** NEW `src/pages/auth/AcceptInvite.jsx` magic-link landing page - 3 states (loading/redirecting/error) - 600ms verify flash then auto-redirect to login or register with query params. Register.jsx extended to read `?email=` from URL params - pre-fills email on initial state (lock to come in next iteration if needed). CorporateChallenges.jsx invite modal now has "Or invite by email" section below the user-picker with enter-or-comma email entry, chip display, and unified Send Invites button that fires both lists in one API call. Send button toast updated to surface both in-app and email invite counts ("3 in-app invites sent · 2 email invites sent to non-OpenI users"). New route /invite/accept/:token added to App.jsx. **Architectural notes:** Marketplace + Invitations Inbox + Pending Email Invites are now THREE related-but-distinct surfaces: Marketplace = browse public, Inbox = challenges I was specifically invited to as an existing user, pending_email_invites = invites sent to my email before I existed on OpenI. Consume hook auto-materializes the third into the second on signup completion. Idempotency via UNIQUE(email, entity_type, entity_id): re-inviting the same email-entity tuple refreshes the token + extends the expiry instead of duplicating. **5 new lessons banked:** (a) Polymorphic invite pattern for non-existing-user invites - `pending_email_invites` table + materialization-on-verify scales to any entity type (challenge, collaboration, watchlist, etc.) without per-entity duplicated logic; (b) Consume-on-verify is the right hook location - by the time email is verified, identity is proven and the invite can safely materialize - never auto-consume on registration alone (email-not-yet-proven); (c) UNIQUE(email, entity_type, entity_id) with ON CONFLICT DO UPDATE token + expires_at + status='pending' is the clean idempotency pattern for re-invites - sender doesn't see "already invited" errors, recipient gets a fresh link; (d) Public route + has_account boolean in the magic-link landing response avoids two round-trips - frontend knows immediately whether to route to login vs register; (e) Subject-line format "<inviter_name> invited you to collaborate on OpenI Hub" is the standard SaaS pattern (Notion / Figma / GitHub all use it) - clear from + clear action + clear product name in 8 words. **Cohort verification path:** as corporate, open invite modal on a private challenge, type a non-OpenI email like `test123@mailinator.com` in the "Or invite by email" field, hit Enter to add as chip, click Send Invites -> toast shows "1 email invite sent to non-OpenI user". Check mailinator inbox -> click magic link -> lands on AcceptInvite -> redirects to /register?email=test123@mailinator.com&invite_token=... -> complete signup -> verify email -> log in -> the pending invite materializes into challenge_invites, the new account sees the invitation in Invitations Inbox + can Accept it via the Phase 108 Option B deep-link to land directly on the Apply form. Previously (v5.12, 24 May EVENING): **Option B: Accept invite -> Apply form deep-link** (frontend `4159688`). Closes cohort feedback: "if Invitation Inbox says invited to private challenge, you still have to go to challenge page and apply, it's not a great user experience." Pre-change: Accept invite -> toast "accepted" -> Phase 99e creates empty application stub -> nothing else happens UI-side; invitee has to manually find the challenge in Marketplace + open detail + click Apply + fill in pitch. Post-change: Accept -> auto-navigate to `/dashboard/marketplace?challenge=<id>&action=apply` -> Marketplace mounts, deep-link effect reads query params, calls `openDetail(id)`, then `setShowApply(true)`, then `setTimeout(scrollIntoView + focus, 300)` on the pitch textarea -> invitee starts typing immediately. 2 frontend edits, zero backend changes. Marketplace.jsx new useEffect (mount-only, no exhaustive-deps regression risk) + useSearchParams import. ChallengeInvites.respond('accept') captures `res.invite.challenge_id` and navigates; decline flow unchanged. **Architectural decision:** Marketplace + Invitations Inbox remain SEPARATE per 21 May design (different discovery intents - Marketplace = browse all public challenges, Inbox = challenges I was specifically invited to). The deep-link closes the UX seam between them without merging them. **New lesson banked:** Deep-link query params close two-surface UX gaps without merging the surfaces. Reusable pattern: any two surfaces that handle adjacent steps of a journey can be linked via `?query=param` instead of merged; URL becomes bookmarkable + shareable as a bonus. Same pattern as Phase 68 `?tab=billing&focus=plans` on Settings. **Cohort verification path:** as a startup, click Accept on any pending invite in Invitations Inbox -> URL should become `/dashboard/marketplace?challenge=<id>&action=apply`, challenge detail opens, Apply form visible, pitch textarea focused with cursor blinking, page scrolled so textarea is centered. Previously (v5.11, 24 May PM): **Cohort spider-map fix + stale-FK cleanup** (backend `55b4813`). Cohort reported empty sub-group bubbles on `/dashboard/cluster/110` Innovation Map. Triage uncovered TWO related stale-data classes both rooted in the 13 May s39 hygiene DELETE (which removed 8,207 name-only stubs). Part A: `cluster_subgroups.member_count` is a denormalized snapshot from Phase 71c (11 May 2026) that does NOT auto-update when underlying members are deleted. 1,712 of 2,369 sub-group rows (72%) had zero active members. Ad-hoc DB fix shipped on prod via DELETE-first strategy (avoids `CHECK > 0` constraint): 2,369 -> 657 rows, sum 250,985 -> 5,960. New script `src/scripts/refresh-cluster-subgroups.js` committed for re-runnable future ops. Part B: `user_notifications.related_id` is polymorphic by design (no FK constraint) so CASCADE doesn't help when parent entity gets deleted. Audit found 3 stale `challenge_invite` notifications pointing at deleted challenge id 26. Backfill via new `src/scripts/prune-stale-notifications.js` script removed them on prod. NEW `notificationService.pruneForEntity(client, entityKind, entityId)` polymorphic helper + `ENTITY_TO_NOTIFICATION_KINDS` map covering challenge/event/watchlist/meeting/ipr. Inline pruneForEntity hooks added to 4 active delete sites: `corporateController.deleteChallenge` (pool, fire-and-forget), `adminController.deleteChallenge` (client, in-txn awaited), `eventController.destroy` (pool, fire-and-forget), `watchlistController.remove` (pool, fire-and-forget). All 4 follow correct architectural shape per Phase 99 lessons (admin txn vs single-statement fire-and-forget). **4 new lessons banked:** (a) Phase 95 DR-drill floor lesson generalized to ANY denormalized snapshot, not just CI workflow floors - bulk DELETE > 0.1% of users requires post-delete snapshot recompute in the same SSH session; (b) `user_notifications.related_id` polymorphic by design - no FK constraint - so CASCADE doesn't help and inline pruneForEntity hooks at every delete site are the architecturally correct fix; (c) `require('pg')` from `/tmp/_x.js` does NOT resolve on Railway containers (node_modules at `/app/node_modules`) - use absolute path for one-shot scripts piped via railway ssh, or repo-installed scripts work fine with relative require; (d) CHECK constraints can block bulk UPDATE strategies - `cluster_subgroups.member_count CHECK > 0` aborted UPDATE-to-0; DELETE-first then UPDATE-non-zero is the safe pattern. **P2 NEW (low priority):** Add Phase 102-style integrity-check cron that scans for stale denormalized snapshots + dangling polymorphic FK refs - would have caught today's bug 11 days earlier. Cohort verification path: reload `/dashboard/cluster/110` and confirm only 4 sub-groups render (was 39 stale bubbles). Previously (v5.10, 24 May EOD): **3 additional commits after v5.9.** (1) `fda1684` Lint hygiene - wired `eslint-plugin-unused-imports@3.2.0` into `.eslintrc.cjs`, auto-removed 230 unused imports across 68 files. Warning count 302 -> 72. Build green at 5.53s. Pre-Phase-104d setup tracked unused imports as warnings but had no auto-fix path because default `no-unused-vars` refuses to touch imports. New plugin IS safe to auto-fix for named-import removal. Vite bundle size unchanged (unused imports were tree-shaken at build time anyway - purely code-hygiene improvement). (2) `799cf5f` Currency-aware `formatFunding` on `StartupDiscovery.jsx` - replaces hardcoded `$` prefix with proper INR / USD branching. `formatFunding(val)` -> `formatFunding(val, currency, unit)`. Respects backend-stored `funding_raised_unit` when present (Cr / L / K / M / B - Phase 92.3+ rows). Auto-formats legacy raw-rupees / raw-dollars rows by magnitude. Caller passes `funding_raised_currency` + `funding_raised_unit` from row. DollarSign icon -> Wallet (Phase 92.4 currency-agnostic pattern). Cohort impact: Indian investors browsing Discover Startups now see "Rs 5Cr" instead of "$50M" for Rupee-denominated startups. Audit revealed 7 other recommendation surfaces were already INR-aware from Phase 87 + Phase 92 era - the "9 surfaces" estimate in CLAUDE.md memory was 21 days stale, actually only 1 surface remained. (3) Stale-memory triage - 3 P2 items in CLAUDE.md were lying (claimed open but actually shipped). BodyBullets cosmetic bug (fixed 21 May `911f315`, verified against real prod data with regex test). Admin missing "What's New" sidebar entry (fixed 21 May, DashboardLayout.jsx:310 removed role gate via `{true && ...}`). All 3 removed from P2 carry-forward list. **Lessons banked:** (a) `/tmp/` apply-script pattern is the standard workaround for malware-reminder false-positive on code AND config AND markdown files; (b) `eslint-plugin-unused-imports` is the right tool when default ESLint `--fix` won't touch imports; (c) periodic stale-memory audits BEFORE rescoping a session save hours - 3 of 3 P2 items today were stale, 30 seconds of grep/Read/DB-query proved each before any code was written. **Live verifications on prod (post-deploy):** OpenAI cost panel collecting (1,103,574 tokens / 367 requests for 2026-05-22). All 4 BetterStack monitors UP. 0 cost-threshold alerts last 7d. Cloudinary at 69.88% of monthly limit (under 80% threshold). 0 new Sentry events since session start. **Both repos clean at EOD.** Backend HEAD `bcf8e11`. Frontend HEAD `799cf5f`. Previously (v5.9, 24 May earlier today): **Phase 107c SHIPPED** (backend `bcf8e11`). OpenAI cost-watchdog endpoint swap unblocked end-to-end after admin-key smoke-test confirmed the fix shape was correct. `fetchOpenAI()` in `costWatchdogService.js` now reads `OPENAI_ADMIN_API_KEY` (admin token, `sk-admin-` prefix, set on Railway prod 24 May) and hits `/v1/organization/usage/completions` with unix-timestamp window math. New response shape (`data[].results[]` with `input_tokens` + `output_tokens` + `input_cached_tokens` + `num_model_requests`) parsed into extended `_summary` (`total_tokens`, `input_tokens`, `output_tokens`, `cached_tokens`, `num_requests`, `buckets`). **`OPENAI_API_KEY` (sk-proj-) retained** - admin keys are read-only for org usage and cannot make inference calls; project keys cannot read org usage. Both keys permanently required. Live verification: post-deploy `collectAndPersist()` on prod inserted 2026-05-22 row with `total_tokens: 1,103,574` (945,535 input + 158,039 output + 284,672 cached) across 367 requests. Previous rows on 21/20 May stuck at `total_tokens: 0` (legacy code hitting deprecated `/v1/usage`). 4-edit apply-script ran cleanly via `/tmp/phase107c_openai_swap.py` - the malware-reminder false-positive on `costWatchdogService.js` still fires on direct Read but does not block apply-scripts in `/tmp/`. **2 new lessons banked:** (a) admin-key smoke-test against the actual endpoint with the actual credentials BEFORE any code swap saves hours of debugging - pattern reusable for any external-API integration change; (b) OpenAI two permanent key types (project vs admin) are non-overlapping by design - any future OpenAI feature must know upfront which key class it requires (inference = project; org-usage = admin). Previously (22 May LATE EVENING) — 12 additional commits after the v5.5 cutoff, all live on prod. **Late-evening ships (10):** (A) Phase 101 Sub-C hotfix (backend `5f99627` + frontend `8652997`) — Tata reported "Failed to load challenge" on every challenge click. Root cause: a JS `//` comment embedded inside the `getChallenge` SELECT template literal at `corporateController.js:287` (shipped by Phase 101 Sub-C Remind-button feature). Postgres parsed `//` as syntax error → 500 → frontend bare-catch swallowed the message. One-character fix (`//` → `--` SQL line comment) on backend. Frontend `loadDetail` bare-catch upgraded to Phase 64 visibility pattern (surface `err.response.data.message` via toast + `console.error`). Affected every seeker persona since 19 May (3-day silent regression). Cohort-verified end-to-end on Tata Enterprise. (B) Phase 104 + 104b Document Repository (backend `6d8a399` + frontend `c598b9b`) — Cohort bug "Upload Document not clickable; folder counts not showing on click". Two distinct bugs: (1) Upload button was a Phase 66/T9-pattern stub with no onClick (6th instance). (2) Sidebar folders were hardcoded fictional DRDO demo data (`ArmorTech AI` / `DroneShield Systems` / `QuantumDefense` sub-folders) with hardcoded counts (12/8/6/15/10/7) that had zero relationship to actual `files` state. Backend Phase 104: privacy scoping on `list/getOne/update` — pre-Phase-104 controller returned every row in `documents` table to every authed user (audit confirmed 0 rows on prod so no actual leak occurred, but gate is now in place). NEW `buildPrivacyClause(user, startIdx)` helper: visible when `uploaded_by=user.id OR access='public' OR role IN ('admin','evaluator')`. `update()` ownership gated similarly. Frontend Phase 104b: full rewrite. Dropped hardcoded `FOLDERS` const + fictional sub-folders. NEW `CATEGORY_SUGGESTIONS` array (7 categories) + `FOLDER_COLOR_ROTATION` palette. NEW `dynamicFolders` `useMemo` derived from real `files` data (Object.entries reduce-by-category). NEW Upload Document modal: file picker (10MB max) + name + folder dropdown + Public/Internal/Restricted toggle + tags. NEW Eye / Download / Share onClick handlers (Eye → preview modal, Download → window.open(file_path), Share → navigator.clipboard.writeText). Phase 64 visibility upgrade on every catch. (C) Phase 104d Watchlist multi-select (frontend `ebe75bd`) — Cohort bug "User is able to select only one startup at a time. Multi selection is not available. On clicking the Add Button an exception is shown." Root cause: row-level `+ Add` button had `onClick={() => { addStartup(s.id); setShowAdd(false); }}` — clicking ANY row immediately closed the modal. Single-select only. User-requested fix: checkbox UX + bulk Add. NEW `addSelected` Set state. `addStartup(arg)` supports BOTH single-id (scalar) AND array (parallel Promise.all + single success toast). Modal body: per-row `+ Add` button replaced with row checkbox + clickable-row toggle. Selected rows highlighted with gold tint. NEW "Select all visible" master checkbox + selection counter at top. NEW sticky footer with Cancel + "Add N Selected" gold button (disabled when 0). Defensive `(l.startupIds || [])` guard on local state update. (D) Ship #10 follow-up — persistent event_registrations (backend `3db94d9` + frontend `64b6432`) — closes deferred follow-up from morning Ship #10. NEW `event_registrations(id, event_id FK, user_id FK, created_at, UNIQUE(event_id, user_id))` table. Pre-existing `events.registered` counter column preserved + auto-synced via `COUNT(*)` recompute. Controllers rewritten: `register()` now does transactional INSERT ON CONFLICT DO NOTHING + recompute. NEW `unregister()` handler. NEW `myRegistrations()` returns `{event_ids: [...]}` for current user. Frontend EventsRepository.jsx mount effect: parallel fetch of events list + user's registration ids via `Promise.all`. `registeredSet` seeded from backend — refresh now keeps the Register button disabled across page reloads. (E) Ship #12 follow-up — 4 more page tours (frontend `73dafa5`) — Marketplace + Directory + CorporateChallenges + CorporateRecommendedStartups PAGE_TOURS entries. Anchor IDs follow `tour-page-<slug>-<element>` convention. Each anchor wrapped on a STRUCTURALLY-UNIQUE container (per Ship #12 hotfix lesson). Frontend-only, no backend changes. (F) Ship #4 follow-up — tokenized public watchlist share (backend `eb9be98` + frontend `9e898e5`) — replaces clipboard deep-link with proper public-shareable tokens. Locked spec: 30-day default expiry + manual revoke. NEW `watchlist_shares(id, watchlist_id FK, token VARCHAR(64) UNIQUE, created_by FK, expires_at, revoked_at, created_at)` table + `idx_ws_token` partial UNIQUE (WHERE revoked_at IS NULL). 4 new routes: `POST /watchlists/:id/shares` (owner+admin, crypto.randomBytes(32) URL-safe token + clamp 1-90 days), `GET /watchlists/:id/shares` (owner+admin), `DELETE /watchlists/shares/:shareId` (revoke, JOIN to enforce owner), `GET /public/watchlists/share/:token` (NO auth, 404 unknown / 410 revoked-or-expired / read-only payload). Frontend: NEW `SharedWatchlist.jsx` public page modeled on SharedChallenge.jsx + route `/watchlists/share/:token`. StartupWatchlist.jsx: Share button replaced with Manage Shares modal showing status badges (ACTIVE green / EXPIRED amber / REVOKED red) + Copy + Revoke per row + "Create new share link" gold button. (G) Phase 105a — 6 new bell notification hooks (backend `c6ee473`) — locked spec from user request "essentially new notification for every user to get their attention and action they need to take." Six new kinds: `watchlist_added` (when watchlistController.addStartup actually INSERTS via ON CONFLICT DO NOTHING RETURNING id, notifies added startup), `watchlist_share_viewed` (when publicReadByToken hit on active share, notifies creator, THROTTLED 1 per 24h via new `last_notified_at` column + atomic UPDATE...WHERE last_notified < NOW() - 24h RETURNING id — race-safe), `event_published` (when events.publish flips draft → public, notifies users with matching `startup_profiles.sector` ILIKE-ANY event.tags, capped at 200 to avoid runaway fan-out), `event_registered` (notifies event creator when event_registrations INSERT actually inserts), `profile_viewed` (notifies viewed user when profile_views INSERT WHERE-NOT-EXISTS actually inserts — naturally throttled to 1 per viewer/viewed/day by existing dedup), `challenge_applied` (notifies challenge corporate when startup applies, alongside existing email). (H) Phase 105b — daily deadline-reminder CRON (backend `7c5600d`) — NEW `src/services/deadlineReminderService.js` (~125 lines). Schedule: `30 21 * * *` (21:30 UTC = 03:00 IST). Three categories: `challenge_deadline_soon` (applied with empty pitch+proposal_url + deadline within 48h), `event_tomorrow` (event_registrations where event.start_date within 24h, not cancelled), `watchlist_share_expiring` (non-revoked share with expires_at within 72h, notifies creator). Idempotency: daily cron + narrow windows means single daily fire per (event, user) is acceptable. Registered alongside Phase 102-sentry watchdog in `cronService.initScheduler()`. (I) Phase 105c — email-fallback for meeting cancelled (backend `aeb2ad7`) — audit found most email-fallback scope was ALREADY wired in earlier phases (applicationStatusEmail, meetingInviteEmail, meetingRsvpEmail, connectionRequestEmail, connectionAcceptedEmail). The ONE gap was `meeting_cancelled` — Ship #2b added the bell but not the email. NEW `meetingCancelledEmail()` template wired into existing Ship #2b cancellation hook. **Notification surface (FULL after Phase 105):** 14 bell events (challenge_invite, app_<status>, new_message, connection_request, connection_accepted, meeting_scheduled, meeting_cancelled, ipr_linked, watchlist_added, watchlist_share_viewed, event_published, event_registered, profile_viewed, challenge_applied) + 3 CRON events (challenge_deadline_soon, event_tomorrow, watchlist_share_expiring) + 6 email channels (app status, connection req/accept, meeting invite/RSVP/cancelled). **38 total commits since 21 May morning across both repos. 17 ships today.** All known cohort bugs resolved. Both repos clean. Final state on prod EOD 22 May LATE EVENING: backend HEAD `aeb2ad7`, frontend HEAD `9e898e5`. **3 new lessons banked in CLAUDE.md Don'ts:** (a) `ON CONFLICT DO NOTHING + RETURNING id` is the cleanest idempotency-firing pattern for notification hooks — avoids "spam recipient on every re-attempt" footgun; (b) `last_notified_at` column + atomic UPDATE...RETURNING is race-safe throttle for fan-out events; (c) audit-before-spec for "this thing should have X" requirements — Phase 105c shipped 32 lines instead of budgeted 80 because 30 seconds of `grep sendEmail` found 5/6 already wired. Previously (v5.5, 22 May morning + early afternoon): CARRY-FORWARD QUEUE CLOSED end-to-end. All 5 deferred ships from 21 May session shipped. **Ships:** (1) Ship #10 Event Register disable + Brochure (backend PDF) + Calendar (multi-format) — `cc80332` backend (NEW generateEventBrochurePdf in pdfService.js, NEW brochure() handler + GET /events/:id/brochure route, resolveOrgName via existing helper) + `54c5d85` frontend (in-session registeredSet useState for disable-after-click, handleDownloadBrochure authed fetch+blob+anchor, Calendar dropdown with .ics/Google/Outlook deep-links via pure-function helpers buildIcsContent / buildGoogleCalUrl / buildOutlookUrl). (2) Ship #2b Notification Phase 2 — Meeting cancelled + IPR linked (`35093f0` backend, 2 hooks reusing existing notifyAsync/notifyManyAsync from Ship #2a). meetingController.updateMeeting fires meeting_cancelled when status='cancelled' (notifies all participants except organizer). iprController.create fires ipr_linked when admin specifies startup_id via Phase 94 Ship 3 typeahead (lookup startup_profiles.user_id, skip silently if null). 9-event notification surface now complete. (3) Ship #4 Watchlist Export PDF + clipboard Share (`5fc0f24` backend + `44fd848` frontend) — NARROWED from public-share-with-token to clipboard deep-link. NEW generateWatchlistPdf (~100 lines, auto-paginating card-style rows). NEW exportPdf() handler + GET /watchlists/:id/pdf route. Frontend Export PDF authed fetch+blob (same shape as Ship #10 brochure); Share button uses navigator.clipboard.writeText for in-OpenI deep-link. Public-share-with-token deferred (proper security review needed). (4) Ship #5 Knowledge "Suggest an Article" for non-admin (`0436a0e` backend + `001f2a3` frontend) — Option (b). NEW suggestArticleEmail template in emailService.js. NEW suggest() handler in knowledgeController (lookup suggester via users table, fire-and-forget sendEmail to ADMIN_EMAIL env). NEW POST /knowledge/suggest route (auth-only, no role gate). Existing POST /knowledge stays admin-only. Frontend: useAuth + isAdmin gate. Button label changes by role — Add Article (admin, still stub) vs Suggest an Article (non-admin, opens modal with title/summary/URL/type fields). (5) Ship #12 Joy Ride per-page tour MVP — NARROWED (`227c9fc` frontend). NEW PAGE_TOURS export in src/config/tours.js — route-keyed dictionary separate from role-keyed TOURS. TourWrapper multiplexed with pageMode state + new 'openi-page-tour' window event listener. Page tours do NOT markTourSeen (replayable). NEW "Tour this page" topbar button gated by PAGE_TOURS[location.pathname] existence. MyProfile shipped as first MVP page with 3 anchor IDs (header/completeness/save). Remaining 4 target pages (Marketplace/Directory/CorporateChallenges/Recommendations) deferred until copy review of MyProfile tour. Pattern proven; follow-up = JSX anchors + PAGE_TOURS entries. **Lessons banked (5 added to CLAUDE.md Don'ts):** (a) Python regex anchors against JS module-export blocks are fragile — fall back to direct Edit; (b) heuristic anchors with [warn] fallbacks are acceptable for cosmetic but NEVER for required imports — must verify post-substitution; (c) PAGE_TOURS / TOURS multiplex pattern is reusable for any per-route vs per-persona walkthrough surface; (d) "done" screenshot from user replaces Railway poll loops; (e) backend-first deploy order honored on every ship today (zero half-shipped state). 23 commits since 21 May morning across both repos. Both repos clean. Memory + DOCUMENTATION.md updated. **Final state on prod EOD 22 May:** backend HEAD `0436a0e`, frontend HEAD `227c9fc`. 4 backend PDF templates (invoice + challenge + event brochure + watchlist). 9-event notification surface complete. PAGE_TOURS pattern proven. Previously (21 May LATE EVENING): TEST TEAM FEEDBACK BATCH session, follow-on to morning shipping work. 5 cohort-flagged issues closed end-to-end (Ship #3 Project Mgmt FK + #1 Infrastructure empty state + #11 Events delete + #6/7 RFI MCQ/PDF + #2a Notification system extension) + carry-forward 3-fix batch + (morning) staged-from-yesterday backend cutover + invoice template hardening. 18 commits total across both repos for the full day. **Afternoon ships:** (1) Ship #3 Project Mgmt FK violation — Path A SERIAL collision fix (commits `030f5cd` backend ALTER projects ADD COLUMN startup_user_id INTEGER REFERENCES users(id) + dual-column controller + COALESCE display + `17c097b` frontend swap to startup_user_id payload). Same pattern as Phase 89.7 IPR — keep legacy FK, add new FK pointing at correct entity, COALESCE display, controller accepts either key. (2) Ship #1 Infrastructure & Test Facilities empty state (commit `d81482b`) — Option B better empty state, not the cohort's literal hide-sidebar ask. Conditionally hides KPI tiles + search bar when infraList.length === 0; renders gradient empty-state card with onboarding copy + mailto:info@openi.ai request link. (3) Ship #11 Events Repository delete (commits `c4b9f9d` backend + `28125b8` frontend) — NEW DELETE /events/:id route + destroy() handler with inline gate (creator + active same-org member via org_members JOIN + admin), auditLog('delete_event'), Trash button on EventDetail (UI-gated to creator). (4) Ship #6+7 RFI MCQ + PDF (`91c4590` backend PDF padding 60→72 + `dbbad1e` frontend per-option Add/Remove input + DB backfill 1 row). Storage bug at source: comma-separated text input replaced with per-option Add/Remove UX. Backfill split `["YesNO"] → ["Yes","NO"]` on challenge 26 with defensive known-pattern matcher (YesNO/YesNo/NoYes/TrueFalse/FalseTrue + camelCase fallback). PDF tile padding bumped 60→72. (5) Ship #2a Notification system extension (commit `65dfb48`) — was 1 event type (challenge_invite from Phase 99e), now 8. NEW src/services/notificationService.js (~50 lines, lifts createNotification + adds notifyAsync + notifyManyAsync helpers). 7 controller hooks: corporateController.updateApplication → app_<status> notification on ANY status change (statusTitleMap for shortlisted/selected/rejected/evaluating), messageController.sendMessage → new_message to all OTHER conversation_members, connectionController.sendRequest → connection_request to recipient, connectionController.respondToRequest (accept only) → connection_accepted to requester, meetingController.createMeeting → meeting_scheduled to each non-organizer participant. Fire-and-forget pattern via .catch(err => console.error). Frontend bell (Phase 101 Sub-D) picks up automatically on 60s poll. Smoke-test PASSED: 5 kinds inserted via notifyAsync in-process on prod → all 5 landed → cleaned up transactionally. Ship #2b deferred (Meeting cancelled + IPR linked, ~30 lines). 5 other test-team issues deferred with locked specs (Ship #4 Watchlist Export+Share NARROWED to backend PDF + clipboard deep-link, Ship #5 Knowledge Suggest article for non-admin via email-to-admin, Ship #10 Event Register disable + brochure PDF + multi-format calendar, Ship #12 Joy Ride per-page tour MVP for top-5 pages). #8 8-Vector Evaluation visibility blocked on cohort screenshot. #9 "Merge Invites+Marketplace?" answered: no, keep separate. **Morning ships (carry-forward from 20 May staged):** Phase 87c-6 Ship 1 backend cutover (`2d61d9f`) — retire legacy text[] chip columns in favor of JSONB _v2 from Phase 87c, controller V2_WRITE + getOne shim flattens _v2 → string[] under legacy keys. Phase 94 Ships 1-3 per-org IPR portfolio (backend `2d61d9f` + frontend `28e9af8`) — NEW /ipr/my-portfolio route + buildPortfolioFilter helper, closed pre-existing read leak on /ipr (now requireRole admin/evaluator), IPR Portfolio nav re-added to 5 personas with read-only semantics, search-driven startup picker on admin Add modal. Invoice template hardening — 4 narrow-scoped commits: meta-block label width 90→70 (`7bbf276`), USD export template with USD_INR_RATE=83.50 constant + fmtMoney + numberToWesternWords (`eb1204b`), INDIA_ALIASES Set tightening (`c76daab`), Place of Supply parenthetical → dedicated Type of Supply row (`3b2646f`). DOCUMENTATION.md v5.3 (`5f8d12c`). Carry-forward 3-fix batch (`911f315`) — admin What's New nav entry + BodyBullets multi-format split + select_dependent friendly placeholder. **Ops:** LUT_ARN=AD2705260161482 live in container, Dentsu qosadiqy application restored after wrong DELETE (new row id=6 preserved created_at=2026-05-12, first INSERT attempt failed because status='Applied' violated lowercase CHECK — fixed). Lessons banked: (a) Path A pattern for SERIAL collision FK fixes is now battle-tested; (b) storage bugs masquerading as render bugs need DB audit first; (c) heredoc commit messages with apostrophes still break bash — default to git commit -F /tmp/msg.txt; (d) fire-and-forget notification pattern is the right shape for non-financial event hooks; (e) smoke-test notifications in-process via the service helper, not over HTTP. Previously (21 May morning): STAGED-FROM-YESTERDAY-NOW-FULLY-SHIPPED session. Railway healthy all day (yesterday's incident fully resolved). 6 commits live on prod + 1 operational restore. Three workstreams completed end-to-end: (A) **Phase 87c-6 Ship 1 + Phase 94 Ships 1-3 staged work landed cleanly** (commits `2d61d9f` backend + `28e9af8` frontend) — all 4 backend verification gates passed (LUT_ARN live in container, `/ipr/my-portfolio` 401 = registered, `/ipr` 401 = now admin-gated closing pre-existing read leak, shim flattens `investor_names_v2` → `["Sequoia Capital","Accel"]` while preserving raw `_v2` alongside). (B) **Invoice template hardening — 4 narrow-scoped commits** for Finance-ready USD export invoice. `7bbf276` meta-block label width 90 → 70 (fits long invoice numbers on one line). `eb1204b` USD export-invoice template: `USD_INR_RATE=83.50` hardcoded constant (quarterly review with Finance), new `fmtUSD`/`inrToUsd`/`fmtMoney(amount, isExport)` helpers + `numberToWesternWords` (Thousand/Million/Billion + US Dollars/Cents vocab), 9 currency-aware money sites + total-in-words branches on `isExport`. Ship spec: USD-only (no GBP/EUR multi-currency), no FX rate displayed on invoice, Razorpay charges in INR but invoice displays USD-equivalent. `c76daab` INDIA_ALIASES tightening — new module-level `INDIA_ALIASES = new Set(['india', 'in', 'ind', 'bharat', 'भारत', 'republic of india'])` with `customerCountry.trim().toLowerCase()` lookup. Pre-flight audit confirmed 0 rows reclassified. Closes silent misclassification of ISO codes / Hindi variants as export. `3b2646f` Place of Supply parenthetical → dedicated row — `(Outside India)` parenthetical removed from inline `placeOfSupply`, new conditional `['Type of Supply', 'Export of Services']` meta row appended for export. Two single-line rows on export (never overflows regardless of country-name length). Domestic stays at 3 meta rows. Smoke-tested with 3 cases (UK + India literal + India ISO 'IN') — all PDFs render correctly. (C) **Dentsu qosadiqy application restored** (no commit) — yesterday's wrong DELETE reversed via transactional re-INSERT in `challenge_applications`. New row id=6 preserved `created_at='2026-05-12'` (9-day age). First INSERT attempt failed because status='Applied' violated lowercase CHECK constraint — fixed and re-ran. Karuna's review queue now shows the application again. Lessons banked: don't trust JOIN-output column aliases for enum INSERTs (query `pg_constraint` directly); don't put non-ASCII in `git commit -m heredoc` (use `-F /tmp/msg.txt`); audit before tightening any semantic classifier; Railway "REMOVED" deploy label means superseded not reverted; ship narrow-scoped invoice fixes per concern not batched. Sample artifacts in `~/Downloads/test-uk-v2.pdf` (canonical USD export sample for Finance review) + `test-india-literal-v2.pdf` + `test-india-IN-v2.pdf` (INR proofs). Previously (20 May): STAGED-NOT-COMMITTED work session due to Railway Service Disruption. Three workstreams: (1) LUT_ARN env var set on Railway prod to `AD2705260161482` per Finance team direction (verified persisted server-side via `railway variables --json`, awaits live container restart to take effect on export invoice PDFs — renders as `(LUT ARN: AD2705260161482)` suffix in the existing declaration string at `pdfService.js:524-528`). Closes a months-long carry-from-8-May open item. (2) Phase 87c-6 Ship 1/4 backend cutover for retiring legacy text[] chip columns (`investor_names`, `accelerator_programs`, `corporate_partners`) in favor of JSONB `_v2` from Phase 87c — audit pre-flight confirmed 0 drift between legacy + _v2 across 421,986 populated rows, dual-write hook rewritten to `_v2`-only with `delete updates[legacyKey]` guard, `startupController.getOne` gains a shim that flattens `_v2` JSONB → string[] under legacy key (Path B chosen over frontend-fanout for single-point absorption of the shape change). 4 backend files modified +32/-20. (3) Phase 94 Ship 1-3 for per-org IPR portfolio view to 5 personas (corporate, government, investor, accelerator, incubator) — recon revealed `ipr_records` is empty in prod (0 rows) AND that `GET /ipr` + `GET /ipr/:id` had no role gate (pre-existing read leak). Ship 1: NEW `GET /ipr/my-portfolio` route + `buildPortfolioFilter(role, userId)` helper with per-persona JOIN paths (corporate/govt via challenges, investor via investor_names_v2 ↔ firm_name, accelerator via accelerator_programs_v2 ↔ accelerator_name, incubator empty until junction table ships); existing /ipr routes now `requireRole('admin','evaluator')`. Ship 2: `iprAPI.myPortfolio()` helper, IPR Portfolio nav added to 5 personas' actions arrays, IPRDatabase.jsx persona-aware via `useAuth` (title "IPR Database" admin vs "IPR Portfolio" org-persona, fetcher endpoint switched, Add button admin-only). Ship 3: search-driven startup typeahead picker on admin-only Add modal (Phase 89.8/T31 + 87b race-guard pattern), Owner field renamed to "Co-owner / Notes (optional)", payload uses `form.startup_id` (was hardcoded null). 2 backend files +109/-3 + 3 frontend files +138/-15 staged. Dentsu audit run (Karuna Dagur confirmed free plan, 1 challenge, 0 invites sent, 1 mailinator junk application sitting in queue 8 days — DELETED transactionally; user 602045 retained because `audit_logs.user_id_fkey` is RESTRICT; 0 Sentry errors on /applications/* last 7 days; backend application flow healthy). Critical deploy order when Railway recovers: backend Phase 87c-6 Ship 1 + Phase 94 Ship 1 commit together → wait for redeploy → smoke-test 4 endpoints → ONLY THEN push frontend Phase 94 Ship 2+3. Detailed resume protocol at top of `OPENI_HUB_NOTES.md` "Session wrap — 20 May 2026". 5 new lessons banked in CLAUDE.md Don'ts. Previously (19 May LATE evening): Phase 102 — Full Observability Watchdog, full ship end-to-end on prod same evening as Phase 99/100/101). Three layers + threshold alerts + admin dashboard, all behind requireRole('admin'). Phase 102-1 (commits d6163a1 + 5256448) — NEW service_costs_daily table + cron at 02:00 IST + 4 fetchers (Cloudinary + OpenAI + Vercel + Resend). Railway auto-collect dropped pragmatically (Railway GraphQL usage query rejected hand-rolled variables, docs don't expose the right syntax). Phase 102-2 (d353163 + cf7b4bf) — NEW costAlertService.js + service_alerts_fired cooldown table + 7 threshold rules covering Cloudinary credits (80/100/130%), OpenAI token spike (3x/10x 7-day avg), Vercel bandwidth, Resend volume spike (5x), and a generic service-stale-3d. 24h cooldown per (service, threshold). Emails via existing emailService.sendEmail to ALERT_EMAIL_RECIPIENTS. Phase 102-3 (de131db backend + df4e6c4 frontend) — NEW adminCostsController.js + 4 routes + NEW service_manual_costs table + NEW page /dashboard/admin/costs (lazy-loaded, recharts) with 5 panels: Service Status grid, Recent Alerts, Manual Costs + Record Cost modal, 30-Day Trend per service. Sidebar nav entry added. $104 Railway May entry recorded via the modal. Phase 102-uptime (ca0fde1 backend + 4ac34f0 frontend) — NEW uptimeWatchdogService.js + service_uptime_daily table + BetterStack /api/v2/monitors hourly polling. NEW threshold rule uptime_below_99. NEW route GET /admin/uptime/summary + Uptime Status panel on AdminCosts. 4 monitors live (openi.ai, api.openi.ai/api/public/challenges, openi.ai/marketplace, www.openi.ai), all up. Sentry frontend noise filter — main.jsx#Sentry.init ignoreErrors filter for /Unexpected end of JSON input/ + /Failed to execute 'json' on 'Response'/ from defensive api.js#request() catch. Phase 102-sentry (1efe4eb backend + c763200 frontend) — NEW sentryWatchdogService.js polling both openi-hub-backend + openi-hub-frontend projects via Sentry /api/0/projects/.../issues/. NEW service_errors_daily table + hourly cron at :05 + errors_spike_24h alert rule. NEW route GET /admin/errors/summary + Recent Errors panel on AdminCosts. Caught real perf issue: OPENI-HUB-BACKEND-2 N+1 query on GET /api/public/landing-content (5 events, last seen 03/05/2026). Phase 102-resend (untagged) — fetchResend() in costWatchdogService pulling /emails endpoint. NEW volume_spike_5x alert rule. Required RESEND_API_KEY permission upgrade Sending → Full Access via Resend UI (same key value, broader scope). Required new env vars: ALERT_EMAIL_RECIPIENTS, BETTERSTACK_API_TOKEN, SENTRY_API_TOKEN, SENTRY_ORG_SLUG, SENTRY_PROJECT_SLUGS. Lessons banked: 8 new entries in CLAUDE.md Don'ts covering token scope traps, JSONB schema choice, monthly-vs-daily semantic verification, recon agent fabrication, push-vs-pull alerting layers. Earlier today: Phase 101 application review enhancements (Invited chip + Remind button + notification bell + Marketplace chip), Phase 100 review UX (3 hotfixes), Phase 99 T32 private challenges. Yesterday (18 May): Phase 89.3 → 89.9 cohort batch + Phase 97 session hardening + Phase 96 Projects/Tasks wiring.
...
### What's New in v5.8 - Phase 107 + 107b cost watchdog extension (23 May 2026 EVENING)

**2 backend commits. Cloudflare + Strapi added to the Phase 102 cost watchdog. OpenAI gap diagnosed but not fixed (operational, not code). Two secret rotations after chat-history leaks.**

#### Why this shipped

User asked to add Cloudflare + Strapi to the watchdog dashboard so we know when free tier limits get hit. Mid-implementation discovered:
- OpenAI panel was showing 0 tokens (not a Phase 107 bug, but surfaced during recon)
- Strapi is self-hosted on Railway, not Strapi Cloud SaaS as initially assumed
- Resend was already collected by Phase 102 watchdog but missing from the dashboard status grid

#### Two commits

| Phase | Commit | What |
|---|---|---|
| 107 | `dab66d7` | Add fetchCloudflare() + fetchStrapi() (Cloud-SaaS assumption) + 4 Strapi Cloud threshold rules + extend KNOWN_SERVICES with cloudflare/strapi/resend + per-service status grid messages |
| 107b | `4d4de81` | Rewrite fetchStrapi() for self-hosted instance (reads /_health + per-collection counts), remove the 4 Strapi Cloud threshold rules (free tier doesn't apply), update Strapi status grid message |

#### Cloudflare (Phase 107a)

- **What we track:** GraphQL zone analytics — `requests_24h`, `bandwidth_gb_24h`, `cache_hit_ratio`
- **Endpoint:** `https://api.cloudflare.com/client/v4/graphql`
- **Env vars:** `CLOUDFLARE_API_TOKEN` (scoped: Account Analytics:Read + Zone Analytics:Read + Zone:Read on openi.ai), `CLOUDFLARE_ZONE_ID=92de8f949b05803a7f1a710f81c9a88a`
- **Thresholds:** none — free tier is effectively unbounded for our DNS-primary use case. The generic `service_stale_3d` rule catches collection failures.
- **Status grid display:** `N requests (24h), M GB bandwidth, P% cache hit`

#### Strapi self-hosted (Phase 107b)

- **What we track:** `/_health` endpoint up/down + per-collection counts via public Strapi v4 API (`/api/<col>?pagination[pageSize]=1`)
- **No auth token required** — collections are public-readable (frontend uses same path)
- **Env vars:** `STRAPI_BASE_URL=https://openi-hub-cms-production.up.railway.app` + optional `STRAPI_COLLECTIONS=landing-page,testimonials,faqs` (defaults match prod)
- **Thresholds:** none — self-hosted = no free tier caps to track. `service_stale_3d` catches multi-day outages.
- **Status grid display:** `✓ Up — N collections, M entries` or `✗ <error> — N collections, M entries`
- **Discovery:** probed openi.ai prod Strapi → found 3 public types: `landing-page` (single-type), `testimonials` (5 entries), `faqs`. Frontend pulls landing-page content via this path.

#### Resend (bonus fix in Phase 107)

- Was already in `fetchResend()` (Phase 102) but missing from `KNOWN_SERVICES` in `adminCostsController.js`. Dashboard didn't surface it. Fixed in 107.
- Status grid display: `N emails sent yesterday`

#### Critical near-misses + lessons

1. **`railway variables --json | grep <SERVICE>` leaked full token values into chat.** Both Cloudflare and Strapi tokens were exposed mid-session. Both rotated immediately. Lesson: never `grep` env var output — use `python3 -c "...[value][:8]"` to truncate.
2. **"Self-hosted" vs "SaaS Cloud" version mismatch.** Initial Phase 107 spec assumed Strapi Cloud SaaS at `api.strapi.io`. Reality: Strapi self-hosted on Railway. Phase 107b rewrite caught and fixed. Same trap could happen with PostHog, Plausible, Sentry, Matomo — anything offering both SaaS + self-hosted.
3. **Vercel "Redeploy" UI button rebuilds OLD commit + OLD env vars** (banked Phase 106). Today's variant: setting Strapi env var typo + clicking UI Redeploy succeeded on stale commit. Pattern: to rebuild with new env vars, push a new commit (empty if needed).
4. **Strapi v4 `/_health` returns 204 (No Content).** Watchdog must treat 204 as `up`. Same shape for many Node-based health endpoints.
5. **Apply scripts with box-drawing-char anchors are fragile.** The Phase 107b apply script aborted because the `─` separator length didn't match the file exactly. Fell back to direct Edit tool calls. Reusable lesson: prefer text-only anchors over decorative-Unicode anchors.

#### What still needs attention (carry-forward)

| Item | State | Trigger |
|---|---|---|
| OpenAI 0-tokens fix | Diagnosed — OpenAI key lacks `usage.read` scope OR endpoint moved to `/v1/organization/usage/completions`. Code change is 1-line endpoint URL OR operational key rotation. | Whenever you want to triage |
| Lint warning cleanup (~150 warnings) | Doesn't block builds | Low priority gradual cleanup |
| Phase 87c-6 Ships 2-4 (legacy text[] DROP COLUMN) | Wait for Ship 1 soak | Conservative cleanup |

#### Final state EOD 23 May 2026 EVENING

- Backend HEAD: `4d4de81` (Phase 107b)
- Frontend HEAD: `e225adb` (DOCUMENTATION.md v5.7 from earlier today; this commit bumps to v5.8)
- Both repos clean
- Railway deployment online (Phase 107b active)
- Cost dashboard panels after next 02:00 IST cron tick: cloudflare ✓, strapi ✓, resend ✓ (previously dark)
- Phase 102 stack now covers: 7 services (railway-manual + cloudinary + openai + vercel + resend + cloudflare + strapi) + uptime monitors + Sentry errors
