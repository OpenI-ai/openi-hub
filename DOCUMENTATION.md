...
**Version:** 5.54
**Last Updated:** 8 Jul 2026 (session 39 — documentation-reconciliation session for 4 previously-undocumented commits shipped by a context-limited session: sector icon/color centralization to `src/constants/knowledgeIcons.js` removing duplicated maps from `Knowledge.jsx`/`PublicReports.jsx` and fixing a `'Cyber Security'` vs canonical `'Cybersecurity'` key mismatch (FE `a2d473f`); file view/download 401 fix — added `openProxyFile()` blob-fetch helper carrying the `Authorization: Bearer` header, since plain `<a href>` links can't (FE `5c498d4`); Express `trust proxy` mixed-content fix — `app.set('trust proxy', 1)` so `req.protocol` resolves correctly behind Railway's edge TLS termination, also fixing `express-rate-limit` client-IP detection (BE `bced4df`); Knowledge Hub view-count fix — `openArticle()` now opens from cache immediately then merges the server-incremented count via a background `knowledgeAPI.get(id)` fetch, **live-verified** by user screenshot (BE unaffected, FE `048f9da`); backend HEAD `79a9721` → `bced4df` / frontend HEAD `69c94bd` → `048f9da`, version bump v5.53 → v5.54) — 7 Jul 2026 (session 38 — Investor Applications pitch-text truncation fix + Deal-request editor-collaborator write-access fix + Deal Invite non-registered-user investigation (concluded not-a-bug); also retroactively logs session 37's path-to-regexp emergency hotfix, never documented until this pass; backend HEAD `9496d87` → `79a9721` / frontend HEAD `0016980` → `3b300e5`, version bump v5.52 → v5.53) — **Session 37 (7 Jul 2026, backend HEAD `9726627` → `9496d87`, frontend unchanged, emergency hotfix, retroactively logged this session):** Total API outage (every endpoint 502) immediately following session 36's file-proxy commit. Symptom looked like two independent frontend bugs (Knowledge Hub showing 0 articles, public Reports page "0 reports") because both pages fail-gracefully to an empty array on fetch error — root cause was `src/routes/index.js` registering `router.get('/files/:type(pitch-deck|resume|portfolio|knowledge)/:id', ...)`, an Express-4-era `:param(a|b|c)` alternation-group route pattern that Express 5.2.1's bundled `path-to-regexp@8` no longer supports at all — it throws `TypeError: Unexpected (` synchronously during route registration, before `app.listen()`, crashing the entire process at boot. Confirmed via Railway runtime logs showing the exact `path-to-regexp` stack trace. **Fix (BE `9496d87`):** changed the route to a plain `:type/:id` param; `fileProxyController.streamProfileFile` already validates `type` against its `PROFILE_FILE_MAP` allow-list and 404s on unknown values, so the route-level regex constraint was redundant. **Deploy gotcha:** `railway redeploy` re-deployed the still-broken current commit (it does not pull new source) — the actual fix went live via the separate GitHub-webhook-triggered deploy, ~45s after `git push`. **DON'T-REVERT (session 37):** any Express 5 route needing enum/alternation path matching must validate inside the handler, never via `:param(a|b|c)` route-pattern syntax. **Diagnostic principle banked:** when two unrelated pages show identical "empty state" symptoms simultaneously, `curl` the raw API directly before assuming a frontend/data bug — a bare 502 instantly reframes the investigation. — **Session 38 (7 Jul 2026, backend HEAD `9496d87`→`79a9721`, frontend HEAD `8eec641`→`3b300e5`):** Three independent items, one read-only investigation + two shipped fixes. **(A) Deal Invite investigation (read-only, no code shipped):** user reported "Deal Invite sent to non-openi users not getting triggered." DB investigation via `railway ssh` diagnostic scripts found: the email pipeline itself is healthy (172+ successful Resend/SendGrid sends in the lookback window, zero failures, `email_outbox` clean); `pending_email_invites` has **zero** rows for `entity_type='application_invite'` (the non-registered-user branch never fired); `entity_application_invites` has exactly 3 rows, and all 3 `invited_user_id`s resolved to real, pre-existing registered accounts (confirmed via `users` table lookup) that correctly received a `kind:'application_invite'` `user_notifications` row — i.e. the in-app-notification-only path fired exactly as designed for an existing user, no email is supposed to send in that case. Code review of `applicantInviteController.js#createInvites`'s per-email loop found no batch-processing bug (each email independently checked via `findUserByEmail()`, independently try/caught, no shared state). **Conclusion: every tested invite email happened to already belong to a registered user — expected behavior, not a bug** — unless the actual failing test used a genuinely unregistered email, which was not confirmed with the user before the thread moved to the testing team. **Open item:** get the specific email address tested, if the testing team reproduces this again. **(B) Applications pitch-text truncation fix (FE `3b300e5`):** user asked "The entire applications content is not visible to the Investor. Is it because of the Free plan?" re: the My Deal Sourcing → Applications list. Investigation (`InvestorDealRequests.jsx` line ~215) found a pure Tailwind `line-clamp-2` CSS truncation with **no expand affordance at all** — confirmed NOT a plan-tier restriction (no `plan`/`subscription`/`tier`/`isPremium` field exists anywhere on the `user` object; backend `getDealRequest` returns the full untruncated `pitch` to every investor regardless of plan, access control is ownership/collaborator-only). Fix: added per-application `expandedApps` state + a length-gated (`pitch.length > 110`) "Show more"/"Show less" toggle button that swaps the `line-clamp-2` class off when expanded. **(C) Deal-request editor-collaborator write-access fix (BE `79a9721`, shipped 22:52 same day, previously undocumented until this pass):** `PUT /investor/deal-requests/:id` (`updateDealRequest`) only checked `investor_id=req.user.id` — an accepted `entity_collaborators` editor got 0 rows back and a 404 on save, even though the read path (`listDealRequests`/`getDealRequest`, session 34) already granted them view access. Applied the same owner-OR-accepted-editor-collaborator `EXISTS` check (matching the Challenges/`challenge_members` pattern) to `updateDealRequest`, `updateDealRequestApplication`, and `promoteDealRequestToPipeline`. Also fixed `promoteDealRequestToPipeline` inserting the new pipeline deal under `req.user.id` instead of the deal request's actual owning `investor_id` — previously, a collaborator promoting an applicant would misattribute the resulting deal to themselves instead of the real owner. **Deployment:** backend `79a9721` confirmed SUCCESS via Railway deployment list; frontend `3b300e5` confirmed READY/production via Vercel `list_deployments`. Neither (B) nor (C) has been re-clicked live by the user/testing team this session — deploy-confirmed only. **DON'T-REVERT (session 38):** the Deal Invite existing-user path is correct as-is — in-app notification only, no email — do not "fix" this into sending an email to existing users without an explicit product decision; `InvestorDealRequests.jsx`'s pitch-text expand/collapse uses a length heuristic (110 chars) to avoid showing a useless toggle on already-short pitches — keep the toggle, don't just remove `line-clamp-2` outright (removes the layout benefit for genuinely long pitches); `investorController.js`'s owner-OR-accepted-editor-collaborator `EXISTS` pattern must be applied to every future new deal-request write endpoint, not just read endpoints — this is now the 4th write path patched this way (after the session-34 read-path precedent) and there is no shared middleware doing it centrally yet, so each new endpoint must remember to add it manually. **Known gap flagged (not fixed, not this session's scope):** `DOCUMENTATION.md` itself is now 401 KB / ~3070 lines, well past this project's own PRUNE/SPLIT-BIG-FILE rule threshold (~256 KB) — flagged to next session's queue for a verbatim-verified split into `_partN.md` files, not attempted this session given the risk of a blind split on a single-physical-line file of this size. Backend HEAD `9496d87` → `79a9721`; frontend HEAD `8eec641` → `3b300e5`. DOCUMENTATION.md v5.52 → v5.53. Prior addendum retained below for continuity —**Session 36 (Cloudinary upload-corruption fix + Knowledge Hub sector tagging, no new frontend routes):** User reported a Knowledge Hub report displaying only a screenshot instead of the full document. Root-cause investigation found this was a symptom of a platform-wide upload-pipeline defect, not a Knowledge-Hub-specific bug: every document uploaded through `/upload` since folder-preset support was added (Phase 93a) had its original content permanently destroyed at upload time. **Root cause (`uploadController.js`):** top-level `quality:'auto:good'`/`fetch_format:'auto'` params were applied to the incoming/stored original (not just a derived `eager` copy) on every upload call, and combined with `resource_type:'auto'` on document folders (`pitch_decks`/`resumes`/`portfolios`/`knowledge_articles`/`data_room`), Cloudinary classified multi-page PDFs as image resources and permanently rasterized them to a single page-1 JPEG/WebP at upload time — the original was gone, not just the delivery URL. Confirmed via a production DB query (regex-matching stored URLs against Cloudinary's `/image/upload/...\.(jpg|jpeg|png|webp)` pattern): 10 rows corrupted across `startup_profiles.pitch_deck_url` (6), `student_profiles.resume_url` (2), `student_profiles.portfolio_url` (1), `knowledge_articles.file_url` (1). **Fix 1 — stop the ongoing corruption (BE `6bfc605`):** removed the top-level `quality`/`fetch_format` params from the base upload `opts` (these must only ever live inside a folder's own `eager` array, which `LOGO_PRESET`/`PDF_THUMB_PRESET` already did correctly); added a mimetype-driven override forcing `resource_type:'raw'` for Office documents (`.docx`/`.pptx`/`.xlsx`/`.doc`/CSV) so they never enter any image-processing path; response now always returns `url: result.secure_url` (the true original) with the eager thumbnail (if any) surfaced as a new, additive `thumbnail_url` field rather than substituting it for the original — confirmed backward-compatible via an explore-agent audit of every frontend call site (`FileUpload.jsx`, `MyProfile.jsx`, `AddArticleModal.jsx`, `DocumentRepository.jsx`, `Marketplace.jsx`, `Register.jsx`), all of which read only `.url`. **Fix 2 — the 10 already-corrupted rows are unrecoverable** (Cloudinary's own stored "original" is already the rasterized page-1 image — confirmed via `server-timing: oformat="jpg"` on a corrupted asset — there is no byte-exact original anywhere to restore); per-user remediation deliberately deferred pending explicit confirmation rather than silently mutating production rows (see session-36 Part B below). **Fix 3 — Knowledge Hub industry/sector tagging (Phase 124, BE `6bfc605`, FE `0016980`):** new `knowledge_articles.sector VARCHAR(100)` column (matches the `startup_profiles.sector` convention), wired through `knowledgeController.js` create/update and `publicController.js#listPublicReports` (`sector: row.sector || undefined`), reusing the existing `TaxonomySelect` component/`/api/public/taxonomy` endpoint — no new taxonomy system. Frontend `AddArticleModal.jsx` gained a sector picker; `Knowledge.jsx` displays the sector as a badge on cards/detail view, reusing existing `SECTOR_ICONS`/`SECTOR_COLORS` styling. **Also fixed in the same frontend commit:** `AdminKnowledge.jsx`'s contributor-requests fetch was unwrapping the list endpoint's plain JSON array as `res.requests || res.data` — always `undefined` for an array — so the Pending tab silently showed "No requests in this status." regardless of actual pending requests; approve/reject now work as intended. **Session 36 Part B (same day, read-only diagnostic + data-only remediation, no code/deploy):** re-ran the corrupted-row diagnostic query live in prod, confirming all 10 rows exactly as predicted; of these, 3 belong to real (non-test) accounts — `rohan@metricscart.com` (user_id 603206), `tyler.woolsey@scrunchai.com` (602530), `diana@promptwatch.com` (602890) — the remaining 6 are disposable/test-domain accounts and 1 is the admin's own Knowledge Hub article (re-uploaded directly, no remediation needed). Per explicit user decision ("in-app notification only"), inserted one `user_notifications` row per real user (`kind:'pitch_deck_reupload_needed'`, no existing CHECK constraint on `kind` so no schema change needed, idempotent re-run guard via a pre-insert existence check) directing them to re-upload their pitch deck from `/dashboard/profile`. Test/disposable accounts and the admin's own article were deliberately excluded from notification. **DON'T-REVERT (session 36):** top-level `quality`/`fetch_format` must never be set on the base upload `opts` — only inside a folder's `eager` array; Office-document mimetypes must stay forced to `resource_type:'raw'`; the upload response's `url` field must always be `result.secure_url` (never substitute a thumbnail/eager derivative for it) with `thumbnail_url` as a separate additive field; `knowledge_articles.sector` reuses the existing `TaxonomySelect`/taxonomy-sectors system, no new taxonomy table; `AdminKnowledge.jsx`'s contributor-requests fetch must read the endpoint's plain array directly, not `res.requests || res.data`. Backend HEAD `ff3e7d4` → `6bfc605`; frontend HEAD `a7efbe9` → `0ae0d26` → `0016980`. DOCUMENTATION.md v5.51 → v5.52. Prior addendum retained below for continuity —**Session 35 (Services landing section + bug-fix batch, no new schema):** Two independent workstreams. **(A) Corporate/Enterprise Services section (BE `ff3e7d4`, FE `d71c78b`, `c30c610`):** ahead of the "Open Innovation, Unfiltered" campaign (posts 3-5, 14-21 Jul) which promises startup fusion, sandboxes, synthetic data, and stealth-mode scouting — services OpenI delivers on top of the platform for the Corporate/Enterprise persona, previously with no landing-page section explaining them. Backend: `DEFAULT_LANDING.services` (6 cards — Solution Architecture, Startup Fusion, Managed Sandbox, Synthetic Data, Innovation Project Management, Stealth-Mode Scouting), `getLandingContent` result gains `services: mapFeatureCards(featureCards, 'services') || DEFAULT_LANDING.services` — reuses the existing `mapFeatureCards`/`featureCards` (Strapi `/feature-cards`, `section` filter) with **no new Strapi content-type** (Content-Type Builder locked in prod). Frontend `Landing.jsx`: `ICON_MAP` gains `Layers`; new `#services` section directly below "Built for Every Stakeholder", above "Features" — eyebrow "FOR CORPORATES & ENTERPRISES", 3-column `FeatureCard` grid (description-only, no per-card links, matching existing `FeatureCard` exactly), closer CTA → `/register` (no dedicated lead-capture flow exists yet, reuses the same convention as the "Contact Sales" Enterprise pricing tier); `#services` anchor added to desktop + mobile nav between "How It Works" and "Pricing". Follow-up `c30c610` fixed the closer CTA copy from "Talk to Us" to "Get Started for Free" to match the hero/final-CTA convention. **(B) Header-responsiveness + tour bug-fix batch, found during live verification of (A) and via user report:** **(1)** Adding the `#services` nav link overflowed the desktop header at 768-1023px (`d474cae`) — moved the nav/hamburger breakpoint from `md` (768px) to `lg` (1024px) so all 5 links + search only render together where there's room. **(2)** That breakpoint change then collided with the header search bar (also `lg:block`) in the 1024-1279px band (`b1d8326`) — nav/button group now `shrink-0`, search wrapper `min-w-0` so it absorbs the shrink, and the search bar's own reveal breakpoint deferred to `xl` (1280px). **(3)** At exactly 1280px the `SearchBar`'s rendered content (~370px) still slightly exceeded its flex-sized wrapper (~340px), overlapping the LinkedIn icon (`1ee155a`) — trimmed header gaps (`gap-4`→`gap-3` outer, `gap-6`→`gap-4` nav) freeing ~44px. **(4)** User-reported: "Take a tour" button did nothing on the `/register` persona-picker step (`cad01c0`) — `Register.jsx` renders Step 0 via its own early `return`, separate from the Step 1-3 return; `<PublicTour />` (listens for `openi-page-tour`, renders the Joyride overlay) was only mounted in the Step 1-3 return, so Step 0 had no listener at all. Added `<PublicTour />` to the Step 0 return. **(5)** User directive "pls use only '?' button" (`a7efbe9`): `PageTourButton.jsx` (shared by all 5 standalone pages — Register/Landing/VerifyEmail/ForgotPassword/ResetPassword) rendered as a bordered text pill ("Take a tour"), inconsistent with `PublicLayout.jsx`'s header tour button (icon-only circular `HelpCircle`). Switched to the same icon-only circular style (`width:36,height:36,borderRadius:'50%'`, `HelpCircle size={18}`, no text) — single shared-component edit updated all 5 pages at once. **Verification:** Services section confirmed live via `curl https://api.openi.ai/api/public/landing-content` → `services` array present, 6 items. Icon-only tour button + persona-picker tour fix both visually confirmed live on `https://openi.ai/register` and `https://openi.ai/landing` (screenshots). The 3 responsiveness fixes were NOT re-screenshotted across all 3 breakpoints this session after the final (`1ee155a`) fix — carried to next session's queue. **DON'T-REVERT (session 35):** `services` in `DEFAULT_LANDING` + `getLandingContent` reuses `mapFeatureCards(featureCards,'services')`, no new Strapi collection; Landing.jsx `#services` section sits between the persona section and Features, description-only cards, single closer CTA to `/register`; header nav/hamburger breakpoint is `lg` (1024px) not `md`; search bar reveal breakpoint is `xl` (1280px) not `lg`; nav/button group `shrink-0` + search wrapper `min-w-0`; `<PublicTour />` must be mounted in EVERY early-return branch of a multi-step component (Register Step 0 was the gap); `PageTourButton.jsx` is icon-only everywhere now — don't reintroduce the "Take a tour" text label. Backend HEAD `6ac27db` → `ff3e7d4`; frontend HEAD `e470559` → `a7efbe9`. DOCUMENTATION.md v5.50 → v5.51. Prior addendum retained below for continuity —**Session 34 (bug-fix batch, no new schema):** Five independent fixes. **(1) Challenge invite/application count accuracy** (BE `0cc692d`, `8504095`): `getChallenge`'s `invites_total/pending` only counted `challenge_invites`, undercounting against the "Invited (N)" modal which already UNIONed in `pending_email_invites` (4 vs 8 shown) — fixed to match. Separately, accepting a challenge invite auto-creates an empty `challenge_applications` stub row (so the invitee appears in the review queue before filling in pitch/RFI/docs) — this stub was being counted as a real application on every surface reading `challenge_applications`: corporate dashboard `listChallenges`/`exportChallengePdf` (fixed first), then extended to public marketplace list/detail, global search (FTS/ILIKE/structured/semantic branches) in `publicController.js`, and the applicant-facing browse/detail endpoints in `challengeApplyController.js`. **(2) Deal-request collaborator access** (BE `6ac27db`, FE `e470559`): accepted `entity_collaborators` on an `investor_deal_requests` row (editor/reviewer/viewer) could not view or list the deal at all — `listDealRequests`/`getDealRequest` in `investorController.js` were hard-scoped to `dr.investor_id=$1` (owner only). Added an `EXISTS` branch matching accepted collaborators, plus an `is_owner`/`is_manager` flag surfaced through `opportunityController.js`'s unified Marketplace feed so the frontend routes owner/collaborator viewers straight to the Manage Deal page (`InvestorDealRequests.jsx`, new `?open=<id>` deep-link param) instead of the applicant-only Apply modal. **(3) Portfolio edit Save silently blocked** (FE `591edae`): `InvestorPortfolio.jsx`'s edit-Save rejected saves whenever a legacy row already had a negative value in a numeric field the UI doesn't allow negative input for — validation ran against the merged (existing+edited) object instead of just the edited fields, so an untouched legacy negative blocked every save. **(4) Dead Apply button on Deal Sourcing marketplace cards** (FE `78ff9fc`): the unified Opportunities feed hardcoded `apply_url=NULL` for `deal_sourcing` rows and `Marketplace.jsx`'s `openItem()` had no fallback for that type, so Apply was a silent no-op. Added a standalone Apply-to-Deal modal calling the existing `POST /deal-requests/:id/apply`. **(5) Applicant-invite accept/decline UI** (BE `e4ba95c`, FE `8cf11a5`): the Phase F generic applicant-invite subsystem (investor/incubator/accelerator/lab, session 27) had a working backend `respondToInvite` but no frontend path ever reached it. Added `GET /my/applicant-invites` (CASE/COALESCE + per-persona LEFT JOIN resolving entity_title/deadline across the 4 owner tables) + `applicantInviteAPI.myInvites`/`.respond` + new `ApplicationInvites.jsx` page (mirrors `ChallengeInvites.jsx`) behind the existing notification link `/dashboard/application-invites`. **DON'T-REVERT (session 34):** stub-exclusion filters in `publicController.js`/`challengeApplyController.js` must stay applied everywhere `challenge_applications` is counted, not just the dashboard; `investorController.js`'s collaborator `EXISTS` branch checks `status='accepted'` only; `opportunityController.js`'s `is_manager` column is `deal_sourcing`-only (hardcoded `FALSE` for every other Opportunities type); `Marketplace.jsx`'s `openItem()` branches by `is_manager` BEFORE the generic `deal_sourcing` Apply-modal branch. **Deployment:** Railway backend HEAD `6ac27db` confirmed ACTIVE; Vercel frontend HEAD `e470559` confirmed deployed; backend smoke test (`GET /api/public/landing-content`) returns 200. **Functional live verification of all 5 fixes above has NOT been run this session — deploy-confirmed only, not yet user/testing-team verified.** Backend HEAD `8d453cb` → `6ac27db`; frontend HEAD `c52ee52` → `e470559`. DOCUMENTATION.md v5.49 → v5.50. Prior addendum retained below for continuity —**Session 33 (Phase 123 — real recurring auto-renew, self-serve, all plans):** Replaced the soft "reminder-email nudge to manually re-checkout" renewal model with genuine recurring auto-charge via Razorpay's Subscriptions API (Plans/Customers/Subscriptions), available as a self-serve opt-in checkbox on every plan tier (not enterprise/admin-gated). Built alongside, but explicitly WITHOUT touching, Dentsu's already-expired Pro-plan account (feature-only ship, per user directive). Schema (`startup.js#runMigrations`, idempotent): `user_subscriptions.auto_renew BOOLEAN DEFAULT false`; `users.razorpay_customer_id VARCHAR(200)`; `subscription_plans.razorpay_plan_ids JSONB DEFAULT '{}'` (lazily-populated cache keyed `"monthly_INR"`/`"yearly_INR"`/`"monthly_USD"`/`"yearly_USD"`); partial index `idx_user_sub_rzp_sub` on `user_subscriptions(razorpay_subscription_id) WHERE razorpay_subscription_id IS NOT NULL`. Backend `subscriptionController.js`: new `getOrCreateRazorpayPlan`/`getOrCreateRazorpayCustomer` lazy-cache helpers; new `recordSubscriptionCharge` — the shared idempotent GST/invoice-PDF/email pipeline factored out of the existing one-off `verifyPayment`, transaction-wrapped with a race-safe `payment_history.razorpay_payment_id` dupe re-check so Razorpay's webhook retries can't double-invoice; new `POST /subscription/create-recurring-order` (same billing-address/GST gate as `createOrder`, but resolves a Razorpay Plan + Customer and calls `rzp.subscriptions.create` with `customer_notify:1` — delegating the RBI-mandated pre-debit notice to Razorpay — and `total_count` 120 monthly / 25 yearly cycles as the practical proxy for "no fixed commitment", since Razorpay subscriptions cannot be truly infinite); new `POST /subscription/verify-recurring-payment` (HMAC-verifies `razorpay_payment_id|razorpay_subscription_id`, UPSERTs `user_subscriptions` with `auto_renew=true` — deliberately does NOT write `payment_history` itself, leaving that to the `subscription.charged` webhook as the single source of truth for both the first and all subsequent charges); new `POST /subscription/toggle-auto-renew` (`enabled:false` → `rzp.subscriptions.cancel(id,{cancel_at_cycle_end:1})`, access retained through period end, no un-cancel API so re-enabling means re-subscribing); existing `cancelSubscription` extended to also immediately cancel the live Razorpay subscription (`cancel_at_cycle_end:0`) when one exists, preventing orphaned auto-charges after a local downgrade; existing `webhook` handler extended with `subscription.charged` (the actual zero-user-action renewal — records the charge, extends `current_period_end`), `subscription.cancelled` (terminal — reverts `users.current_plan` to free), and `subscription.halted` (Razorpay dunning exhausted — sets `status='past_due'`, sends new `subscriptionAutoRenewFailedEmail`). `renewalReminderService.js`: 4 of 5 reminder queries (`fireRenewal30/7/3`, `fireExpired`) gained an `auto_renew = false OR auto_renew IS NULL` guard so recurring subscribers stop getting pointless "please renew manually" nudges (`fireCancellationEnding` left unchanged, already scoped to `status='cancelled'`). Frontend `Settings.jsx` Billing tab: new "Enable auto-renew" checkbox in the plan-comparison grid (default unchecked — opt-in, existing customers keep today's manual-renewal behavior unless they explicitly opt in), `runUpgrade` branches on it to call the new recurring endpoints and pass `subscription_id` (not `order_id`) to the Razorpay Checkout modal, and a new "Auto-renew is ON · next charge on {date}" management block with a "Turn off auto-renew" button when `myPlan.subscription.auto_renew===true`; existing one-off checkout path completely unchanged for customers who don't opt in. Migration applied live via `railway ssh` → `migrate-bootstrap.js`; all 3 new columns + 1 new partial index confirmed present via direct `information_schema`/`pg_indexes` query. Production spot-check post-deploy confirmed the plan-comparison grid, Monthly/Annual toggle, and new auto-renew checkbox all render correctly with zero console errors on a live Growth-tier startup-persona account. **Deferred to next session (not yet run):** full Razorpay TEST-mode paid checkout end-to-end (triggering the actual `create-recurring-order` → Checkout-modal → `verify-recurring-payment` chain); simulated `subscription.charged`/`cancelled`/`halted` webhook payloads confirming `payment_history` idempotency, `current_period_end` extension, and the three status transitions; confirming "Turn off auto-renew" shows "Cancels at cycle end" in the Razorpay dashboard (not immediate cancellation); regression-check of the existing one-off checkout path post-deploy. **DON'T-REVERT (session 33):** `recordSubscriptionCharge`'s idempotent dupe-guard (must stay the single invoice-writing path for both one-off and recurring charges); `verify-recurring-payment` NOT writing `payment_history` (webhook is the sole source of truth, avoiding double-invoice races); `toggleAutoRenew` using `cancel_at_cycle_end:1` (deferred) vs. `cancelSubscription`'s `cancel_at_cycle_end:0` (immediate) — these are deliberately different semantics for two different user actions, not inconsistent code; the `renewalReminderService.js` auto_renew guard on exactly 4 of 5 queries (not `fireCancellationEnding`). **Frontend HEAD `23a9d5d` → `4aadcb1`; backend HEAD `3cb03b6` → `8d453cb`.** DOCUMENTATION.md v5.48 → v5.49. Prior addendum retained below for continuity —**Session 31 (Part A — dedup root-cause + org-merge data-loss fix; Part B — Phase 121 Knowledge Hub):** Two independent fixes plus a new feature, all backend-heavy. **(A) Startup dedup root cause + org-merge silent data-loss (BE `add433e`→`8cf1b55`):** the Phase 120 flag "`domain_name` index shipped NON-UNIQUE" traced to inconsistent domain derivation between `import-csv-bulk.js` Phase 4b and `crawlController.approveStartup` — different normalization let visually-identical hosts slip past `ON CONFLICT`, accumulating ~1,302 duplicate domain groups in prod. Fixed by extracting a single shared validator, NEW `src/utils/domainHost.js` (`isValidHost`/`normalizeHost` — rejects junk/IPs/emails/paths, accepts punycode `xn--`), and routing both call sites through it. Ran a live cleanup pass collapsing the ~1,302 duplicate groups, then promoted `startup_profiles.domain_name`'s partial index from non-unique to **UNIQUE partial** (`WHERE domain_name IS NOT NULL AND domain_name <> ''`) — resolving the Session-30 flag. Re-import of an already-known domain is now a genuine no-op via a `NOT EXISTS` guard plus `ON CONFLICT(domain_name) DO NOTHING`; `approveStartup`'s 23505 race path now returns the existing row instead of a 500. Dropped the now-redundant `idx_sp_domain` index (`2ee0d18`) since the new UNIQUE partial supersedes it. While investigating dedup, found a separate **HIGH SEVERITY** bug in `claimController.js#executeOrgMerge` (`8cf1b55`): the reassignment of `org_members`/`organization_personas` rows during an org merge used `UPDATE ... ON CONFLICT DO NOTHING`, which is invalid SQL (`ON CONFLICT` is INSERT-only) — the syntax error was silently swallowed by a per-table `SAVEPOINT` catch block, meaning **every org merge since session 19 silently failed to reassign membership rows**, which were then lost when the placeholder org's `ON DELETE CASCADE` fired. Fixed with a `NOT EXISTS`-guarded `UPDATE` plus a prune-on-collision `DELETE` for rows that would violate `org_members`'s partial unique index, respecting the existing `activeOnly` semantics. Verified via a live rollback-wrapped functional test, 4/4 cases passing. This fix is **forward-only** — it does not repair merges already lost between sessions 19-31; there is no audit trail to reconstruct which rows were dropped. **(B) Phase 121 — Knowledge Hub (BE `0b15e0d`, FE `9457b77`):** built out the three missing pieces of the Knowledge Hub reported broken by the user ("knowledge button not working, can't upload report, can't approve any user who wants to upload"): a working admin "Add Article" flow, file-attachment support, and a self-service contributor-approval workflow, then extended it per user follow-up to also publish reports to the public `openi.ai/reports` page, make them semantically searchable, and add a fifth content type ("Case Studies"), plus give admins moderation authority over any article. Schema: `knowledge_articles` gained `file_url`/`file_name` (Cloudinary attachment via the existing generic `POST /upload?folder=knowledge_articles` — no new preset needed), `is_public BOOLEAN DEFAULT false` (gates public-reports + semantic-search visibility separately from the existing internal `is_published`), and `embedding VECTOR(1536)` + HNSW cosine index (reusing the `pgvector` infrastructure already powering `startup_profiles`/`challenges`/etc.). `users` gained `is_knowledge_contributor BOOLEAN DEFAULT false`, added to `USER_BASE_FIELDS` in `middleware/auth.js` so a freshly-approved contributor's flag is live on their very next API call with no re-login required. New `knowledge_contributor_requests` table (status pending/approved/rejected, partial UNIQUE index enforcing one active pending request per user). `knowledgeController.js`: `create`/`update` now gate on `role==='admin' || role==='evaluator' || is_knowledge_contributor` (replacing the old route-level admin-only gate); `update` additionally requires admin-OR-author ownership (fetches `author_id` first, 404 if missing, 403 if neither); new admin-only `deleteArticle` hard-delete; new `requestContributor`/`myContributorStatus`/`listContributorRequestsAdmin`/`approveContributorAdmin`/`rejectContributorAdmin` (approve flips both the request row and `users.is_knowledge_contributor` in one transaction); `list()` now only forces `is_published=true` for non-admin callers, so admins can see (and later restore or hard-delete) taken-down articles, with an optional `?status=` filter. `embeddingService.js`'s `idCol` ternary (previously hardcoded `table==='challenges' ? 'id' : 'user_id'`) generalized to include `knowledge_articles`, plus a new `buildEntityText` branch (title+content+category+tags); embeddings fire-and-forget refreshed on create/update regardless of `is_public` (the leak-prevention gate lives entirely in the read-side queries). `publicController.js#listPublicReports` now merges `is_public=true AND is_published=true` KB rows into the existing Strapi-sourced `/public/reports` response in the same shape (`id:'kb-'+id`, `cover_url:file_url`, `technologies:tags`), so DB-authored reports render on `openi.ai/reports` alongside CMS ones with zero id-collision risk. `publicController.js#semanticSearch` gained a new `type==='knowledge'` branch under the same `is_public`/`is_published`/`embedding IS NOT NULL` gate. Frontend: `Knowledge.jsx` gained a working Add-Article modal (reused as `AddArticleModal.jsx` from both the dashboard page and the new admin console module), a "Request contributor access" CTA with pending/approved/rejected state, admin-only publish/unpublish + hard-delete (with confirm) controls visible on every article regardless of author, an "Unpublished" badge for admins, a download/view link when `file_url` is present, and a new "Case Studies" filter tab/icon/form option. NEW `AdminKnowledge.jsx` (mirrors the existing `AdminClaims.jsx` shell: status tabs, list + detail panel, approve/reject with admin note) wired into `AdminConsole.jsx` as a new tile and `App.jsx` as a new admin-gated route. `GlobalSearch.jsx` gained a `knowledge` tab + `KnowledgeCard` in its semantic-search fan-out. Migration ran live in prod; all new columns/tables confirmed present (`file_url`/`file_name`/`is_public`/`embedding`/`is_knowledge_contributor`/`knowledge_contributor_requests`). A rollback-wrapped functional suite covering approve/reject, duplicate-pending-request 409, admin cross-author moderation, non-admin ownership 403, and role-based list visibility passed in full, then a live browser end-to-end pass confirmed Add Article, the Case Study filter, and the new Admin Console tile all work in production; the test article created during E2E verification was cleaned up afterward. **DON'T-REVERT (session 31):** the shared `domainHost.js` validator (both dedup call sites MUST route through it, not re-implement inline normalization); the `domain_name` UNIQUE partial index (do not revert to non-unique, and do not re-add the dropped `idx_sp_domain`); `executeOrgMerge`'s NOT-EXISTS-guarded UPDATE + prune-on-collision DELETE (do not restore the invalid `ON CONFLICT` form); the `knowledgeController.js` inline admin-OR-author ownership gate on `update()` (this is what lets admin moderation apply to any article while non-admin contributors stay scoped to their own); `list()`'s admin-only unpublished visibility; `embeddingService.js`'s generalized `idCol` ternary. **Frontend HEAD `b985cfd` → `9457b77`; backend HEAD `4b756d1` → `0b15e0d`.** DOCUMENTATION.md v5.47 → v5.48. **Addendum — Session 32 (2 Jul 2026, backend HEAD `0b15e0d` → `3cb03b6`, frontend HEAD unchanged at `23a9d5d`, NO version bump — stays v5.48):** Phase 122 — root-caused a recurring integrity-check alert (1 orphan `user_notifications` row) and closed the underlying gap structurally: 5 new DB-level `AFTER DELETE FOR EACH ROW` triggers (one per parent table — `challenges`/`events`/`watchlists`/`meetings`/`ipr_records`) now auto-prune dangling `user_notifications.related_id` rows regardless of which code path performs the delete, backstopping the existing Phase 99e/24-May inline `pruneForEntity` hooks with a structural guarantee. Made the existing Phase 113-Tier-B4 `integrityCheckService.js` Category B check self-healing (deletes any orphan found AND still alerts, since a post-Phase-122 orphan now signals trigger breakage rather than routine drift). Migration ran live in prod (`railway ssh` → `migrate-bootstrap.js`, delayed ~24.5 min behind a concurrent `pg_dump` backup holding table locks — expected Postgres lock-queueing behavior, not an error); all 5 triggers confirmed present via a direct `pg_trigger` catalog query. Bug-fix/hardening to existing infrastructure, no new API surface, no frontend change — **no version bump.** Backend HEAD `0b15e0d` → `3cb03b6`.

---

### Session 30 (30 Jun 2026 — Phase 120 in-app "Add Startup" scout-add + corporate-persona parity; backend HEAD `2dac8e9` → `4b756d1` / frontend HEAD `9e4af2c` → `b985cfd`, version bump v5.46 → v5.47) — **Session 30 (Phase 120 — in-app "Add Startup" scout-add):** A new in-app modal lets OpenI staff (admin/evaluator) + the 8 innovation-seeker personas (corporate / government / investor / mentor / lab / incubator / accelerator / service_provider) submit a scouted startup that isn't in the Hub yet — creating the same claimable imported `startup_profiles` stub the crawler/CSV path produces (so the founder can later claim and complete the profile). Provider personas (startup / student / academia) are excluded. Button label "Add Startup"; modal title "Submit a startup to the database"; live immediately (no approval queue — relies on the dedup guard); minimal form (Company name required, Website required, Sector optional, one-line Description optional). **Backend `4b756d1`** — NEW `startupAddController.js#addStartup`: a single-`client` transaction (`BEGIN` → insert `users` (role `startup`, `is_imported=true`, `is_active=false`, post-insert email `added_${id}@imported.openi.ai`) → `startup_profiles` (`import_metadata.source='scout_add'`, derived `domain_name`) → `directory_profiles` `ON CONFLICT (user_id) DO NOTHING` → `COMMIT`), with a dedup pre-check mirroring `import-csv.js` order (derived `domain_name` then normalized `company_name`) returning **409** in the existing recourse shape `{ error, recourse: 'claim'|'join', startup: { id, company_name, website } }` on a hit; 23505 race re-queries to the same 409; always `client.release()` in `finally`. Route `POST /startups/add` (NOT bare `/startups`, which is the legacy deprecated create) placed before `/startups/:id`, gated `requireRole('admin','evaluator','corporate','government','investor','mentor','lab','incubator','accelerator','service_provider')` + `auditLog('add_startup','startup')`. **Migration (Phase 120, in `startup.js#runMigrations`):** a **NON-UNIQUE** partial index on `startup_profiles.domain_name` (UNIQUE was the intent, but prod holds 1,322 duplicate domain groups / 2,653 legacy crawled rows, so a UNIQUE index would fail — the in-controller application-layer dedup remains the authoritative guard; revisit UNIQUE after a domain dedup pass). **Frontend `b985cfd`** — NEW `src/components/AddStartupModal.jsx` (props `{open, onClose, onAdded, initialName}`; seeds company_name from `initialName` on open; light URL sanity check `/\.[a-z]{2,}/i`; on 409 renders an inline "already in the database — it can be claimed" panel from `err.recourse`/`err.startup`; calls `startupAddAPI.add(...)`), `src/services/api.js` adds `startupAddAPI = { add }` (attaches `data.startup` to the thrown error so the modal can render the dedup recourse). Button surfaced on BOTH Find-Startups surfaces, each gated by `STARTUP_ADDER_ROLES = ['admin','evaluator','corporate','government','investor','mentor','lab','incubator','accelerator','service_provider']`: investor + all-other-seeker personas via `StartupDiscovery.jsx` (`canAdd` header button + empty-state CTA + always-visible footer CTA + modal), and the corporate persona via its route-overridden `CorporateStartupSearch.jsx` (`personas.js FIND_STARTUPS_ROUTE.corporate = '/dashboard/corporate/search'`; gold `G='#D0A848'`; header button + empty-state CTA + footer CTA + modal `onAdded={() => { setPage(1); loadStartups(); }} initialName={filters.search}`). **DON'T-REVERT (session 30):** `startupAddController.js` txn stub-insert shape (verbatim from the crawler path) + dedup pre-check order (domain then normalized name) + 409 recourse `{error, recourse, startup}` + 23505 re-query; route `POST /startups/add` before `:id` with the 10-role `requireRole`; the Phase 120 NON-UNIQUE `domain_name` partial index (do NOT switch to UNIQUE without a prior dedup pass); `STARTUP_ADDER_ROLES` (10 roles, provider personas excluded); the TWO-surface routing (corporate → `CorporateStartupSearch.jsx`, all others → `StartupDiscovery.jsx`) with the footer CTA on BOTH. **Banked lesson (high-leverage):** a `git push` can silently NOT trigger a Vercel build (missed GitHub→Vercel webhook) — the corporate footer-CTA "not there" report was prod stuck at `3516fe8` (header-only), NOT a code bug; fixed via empty-commit rebuild `b985cfd` (Vercel `dpl_BsW4bKNVZBnQC2g7Ebv3QhYfpS3p` READY). **RULE: before blaming code for a "not deployed" symptom, confirm via Vercel `list_deployments` that a deployment EXISTS for the HEAD sha.** **⚠️ Verify at next session:** the Phase 120 `domain_name` index was applied in prod (`railway ssh` → `\d startup_profiles`). Add Startup USER-VERIFIED live ("yes it's there"); dedup-409 + persona-gate verification delegated to the testing team. Backend HEAD `2dac8e9` → `4b756d1`; frontend HEAD `9e4af2c` → `b985cfd`. DOCUMENTATION.md v5.46 → v5.47.

---

### Session 29 (30 Jun 2026 — P2/P3 backlog clearance + IPR end-to-end fix + mobile overflow fixes; frontend HEAD `abbfbf7` → `9e4af2c` / backend HEAD `023d8b7` → `2dac8e9`, version bump v5.45 → v5.46) — **Session 29:** Cleared the queued P2/P3 items, then fixed IPR end-to-end and two mobile overflow bugs. (1) **ESLint → 0 warnings** (`c7e6aed`): moved misplaced `eslint-disable-next-line` comments to their own line above the `useEffect` in `FindMentees.jsx` + `BrowseLabFacilities.jsx`, plus Phase-D dead-code removal in `Marketplace.jsx`. (2) **iOS auto-zoom fix** (`69d22cb`): bumped the shared `const inp` form-control style `fontSize: 13 → 16` across 7 byte-identical dashboard clones (Accelerator/Incubator/ProgramPartner family) so focused inputs no longer trigger Safari zoom. (3) **STRAPI env consolidation** (BE `f7b272d`): `costWatchdogService.js#fetchStrapi` now reads canonical `STRAPI_URL` instead of legacy `STRAPI_BASE_URL` (both set byte-identical in Railway prod, so zero-risk; `STRAPI_BASE_URL` is now an unused Railway key). (4) **Mobile Ship 2** (`870b29e`): DL2 — "Tour this page" topbar pill `hidden sm:inline-flex` to stop 375px overflow; DL3 — `md:hidden` page-title anchor derived from `NAV` longest-prefix match; MP3 — Marketplace page wrappers `padding: 24 → '24px 16px'`. (MP1/MP2 already resolved by the Phase-D `auto-fit minmax` filter grid; USD_INR_RATE review deferred to Aug 2026.) (5) **IPR end-to-end fix** (BE `2dac8e9` Phase 119b + FE `37aa361`): `ipr_records.startup_id` FK was pointed at the legacy `startups` CRM-seed table (ids 1–725) instead of the real `startup_profiles` catalogue, so portfolio JOINs returned no startup name and the page looked empty. Repointed the FK to `startup_profiles` and switched all 4 `iprController.js` JOIN sites to `LEFT JOIN startup_profiles s ON r.startup_id=s.id` + `s.company_name AS startup_name` (note: `startup_profiles` column is `company_name`, NOT `name`); migration RAN + 5 IPR test rows seeded live. Seeding populated the table, which exposed a frontend crash: both normalizers in `IPRDatabase.jsx` stored `jurisdiction` as the raw DB string (e.g. `'India'`) while the render path calls `record.jurisdiction.map(...)` → `.map is not a function`. Added a `toJurisdictionArray(value)` helper (array→as-is, comma/semicolon string→split, single string→`[string]`, null→`['IN']` fallback) used by both normalizers, and coerced `inventors` to `Array.isArray(...) ? ... : []`. Verified working in prod (Corporate + Admin personas render the IPR Database with 5 records, jurisdictions like India / United States rendering as chips, no error boundary). (6) **Pipeline Health mobile overflow** (FE `9e4af2c`): the Startup Crawling page's Pipeline Health tab required right-to-left horizontal scrolling on narrow screens — `PipelineHealth.jsx` root added hardcoded `px-6` on top of the parent `StartupCrawling.jsx` content wrapper's `p-4 sm:p-6`, double-padding and forcing the wide status strip / card grid to overflow. Dropped the root horizontal padding (parent already provides it) + added `min-w-0`, made the top status strip padding/gap responsive (`p-4 sm:p-5`, `gap-3 sm:gap-4`), changed the controls cluster `ml-auto` → `sm:ml-auto ... flex-wrap` so it wraps below `sm`, and `break-words` on the long watchdog footer string. **Frontend HEAD `abbfbf7` → `9e4af2c`; backend HEAD `023d8b7` → `2dac8e9`.** DOCUMENTATION.md v5.45 → v5.46. **DON'T-REVERT (session 29):** `iprController.js` 4× `LEFT JOIN startup_profiles s ON r.startup_id=s.id` + `s.company_name AS startup_name` + `ILIKE s.company_name` search; `IPRDatabase.jsx` `toJurisdictionArray` helper + both normalizers' `jurisdiction: toJurisdictionArray(r.jurisdiction ?? r.jurisdictions)` + `inventors: Array.isArray(...) ? ... : []`; `PipelineHealth.jsx` root `py-5 space-y-5 min-w-0` (no `px-6`) + responsive top strip + `sm:ml-auto flex-wrap` controls + `break-words` footer.

---

### Session 28 (29 Jun 2026 — every-page react-joyride product tours; frontend HEAD `d6bdd02` → `cada73a` / backend HEAD `023d8b7` UNCHANGED, version bump v5.44 → v5.45 — **frontend-only, no backend, no schema, no migration**) — **Session 28:** Joyride product tours now cover EVERY page of the platform via two complementary architectures. Dashboard pages keep the existing role+page `TourWrapper.jsx` (its v3 machinery is untouched: `initialStepIndex` + `key={runKey}` + `onEvent`). All non-dashboard pages — Landing, the 4 auth pages (Register / ForgotPassword / ResetPassword / VerifyEmail), the public marketplace/reports/FAQ/search pages (via `PublicLayout`), and the 7 shared-link pages — get a new self-contained `PublicTour.jsx`: no role, page-mode only, auto-start ONCE per page per browser (localStorage `openi_page_tours`, keyed by pathname), and replayable on demand via the `openi-page-tour` window event. `config/tours.js` exposes `TOURS` (role-keyed) + `PAGE_TOURS` (route-keyed) + `resolvePageTour(pathname)` (exact-match then `matchPath`, so `:token` share routes resolve). `PublicLayout` mounts `<PublicTour/>` exactly once and shows a HelpCircle "Tour this page" header button (desktop topbar + mobile drawer) only when `resolvePageTour(pathname)` is truthy. Standalone pages that don't use `PublicLayout` get a new inline `PageTourButton.jsx` rendered next to their heading (renders null when the route has no page tour). Anchor convention: each page's `<h1>` carries a `#tour-page-<slug>` id. Commits: `fcbb186` (every-page tours + `PublicTour.jsx` + `PublicLayout` mount/button + page-seen localStorage store in `tourService.js`), `0657dad` (matchPath resolve in `resolvePageTour`), `cada73a` (P4 follow-up: NEW `PageTourButton.jsx` for the standalone pages + de-dupe the duplicate `PublicTour` mount/import on the 7 shared-link pages, which already render inside `PublicLayout` and so were double-mounting Joyride). Build clean; pre-existing unused-import warnings (Tag/ExternalLink in PublicMarketplace/PublicReports) left as-is. **Frontend HEAD `d6bdd02` → `cada73a`; backend HEAD `023d8b7` UNCHANGED.** DOCUMENTATION.md v5.44 → v5.45.

> **Correction of record (26 Jun 2026, session 27 — Cross-Ecosystem Phase F: generic applicant-invite subsystem; v5.43 → v5.44 for the new `/applicant-invites` API surface) — Session 27 (backend HEAD `f360169` → `9ae6f54` / frontend HEAD `2b8eb3c` → `d6bdd02`, version bump v5.43 → v5.44):** **Cross-Ecosystem Phase F — one generic "invite applicants" subsystem shared across 4 call-for-applications personas (investor deal requests / incubator programs / accelerator batches / lab announcements).** Owners (creator OR same-org member OR accepted editor-collaborator OR admin) can invite applicants by existing user-id OR by email (magic-link for unregistered users), mirroring the challenge-invite model but parameterized over persona. **Backend `9ae6f54` (4 files, +582/-1; migration RAN + READ-BACK VERIFIED live).** NEW `applicantInviteController.js` — an `OWNER_COLUMN` registry maps each persona to its owner table + column (`investor`→`investor_deal_requests.investor_id`, `incubator`→`incubator_programs.incubator_id`, `accelerator`→`accelerator_batches.accelerator_id`, `lab`→`lab_announcements.lab_id`) and a `TITLE_COL` map (`{investor:'title', incubator:'name', accelerator:'name', lab:'title'}`); the in-controller `isApplicantInviter` gate resolves owner OR same-org OR accepted editor-collaborator OR admin. Handlers `createInvites` (body `userIds` camelCase + `emails` + `message`), `listInvites` (a UNION ALL of `entity_application_invites` source `'registered'` + `pending_email_invites WHERE entity_type='application_invite' AND role=$persona AND status='pending'` source `'pending_email'`), `remindInvitee`, `respondToInvite`, `revokeInvite`. `pendingInviteService.js` gains an `application_invite` type + `materializeApplicationInvite(client, persona, entityId, user)` (converts a pending email invite to a registered row on signup-verify). 5 routes in `routes/index.js`, collection-before-`:id`, all behind `authMiddleware`: `POST /applicant-invites`, `GET /applicant-invites`, `POST /applicant-invites/:id/remind`, `POST /applicant-invites/:id/respond`, `DELETE /applicant-invites/:id`. **Migration (in `startup.js#runMigrations`, RAN + read-back verified in prod):** NEW `entity_application_invites` + `lab_announcement_applications` tables + 3 indexes. **Frontend `d6bdd02` (6 files, +341).** NEW `src/components/ApplicantInvitePanel.jsx` — a shared collapsible panel (props `{entityType, entityId, entityLabel, user}`): user typeahead (debounced via `meetingAPI.searchUsers`) + selected-user chips + email-chip input + message + Send, plus an existing-invites list with source-aware revoke + remind (pending_email only); `canManage = Number(inv.invited_by_user_id)===Number(user?.id) || user?.role==='admin'`; `key=`${inv.source}-${inv.id}``; returns null without an entityId. `src/services/api.js` adds `applicantInviteAPI = { create, list, revoke, remind }`. Panel mounted in all 4 owner dashboards — `InvestorDealRequests.jsx` (in the `selected` detail view, before ReviewPanel), `IncubatorProgramDetail.jsx`, `AcceleratorBatchDetail.jsx`, and `LabAnnouncements.jsx` (per-card, after the action row). **The `/applicant-invites` API surface is the v5.44 bump driver** (new generic endpoints, mirroring the session-24 `GET /api/opportunities` and session-19 org claim/merge precedents). **DON'T-REVERT:** the `applicantInviteController.js` `OWNER_COLUMN`/`TITLE_COL` registries + `isApplicantInviter` 4-way gate + UNION-ALL list (source discriminator `'registered'`/`'pending_email'`); the body key `userIds` (camelCase); `pendingInviteService.js` `application_invite` type + `materializeApplicationInvite` positional signature; the 5 `/applicant-invites` routes collection-before-`:id`; the `startup.js#runMigrations` Phase F block (`entity_application_invites` + `lab_announcement_applications` + 3 indexes); the FE `ApplicantInvitePanel.jsx` + `applicantInviteAPI` + the 4 dashboard mounts. **Banked invariant — applicant-invite gate:** owner (per-persona `OWNER_COLUMN`) OR same-org member OR accepted editor-collaborator (`entity_collaborators role='editor' AND status='accepted'`) OR admin. **Status enums:** registered rows `{invited, accepted, declined, revoked}`; pending_email rows `{pending}`. **Verification delegated to the testing team** (each owner persona invites a registered user → appears as `invited`; invites an unregistered email → pending magic-link; revoke + remind work; a non-owner accepted-editor can also invite; an unrelated user cannot). Both repos clean + synced `0 0`, Vercel + Railway auto-deployed (Railway deploy `fb1ce4f3` Online; `POST`/`GET /api/applicant-invites` live-probed → 401 unauth, route registered + auth-gated). Backend HEAD `f360169` → `9ae6f54`; frontend HEAD `2b8eb3c` → `d6bdd02`. DOCUMENTATION.md v5.43 → v5.44. **Prior (session 26 — invite-gate editor fix; folds in at v5.43, NO bump) — Session 26 addendum (backend HEAD `61f3399` → `f360169`):** invite-gate fix for a Dentsu client report — `challengeVisibility.js#isOrgCoInviter` now also returns true for an accepted `editor` collaborator (`entity_collaborators` `role='editor' AND status='accepted'`), in addition to the creator and same-org members. This single helper gates all 4 challenge invite ops (create/list/revoke/remind), so edit-access users can invite startups (including non-registered startups by email via the existing pending magic-link path). The 4 matching 403 messages updated to mention "users with edit access". Bug fix to an existing gate — no new API surface, **no version bump (stays v5.43)**, frontend HEAD `b77a4e0` → `2b8eb3c` was the docs-only v5.43 bump commit. **Prior (session 25, Phase E):** **Cross-Ecosystem Visibility Phase E: "Find Mentees" / `seeking_mentor` opt-in feed (backend HEAD `b0ab2ca` → `61f3399` / frontend HEAD `2946722` → `b77a4e0`, version bump v5.42 → v5.43 for the new `GET /mentees/public(/:id)` API surface + `seeking_mentor` schema migration).** FINAL (5th) installment of the cross-ecosystem visibility series (A = investor deal sourcing, B = cohort/program applications, C = lab facility announcements, D = unified Opportunities feed, E = reverse-direction mentee discovery) — Phases A–E now ALL shipped. Phase E is a **REVERSE-direction** feed: where A–D let SEEKERS discover open calls, E lets **MENTORS discover provider personas** (startup / student / academia) who opt in to being mentored via a new `seeking_mentor` boolean. **Backend `61f3399`** — NEW `menteeController.js` (`listMentees` + `getMentee`) = a 3-branch `UNION ALL` across `startup_profiles` + `student_profiles` + `academia_profiles`, each normalized to `{persona, user_id, name, avatar, org_name, headline, summary, sector, city, state, areas, linkedin_url, created_at}`; each branch gated `WHERE p.seeking_mentor = true${w}`; student/academia branches `SELECT NULL::varchar AS sector`; rows key on `user_id` (NO `status`). `listMentees` gate `seeking_mentor=true AND user_id<>$1` (self-exclude) + optional persona / sector (`$N = ANY(m.areas)`) / search (`name|org_name ILIKE`); `getMentee` gate `seeking_mentor=true AND user_id=$1 LIMIT 1`. Routes `GET /mentees/public` + `GET /mentees/public/:id` behind `authMiddleware`, NO role guard, public-before-`:id` order. **Schema migration (RAN live + READ-BACK VERIFIED in prod):** Phase E block mirrored into `startup.js#runMigrations` — 3× `ADD COLUMN IF NOT EXISTS seeking_mentor BOOLEAN DEFAULT false` + 3× partial `idx_<persona>_seeking_mentor ... WHERE seeking_mentor = true` on startup/student/academia (3 cols + 3 partial indexes physically confirmed present). **Frontend `b77a4e0`** — NEW `FindMentees.jsx` single-component base-path + `:id` useParams (detail keyed `detail.user_id !== selectedId`) + LinkedIn Connect CTA (no status gate); `api.js` adds `menteeAPI = { listPublic, getPublic }`; `personas.js` adds a "Find Mentees" universal Discover entry + 3× `seeking_mentor` checkbox on PROVIDER personas ONLY (startup/student/academia; corporate gets none). **The `GET /mentees/public(/:id)` API + `seeking_mentor` migration are the v5.43 bump drivers** (new API surface + schema change, mirroring the session-24 `GET /api/opportunities` and session-19 org claim/merge precedents). **DON'T-REVERT:** `menteeController.js` 3-branch `UNION ALL` + per-branch `seeking_mentor=true` gate + self-exclude `user_id<>$1`(list)/`user_id=$1`(get) + student/academia `NULL::varchar AS sector` + `user_id` row key; route order public-before-`:id`; `startup.js#runMigrations` Phase E block (after Phase C, before "All migrations complete"); `FindMentees.jsx` single-component base-path + `:id` useParams + `detail.user_id !== selectedId` key + LinkedIn CTA; `api.js menteeAPI`; `personas.js` "Find Mentees" entry + `seeking_mentor` checkbox startup/student/academia ONLY. **Non-blocking discrepancy:** standalone `migrate-phaseE-seeking-mentor.js` is absent from the BE commit — prod uses the `startup.js#runMigrations` Phase E block (committed/deployed/RAN/VERIFIED); no action needed unless a fresh env is booted.

> **Correction of record (24 Jun 2026, session 24, Phase D) — Cross-Ecosystem Visibility Phase D: unified "Opportunities" UNION feed (backend HEAD `bb2100d` → `b0ab2ca` / frontend HEAD `eb0e06a` → `2946722`, version bump v5.41 → v5.42 for the new `GET /api/opportunities` API surface — no schema, no migration).** Fourth installment of the cross-ecosystem visibility series (A = investor deal sourcing, B = cohort/program applications, C = lab facility announcements, D = unified feed): a single browsable feed merging every persona's "Calls for Applications". **Backend `b0ab2ca`** — NEW `opportunityController.listAll` = a 5-way `UNION ALL` across `challenges` + `investor_deal_requests` + `incubator_programs` + `accelerator_batches` + `lab_announcements`, each normalized to `{type, id, title, description, org_name, sectors, deadline, apply_url, created_at}`; `type` ∈ `'challenge'|'deal_sourcing'|'cohort_incubator'|'cohort_accelerator'|'lab_facility'`; ONLY `challenge` rows emit `NULL::text AS apply_url` (apply via internal `/dashboard/marketplace/:id`), the other 4 carry external `apply_url`; `ORDER BY deadline ASC NULLS LAST, created_at DESC`; params $1=sector (nullable), $2=search (%-wrapped, nullable); returns a FLAT array via `res.json(rows)`. Route `GET /api/opportunities` behind `authMiddleware`, NO role guard (any logged-in persona may browse). **Frontend `2946722`** — `Marketplace.jsx` consumes the flat array via `opportunityAPI.listAll`; a 5-entry `TYPE_BADGE` map; type-routed `openItem` (challenge → `navigate('/dashboard/marketplace/:id')`, others → `window.open(apply_url)`); provider-gated `!isSeeker` Apply CTA; `${type}-${id}` React keys; `api.js` adds `opportunityAPI = { listAll }`. **The `GET /api/opportunities` UNION feed is the v5.42 bump driver** (new public API surface, mirroring the session-19 org claim/merge, session-17 `DELETE /investor/deal-requests/:id`, and session-11 `PUT /events/:id` precedents). **DON'T-REVERT:** the 5-way `UNION ALL` shape; the only-`challenge`-`apply_url`-`NULL` discrimination; `ORDER BY deadline ASC NULLS LAST, created_at DESC`; the auth-but-no-role-guard route returning a flat array; the `Marketplace.jsx` `TYPE_BADGE`/type-routed `openItem`/`!isSeeker` gate/`${type}-${id}` keys; `opportunityAPI.listAll`. **Cosmetic-pagination caveat (open, left as-is):** the UNION returns all rows in one call, so client Previous/Next on `total>20` doesn't re-fetch server-side — functional, deferred. **Phase E (Find Mentees / mentorship-seekers feed) DEFERRED to next session per user.** **Verification delegated to the testing team** (any logged-in persona sees a merged feed across all 5 sources; challenge rows open the internal marketplace detail, the other 4 open their external apply URL; seeker personas see no Apply CTA). Both repos clean + synced `0 0`, Vercel + Railway auto-deployed. Backend HEAD `bb2100d` → `b0ab2ca`; frontend HEAD `eb0e06a` → `2946722`. DOCUMENTATION.md v5.41 → v5.42. Prior the same session (session 24, addendum) — **investor-persona bug batch: Deal Pipeline stage-update cast fix + Deal Team invite copy + Portfolio edit (numbers persist) + Add-Portfolio currency restriction (backend HEAD `fb64ff8` → `bb2100d` / frontend HEAD `cd9554f` → `eb0e06a`, NO version bump — bug/UX only, no new API surface, no migration).** Four investor-persona issues flagged by the testing team, shipped on top of the same session's lab-announcements v5.41 ship (documented below). **(A) Deal Pipeline stage update (BE `0fe76a8`, `investorController.js updateDeal`).** Clicking a kanban stage button threw `inconsistent types deduced for parameter $2`; the dynamic `UPDATE … SET col=$N` builder passed mixed JS types to one positional parameter. FIX: explicit `::type` casts on every assigned column so Postgres no longer infers conflicting types across calls. **(B) Deal Team invite (FE `7fe42fa`, `CollaboratorsPanel.jsx`).** Reported "investor cannot invite any Student/Startup/Academia to a deal" — proven a **perceived bug**, not a logic fault; the invite flow worked. Only the panel subtitle copy was clarified (no logic change). **(C) Portfolio edit (BE `bb2100d` + FE `f7a34ef`, `investorController.js updatePortfolioCompany` + `InvestorPortfolio.jsx`).** The Invested / Equity / Entry Val. fields were not editable/persisting. FIX: FE edit form uses number inputs and reconciles field names to the live schema (Status → `editForm.current_status`, Exit Valuation → `editForm.exit_value`, exit block reads `co.current_status==='exited' && co.exit_value`); BE `updatePortfolioCompany` adds negative-value + equity>100 guards, `COALESCE` partial-update writes, and the correct column names (`current_status` NOT `status`, `exit_value` NOT `exit_valuation`). **(D) Add-Portfolio currency (FE-only `eb0e06a`, `InvestorPortfolio.jsx`).** Adding a company with currency EUR/GBP threw a PostgreSQL `portfolio_companies_currency_check` violation (the CHECK at `startup.js:2387` allows only INR/USD). Per explicit user choice the **frontend currency dropdown was restricted to INR/USD** (the DB CHECK was NOT widened). **Banked schema invariant — `portfolio_companies` columns:** `id, investor_id, startup_user_id, startup_name, entry_date, entry_valuation, equity_stake, board_seat, investment_amount, notes, currency, current_status, exit_date, exit_value, exit_type` (column is `current_status` not `status`, `exit_value` not `exit_valuation`; currency CHECK = INR/USD only). **DON'T-REVERT:** the `updateDeal` `::type` casts; the `updatePortfolioCompany` negative+equity guards + COALESCE + `current_status`/`exit_value` names; the `InvestorPortfolio.jsx` number inputs + field-name reconciliation + INR/USD-only currency dropdown; the `CollaboratorsPanel.jsx` subtitle copy. **No schema, no new API surface, no migration → no further version bump (stays v5.41).** **Verification delegated to the testing team** (drag/click a Deal Pipeline stage → persists, no exception; invite a Student/Startup/Academia to a deal → lands; edit Invested/Equity/Entry Val. → persists, negative + equity>100 rejected; add a company in INR and in USD → succeeds, EUR/GBP not selectable). Both repos clean + synced `0 0`, Vercel + Railway auto-deployed. Backend HEAD `fb64ff8` → `0fe76a8` → `bb2100d`; frontend HEAD `cd9554f` → `7fe42fa` → `f7a34ef` → `eb0e06a`. DOCUMENTATION.md stays v5.41. Prior the same session (session 24) — **Cross-Ecosystem Visibility Phase C: lab facility announcements (owner CRUD + auth-gated cross-org browse) (backend HEAD `1ec7fc6` → `fb64ff8` / frontend HEAD `baf92f1` → `cd9554f`, version bump v5.40 → v5.41 for the new `/lab-announcements` API surface).** Third installment of the cross-ecosystem visibility series (A = investor deal sourcing, B = cohort/program applications, C = lab facility announcements): a lab can publish facility/equipment-access calls that any logged-in persona can browse cross-org, private-by-default. **Backend `fb64ff8` (prod-verified, migration RAN live).** NEW `labAnnouncementController.js` with 6 handlers — `listAnnouncements` (owner's own rows, `lab_id=req.user.id`), `createAnnouncement` (title required → 400; `INSERT … $1..$10 RETURNING *`), `updateAnnouncement` (COALESCE partial update, ownership-gated), `deleteAnnouncement` (ownership-gated), `listPublicAnnouncements` (cross-org browse: conditions `['a.is_public=true', "a.status<>'draft'", 'a.lab_id<>$1']` so it self-excludes the caller's own rows AND hides drafts, JOIN `users u` for `u.name AS lab_name, u.organization_name AS lab_org`), `getPublicAnnouncement` (single public row). 6 routes registered in `routes/index.js` — **public-before-`:id`** ordering, all behind `authMiddleware`. NEW `lab_announcements` table (13 cols: `id, lab_id, title, description, facilities[], sectors[], eligibility, deadline, application_url, is_public, status, created_at, updated_at`) + 3 indexes (`pkey`, `idx_lab_ann_lab`, `idx_lab_ann_public` partial `WHERE is_public=true`). **VERIFIED LIVE** post-migration (13 cols + 3 indexes present). **Frontend `cd9554f` (5 files, +428/-2).** NEW `LabAnnouncements.jsx` (lab-owner CRUD page — create/edit/delete modal, `STATUS_OPTIONS=['draft','open','closed']`, public/private toggle); NEW `BrowseLabFacilities.jsx` (auth-gated cross-org browse list + URL-driven detail — `/dashboard/browse-facilities` and `/dashboard/browse-facilities/:id` BOTH route to it, reading `useParams`; handles a bare-array response); `src/services/api.js` adds the `labAnnouncementAPI` group (`list`/`create`/`update`/`remove`/`listPublic`/`getPublic`); `App.jsx` imports both pages + wires routes `lab/announcements`, `browse-facilities`, `browse-facilities/:id`; `src/config/personas.js` adds a universal "Lab Facility Calls" entry to the shared `discoverGroup(role)` Discover block (visible to every persona; browse self-excludes own rows + hides drafts). The lab-owner nav entry `{ to:'/dashboard/lab/announcements', label:'Facility Calls', icon:'Megaphone' }` already existed. **The `/lab-announcements` API surface is the v5.41 bump driver** (new public + owner endpoints, mirroring the session-19 org claim/merge, session-17 `DELETE /investor/deal-requests/:id`, and session-11 `PUT /events/:id` precedents). **DON'T-REVERT:** the `labAnnouncementController.js` 6 handlers (esp. the `listPublicAnnouncements` `lab_id<>$1` self-exclude + `status<>'draft'` draft-hide + the users JOIN for lab name/org); the public-before-`:id` route ordering in `routes/index.js`; the `lab_announcements` 13-col table + 3 indexes; the FE `LabAnnouncements.jsx` owner CRUD + `BrowseLabFacilities.jsx` browse+`:id`-detail dual route + `labAnnouncementAPI` + the `App.jsx` routes + the `personas.js` universal "Lab Facility Calls" Discover entry. **Banked invariant — cross-ecosystem visibility shape (Phases A/B/C):** rows are `is_public BOOLEAN DEFAULT false` + `status TEXT DEFAULT 'draft'` (private by default); cross-org browse is AUTH-GATED for ANY persona (no role guard), self-excludes the caller's own rows (`owner_id <> $1`), and hides drafts (`status <> 'draft'`); detail base-path + `:id` both route to the SAME component reading `useParams`. **Verification delegated to the testing team** (lab owner creates a facility call → toggles public → it appears in another persona's "Lab Facility Calls" browse but NOT in the owner's own; a draft never appears in browse; detail link opens the right row; owner can edit/delete; non-owner cannot mutate). Both repos clean + synced `0 0`, Vercel + Railway auto-deployed. Backend HEAD `1ec7fc6` → `fb64ff8`; frontend HEAD `baf92f1` → `cd9554f`. DOCUMENTATION.md v5.40 → v5.41. Prior session (session 21, 20 Jun) — **Startup My-Profile URL-clear bug fix + session-20-tail reconciliation (backend HEAD `4f2e7e7` → `a5b1f59` / frontend HEAD `007baf2` → `601c4fa`, NO version bump — logic-only bug/UX, no new API surface, no migration).** Reported: on the Startup "My Profile" page, deleting a populated URL field (website/LinkedIn) then Save did NOT persist the clear — the old value reappeared on reload. Root cause = two cooperating "drop empties" guards: the FE `handleSave` only sent non-empty fields, and the BE `coerceUpdates` dropped `''`/null for text/url/date columns. Neither could express "the user intentionally cleared this field". **FIX (Phase 130, two-layer).** **(A) FE `601c4fa` (`MyProfile.jsx` +17/-2).** The first `handleSave` (the top-level profile save @~509 — NOT the 2nd @~895 child-table repeater) now parses the `baseline` snapshot taken at load and, for any field that was non-empty at load and is now empty, sends an explicit `null` (deliberate clear) instead of omitting it. **(B) BE `a5b1f59` (`profileController.js coerceUpdates` +11/-4).** Now honours an explicit `null` as a deliberate clear (`if (v === null) continue;` → emits `SET col = NULL`; `if (v === undefined) delete`) for ALL column types, while the numeric/date branches key their drop-noise check on `=== ''` only (no longer swallow null); whitespace-only strings still dropped. **Crawler auto-fill protection intact** — the null-clear flows only through `updateMyProfile`; enrichment writes go through `enrichController`/`deepEnrichService` `COALESCE(col,$N)` and never receive these nulls, so an enrichment pass still cannot overwrite a user-cleared field. **Banked invariant — profile-update null semantics (Phase 130):** `undefined`=field absent→don't touch; `null`=explicit clear→`SET col=NULL`; `''`/whitespace=noise→don't touch; numeric/date coercion must guard on `=== ''` (not falsy). **Session-20-tail reconcile (3 commits shipped between the s20 close and now): BE `dcffe55`** `challengeApplyController.js` `SEEKER_ROLES` 403 guard (innovation seekers cannot apply to challenges; roles `['corporate','government','investor','mentor','lab','incubator','accelerator','service_provider']`, gate uses `req.user.activeRole || req.user.role`); **FE `7b2e311`** `StudentPortfolio.jsx` Create-share-link list populate fix (`loadShares` shape-tolerant); **FE `d840295`** "Source Students" opens as a full page not a popup (`StudentDiscovery.jsx` full-page, `G='#D0A848'`). **DON'T-REVERT:** the `MyProfile.jsx` first-`handleSave` baseline-null logic; the `profileController.js coerceUpdates` explicit-null-as-clear + numeric/date `=== ''` branches; the `challengeApplyController.js` `SEEKER_ROLES` guard; the `StudentPortfolio.jsx loadShares` shape tolerance; the `StudentDiscovery.jsx` full-page conversion. **Verification delegated to the testing team** (clear a populated URL → Save → reload → stays empty; untouched field keeps value; numeric + date fields also clearable; seeker cannot apply to a challenge; student portfolio share-link list populates; Source Students is a full page). Both repos clean + synced `0 0`, Vercel + Railway auto-deployed. DOCUMENTATION.md stays v5.40 (no new API surface). Prior session (session 20, 19 Jun) — **post-session-19 batch: DeepTech assessment data-isolation fix + reports sector/technology filtering + register persona-group alignment (backend HEAD `fe4525b` → `4f2e7e7` / frontend HEAD `e330285` → `30b9caa`, NO version bump — all bug/UX/display changes, no new API surface).** Five undocumented commits + one DB-only rename, captured in the session-20 Correction-of-record block below. **(A) DeepTech data isolation (BE `4f2e7e7`, `deeptechController.js` +11/-1).** `list` + `getOne` now scope to `da.assessed_by = req.user.id` for non-admins (admins retain full visibility), mirroring the `exportPdf`/`createShare`/`listShares` owner gate; `getOne` returns **404 (not 403)** to a non-owner so existence can't be probed. The earlier "still an issue" report was proven NOT a bug — both demo rows are owned by user id 7. **(B) DB-only rename.** `deeptech_assessments` row id 1 (assessed_by=7, score 77) renamed `'Test Submit'` → `'Amber Kinetics'` via a prod `UPDATE … WHERE id=1` (no commit) so both of that account's assessments read consistently; verified live (id 1 score 77 + id 2 score 72 both now `'Amber Kinetics'`). **(C) Reports surface (BE `8042a10` reports sort `createdAt:desc`; BE `f31c0d0` source technology tags from the backend lookup table, Strapi prod-locked, `publicController.js` +38/-5; FE `50c4875` add sector+technology filtering + map BFSI/ESG/TamilNadu icons+colors `PublicReports.jsx` +83/-25; FE `9eeb22c` fix report-open link, drop the download+`_blank` combo that opened a blank tab; FE `95af76b` add the static PDF "The AI Data Quality Crisis in BFSI" `public/reports/the-ai-data-quality-in-bfsi.pdf`).** **(D) Register persona picker (FE `d4d1f8e` split the picker into provider/seeker groups + sharpen Startup vs Service Provider copy = Dentsu mis-pick fix `Register.jsx` +73/-47; FE `30b9caa` align the persona groups with the homepage Innovation Providers/Seekers framing `Register.jsx` +19/-11).** **DON'T-REVERT:** the `deeptechController.js list`+`getOne` owner gate (404 on non-owner); the `publicController.js` reports sort + technology-tag lookup; the `PublicReports.jsx` sector/technology filter + icon/colour map + report-open link fix; the `Register.jsx` provider/seeker persona grouping. **No schema, no migration** (the rename was a single-row data edit). **Verification delegated to the testing team** (non-admin only sees own DeepTech assessments + 404 on another user's id; admin sees all; reports filter by sector + technology, newest first, BFSI PDF opens in the same tab; register persona groups match the homepage Provider/Seeker split). Prior session (session 19, 18 Jun) — **Bug #2a duplicate-org guard (409 recourse) + FULL org-persona claim/merge subsystem (backend HEAD `fe4525b` / frontend HEAD `e330285`, version bump v5.39 → v5.40 for the new org claim/merge API surface — `requestClaim` now accepts `target_org_id`, and `createOrg` returns a structured 409 recourse contract).** Two coordinated ships, one schema migration RAN + verified live. **(A) Bug #2a duplicate-org guard (BE `orgController.js`).** `createOrg` now does a pre-insert duplicate check on `name_normalized OR LOWER(domain)`; on a hit it returns **409** via `buildDuplicateRecourse(matchOrg)` = `{error, recourse:'claim'|'join', org:{id,name,slug,domain}}` (recourse=`'claim'` iff the existing org's admin user is `is_imported===true && claimed_at==null`, else `'join'`). The INSERT now writes `name_normalized` (JS `normalizeOrgName` = `trim().toLowerCase().replace(/\s+/g,' ')` mirrors the SQL); the 23505 handler discriminates `organizations_name_normalized_unique` / `organizations_domain_unique` → 409 recourse, slug → 400. **(B) Org claim/merge subsystem (BE `claimController.js`).** Extends the adopt-merge machinery from startup personas to org personas: `CLAIM_ELIGIBLE_ROLES` += 6 (corporate/government/investor/lab/incubator/accelerator → now 9); `requestClaim` accepts EITHER `target_startup_user_id` OR `target_org_id` (XOR, else 400) and routes `target_org_id` to `requestOrgClaim`; `findOrgClaimCandidates(domain)` matches `organizations JOIN users ON admin_user_id WHERE is_imported AND claimed_at IS NULL` (conf 1.0/0.7/0.5); `executeOrgMerge` snapshots → adopts `ORG_MERGE_FIELDS` → reassigns 8 FK sets in SAVEPOINTs (conflictSafe tables `ON CONFLICT DO NOTHING` + DELETE) → deletes placeholder org + imported admin user → stamps the claimer's `claimed_at`; verify/approve branch on `target_org_id`; `rollbackClaim` recreates the imported admin user + org from the snapshot; `resolveOrgMergeIds` derives claimer-org + placeholder-admin. **(C) FE recourse CTA (FE `OrgAdmin.jsx` + `api.js`).** `handleCreateOrg` catch on `err.status===409 && err.recourse && err.org` renders a branching CTA card (claim → `claimAPI.request({target_org_id, evidence})`; join → `orgAPI.requestJoin({org_id})`); `api.js` line-75 generic error path now attaches `err.status/recourse/org/error` from the 409 body. **(D) Migration (BE `migrate-phase53-claims.js` step-2b).** `ALTER TABLE profile_claims ADD COLUMN IF NOT EXISTS target_org_id` (ON DELETE SET NULL — preserves the snapshot for rollback) + `idx_pc_target_org`. **RAN + VERIFIED LIVE** (`railway ssh` → standalone script): `profile_claims` now 19 cols, `target_org_id` integer = col 19, `idx_pc_target_org` ensured. **The org claim/merge API surface is the v5.40 bump driver** (`requestClaim` accepting `target_org_id` + the 409 recourse contract on `createOrg`, mirroring the session-17 `DELETE /investor/deal-requests/:id` and session-11 `PUT /events/:id` precedents). **Banked invariants:** `ORG_MERGE_FIELDS` = name/domain/industry/about/website/headquarters/size_band/founded_year/type/logo_url (`name_normalized` excluded — rederived); `ORG_REASSIGN_REFERENCES` (8) = `org_members.org_id` (conflictSafe), `users.org_id`, `organization_personas.org_id` (conflictSafe), `challenges.org_id`, `challenge_applications.applicant_org_id`, `events.org_id`, `ipr_records.org_id`, `documents.org_id`; 409 payload `{error, recourse, org:{id,name,slug,domain}}`. **DON'T-REVERT:** the `orgController.js` dup guard + `buildDuplicateRecourse` + `name_normalized` INSERT + 23505 discrimination; the `claimController.js` org-claim subsystem (9-role set, `requestOrgClaim`/`findOrgClaimCandidates`/`executeOrgMerge`/`resolveOrgMergeIds` + verify/approve/rollback org branches); `migrate-phase53-claims.js` step-2b; `OrgAdmin.jsx` recourse CTA; `api.js` line-75 409-body attach. **Open follow-up (offered, NOT requested):** mirror step-2b `target_org_id` ALTER into `startup.js#runMigrations` for fresh-DB boots (currently only in the standalone script; prod already migrated). **Verification delegated to the testing team** (create a duplicate org → 409 + claim/join CTA; org claim happy-path → admin verify/approve → executeOrgMerge reassigns 8 FK sets + deletes placeholder + stamps claimer; rollback recreates the imported admin + org; XOR guard → 400 if both/neither target id). Both repos clean + synced `0 0`, Vercel + Railway auto-deployed. Backend HEAD `cd0d53e`/`7275d21` → `fe4525b`; frontend HEAD `2eb09aa`/`c83419d` → `e330285`. DOCUMENTATION.md v5.39 → v5.40. Prior session (session 17, 17 Jun) — **Bug 6: investor deal-request invite + delete (backend HEAD `cd0d53e` / frontend HEAD `2eb09aa`, version bump v5.38 → v5.39 for the new `DELETE /investor/deal-requests/:id` API surface).** Testing-team report: an investor could neither invite an innovation provider to a deal nor delete a deal request. Two blockers fixed, no migration. **(A) Invite by email** — `collaboratorController.inviteByEmail` registered-user INSERT referenced a non-existent column (`invited_by_user_id`) and an illegal status (`'active'` is not in the `entity_collaborators` CHECK list), silently 500ing; re-aligned to the live schema: `invited_by` + status `'accepted'` + `accepted_at NOW()` + `ON CONFLICT (entity_type,entity_id,user_id) DO NOTHING`. **(B) Delete deal request** — `deleteDealRequest` was absent at every layer; added a transactional controller (BEGIN/COMMIT, ownership-gated on `investor_id`) that prunes `deal_request_applications`, `entity_collaborators`, `entity_reviews` before deleting the `investor_deal_requests` row, exported it, and registered `DELETE /investor/deal-requests/:id` behind `authMiddleware`; frontend `investorAPI.deleteDealRequest` + a Trash2 Delete button (confirm-gated) in `InvestorDealRequests.jsx`, plus `collabAPI.inviteByEmail` + `CollaboratorsPanel.submitInvite` routed through it. **The new DELETE route is the v5.39 bump driver** (new API surface, mirroring the session-11 `PUT /events/:id` precedent). **DON'T-REVERT:** the `collaboratorController.inviteByEmail` INSERT, the `investorController.deleteDealRequest` controller + route, and the FE `api.js`/`InvestorDealRequests.jsx`/`CollaboratorsPanel.jsx` wiring. **Banked schema invariant:** `entity_collaborators` uses `invited_by` (NOT `invited_by_user_id`); status CHECK = `'invited'`/`'accepted'`/`'declined'`/`'removed'` (NO `'active'`); has `accepted_at`; UNIQUE on (entity_type, entity_id, user_id); `deal_request` is a supported collab entity type. **No migration** (all tables/columns already exist). **Verification delegated to the testing team.** Both repos clean + synced `0 0`, Vercel + Railway auto-deployed. Backend HEAD `bdf49a4` → `cd0d53e`; frontend HEAD `91c0da2`/`a0b295f` → `2eb09aa`. DOCUMENTATION.md v5.38 → v5.39. Prior session (session 16, 16 Jun) — **OpenI Brand Kit standardized + applied platform-wide (UI + email assets) (backend HEAD `bdf49a4` / frontend HEAD `91c0da2`, version bump v5.37 → v5.38 for the documented platform-wide brand-posture change).** The canonical brand palette + typography were locked into a NEW `OPENI_BRAND_KIT.md` committed to BOTH repo roots (`91c0da2` / `bdf49a4`) as the single source of truth, then swept across the React UI and the backend email accents. **FE brand apply** (`91c0da2`, 105 files, +324/-246) — `tailwind.config.js` ramps retuned (`primary.500`=`#D0A848` gold, `primary.600`=`#b8923a`, `dark.900`=`#152838` deep navy, `dark.950`=`#111E21`, `accent.400`=`#6EC1E4`, `navy`=`#152838`) + `fontFamily.sans`/`display`=Lexend, plus a 102-file `src/**` inline-literal sweep replacing legacy gold `#D5AA5B`→`#D0A848` / old navy `#252147`→`#152838`. **BE email-accent retune** (`bdf49a4`, 4 files, +84/-6) — gold accent literals in `emailService.js` / `orgController.js` / `emailVerificationController.js` moved to `#D0A848`. **GST tax-invoice PDF LEFT UNTOUCHED** (legally constrained — `pdfService.js` GST field layout is colour-only-changeable; re-verify a domestic + an export invoice render after any palette change). **Cosmetic/brand only — no schema, no new API surface, no migration.** **DON'T-REVERT** the `tailwind.config.js` ramp values + the `src/**` literal sweep + the BE email-accent literals + either copy of `OPENI_BRAND_KIT.md`. **Verification delegated to the testing team** (live UI shows new gold/navy + Lexend; NO deprecated `#D5AA5B`/`#252147` survives a hard-reload; domestic + export GST invoices still render correctly). Both repos clean + synced `0 0`, Vercel + Railway auto-deployed. Backend HEAD `1dc6434` → `bdf49a4`; frontend HEAD `21d33f2` → `91c0da2`. DOCUMENTATION.md v5.37 → v5.38. Prior session (session 15, continued, 15 Jun) — **student/academia/mentor bug batch: profile-photo (logo_url) persistence (two-layer, prod migration verified live) + .docx/.doc portfolio/resume/pitch-deck uploads + Register Step-2 prefill + discovery card-click profile modal (backend HEAD `ef349e4` / frontend HEAD `21d33f2`, no version bump — bug/UX only, no new product surface).** See the session-15-continued Correction-of-record block below. Earlier the same session — **testing-team bug batch: admin delete-user FK cascade closeout (USER-CONFIRMED working) + analytics 500 fixes + profile-score cap fix + frontend safeStorage crash-guard (backend HEAD `1dc6434` / frontend HEAD `863b0c5`).** See the session-15 Correction-of-record block below. Prior version-bump baseline (session 12, 11 Jun) — **ISO 27001 pre-audit OWASP security hardening (backend HEAD `9af20d6` / frontend HEAD `095dff5`).** Pre-penetration-test remediation pass across the OWASP Top 10; 7 findings fixed, no migration, prod-probe GREEN. **A01 Broken Access Control / IDOR** — **H1** (BE) startups update/remove now ownership-gated (only `created_by` or admin may mutate, else 403); **M1** (BE) milestone/task update+delete in `corporateController` gated on `corporate_id` (collaboration-ownership self-check → 404 on mismatch); **LM1** (BE) watchlist `getOne`+`exportPdf` gated via `canWatchlistRead(pool, userId, watchlistId)` (admin-bypass, else 403). **A05 Security Misconfig** — **L1** (BE) localhost CORS origins gated to non-prod (`process.env.NODE_ENV !== 'production'`) so prod never trusts `http://localhost`; **L2** (BE) error handler no longer leaks internal `err.message` in prod (generic message client-side, full detail logged server-side); **M2** (FE `vercel.json`) global `/(.*)` response-headers block — `X-Content-Type-Options nosniff`, `X-Frame-Options DENY`, `Referrer-Policy`, `Permissions-Policy`, HSTS (2y, includeSubDomains, preload), and a Content-Security-Policy (`self` + `unsafe-inline`/`unsafe-eval` for the Vite bundle, Sentry, Google Fonts; `connect-src` limited to `api.openi.ai`/Sentry; `frame-ancestors 'none'`; `object-src 'none'`); existing `/assets` + `/index.html` Cache-Control preserved. **A02 Cryptographic Failures** — **L3** (BE) JWT pinned to `HS256` on every verify + on signing (closes algorithm-confusion) across `authController`, `auth` middleware, `emailVerification`, `mfa` controllers. Both repos `npm audit fix` → backend 0 vulns; frontend remaining advisories are build-time-only devDeps (react-snap/vite, never shipped to the browser) — accepted, not force-upgraded. **Findings + remediation report** delivered as a Google Doc (`16bqv-jzxpz2qVIK1-IZS9Xob2smKIxQFSjbqxG7vrG8`, private in Drive root — share manually). **Non-destructive prod probe GREEN** post-deploy (all 6 security headers live on `https://www.openi.ai`, backend health 200). **DON'T-REVERT:** the H1/M1/LM1 ownership guards, the L1 CORS prod-gate, the L2 prod error-message branch, the L3 `algorithms:['HS256']` pins, and the `vercel.json` headers block. **No new API surface, no schema, no migration** → version bump for the security posture change. Both repos clean + synced `0 0`, Vercel + Railway auto-deployed. Backend HEAD `5012b07` → `9af20d6`. Frontend HEAD `d82d961` → `095dff5`. DOCUMENTATION.md v5.36 → v5.37. Prior session (session 11, 10 Jun) — **Testing-team bug batch #1–#5 (backend HEAD `5012b07` / frontend HEAD `d82d961`).** Five reported bugs fixed across both repos, no migration. **#1** (BE `2b0c8e0`) — applying to a Marketplace challenge now auto-accepts a matching pending invite (was leaving the invite stuck pending). **#2** (FE `be4ea2d`) — DeepTech "Recent Assessments" rows are now clickable through to the assessment, and the Avg Score tile no longer shows `NaN` when an assessment has no percentage. **#3** (FE `fc23f26`) — the 8-Vector Evaluation page now lists Recent Assessments date-wise (newest first). **#4** (FE `6607e4c`) — the public DeepTech share page renders the full question text + section grouping + per-question weights (matched to the PDF detail shipped session 9). **#5** (BE `5012b07` + FE `d82d961`) — admins can now Edit AND Delete events they did not create, plus the full event **Edit** feature: NEW `PUT /events/:id` (`eventController.update` — validation mirrors `create()`; auth gate = event creator OR active same-org member via `org_members` self-JOIN OR admin `(req.user?.roles||[]).includes('admin') || req.user?.role==='admin'`; UPDATEs only the user-editable columns RETURNING *; leaves visibility / published_at / registrations / created_by untouched) wired in `routes/index.js` behind `authMiddleware, requireRole(...event.CREATE_ROLES), auditLog('update_event','event')`; frontend `eventAPI.update(id, data)`, `isAdmin`-widened Edit/Delete gate in `EventDetail`, and the Create modal doubles as the Edit modal. **#5 is the v5.36 bump driver** (new API surface `PUT /events/:id`). **Verification delegated to the testing team.** DON'T-REVERT: the BE `eventController.update` auth gate + the PUT route, and the FE `isAdmin` Edit/Delete gate. Both repos clean + synced `0 0`, Vercel + Railway auto-deployed, no migration. Backend HEAD `2afeb70` → `2b0c8e0` → `5012b07`. Frontend HEAD `ef82aee` → `be4ea2d` → `fc23f26` → `6607e4c` → `d82d961`. DOCUMENTATION.md v5.35 → v5.36. Prior session (session 10, 9 Jun) — **Login-gated startup-profile share + collab-key fix (backend HEAD `2afeb70` / frontend HEAD `ef82aee`).** New member-mintable share-link surface for startup profiles: any logged-in member can generate a share link; the link always carries the FULL profile (the earlier redaction model was dropped per the locked design); an unauthenticated visitor who opens a share link gets a register/login CTA carrying `?redirect=<share URL>` and lands on the full profile after auth. Backend `2afeb70` adds the share-invite endpoint + the redirect-preserving public route guard; frontend `ef82aee` wires the Share modal (Link / Email tabs) off `startup.user_id` ownership so the tabs render for the profile owner and members. Shipped alongside a **collab-key fix** (BE `83c1717` / FE `d9a4eaf`) — `createCollab` / `startCollab` now key off `startup.user_id` (was a wrong id, causing the Collab action on Discover-Startups cards to mis-target). **Verification delegated to the testing team** (open a share link logged-out → register → land on full profile; logged-in member mints link + Email tab renders; Collab button on a Discover card targets the right startup). DON'T-REVERT: the login-gated share route guard + `?redirect=` preservation, and the `startup.user_id` collab keying. Both repos clean + synced `0 0`, Vercel + Railway auto-deployed, no migration. Backend HEAD `83c1717` → `2afeb70`. Frontend HEAD `d9a4eaf` → `ef82aee`. DOCUMENTATION.md v5.34 → v5.35. Prior session (session 9, 5 Jun) — **sequential bug-fix batch Tasks A–F (BE `7b01614`/`ac39389`, FE `22d11e5`/`088da40`/`31b5598`/`0b9efcb`, no bump at the time → folded in here).** Task A 8-vector PDF full criterion-level detail (`7b01614`); Task B watchlist row-click opens profile (`22d11e5`); Task C DeepTech assessment PDF full question text + section grouping + per-question weights (`ac39389`); Task D browser-tab favicon wired (`088da40`); Task E bookmark icon opens the real `watchlistAPI`-backed picker modal (`31b5598`); Task F Share Startup Profile link ownership derived from `startup.user_id` (`0b9efcb`). Task G (notification bell) closed as a non-bug after live read-only inspection (working as designed — do NOT re-investigate). See the session-10 + session-9 Correction-of-record blocks below. Earlier baseline (session 8, 4 Jun) — **Bing/IndexNow indexing closeout (frontend ships `0b18d57` + `b5b4df3`, no version bump → stayed v5.34).** Mirrored the prior Google indexing work to Bing/Yandex: shipped `indexnow-submit.js` as the last `postbuild` step (reads `public/sitemap.xml`, POSTs every `<loc>` to the IndexNow API on every production deploy; safe-by-design — no-ops off-prod, never fails the build) + the public key-file `9694112683b05c9da88fc8c7d674717f.txt` for ownership, plus an `index.html` meta-description trim (244→152 chars for SERP fit). Verified post-deploy: build log `[indexnow] submitted 7 URLs — HTTP 202`, key-file live → 200. All 4 Bing goals complete (key URLs submitted, coverage read, IndexNow live, sitemap confirmed). Frontend HEAD `7519362` → `b5b4df3`. Backend HEAD `edb847e` (unchanged). No schema / new-API change → DOCUMENTATION.md stays v5.34. Full detail in the session-8 Correction-of-record block below. Prior session (session 7) — **GEO P1 content layer: public `/faq` page (frontend ship, `7519362`).** New citable answer surface for AI engines (ChatGPT / Claude / Perplexity / Gemini): `src/pages/public/PublicFAQ.jsx` — 5 sections / 13 buyer Q&A pairs (About OpenI Hub, Searching/sourcing startups, Running innovation challenges, Pricing/access, Trust/security/data). A single `FAQ_SECTIONS` array drives BOTH the visible accordion AND a single `FAQPage` JSON-LD block (`@type: FAQPage`, 13 `Question`→`acceptedAnswer`) so the structured data can never drift from the on-page copy. Wired through all 6 prerender touchpoints: App.jsx public route + import, `entry-prerender.jsx` ROUTES, `prerender.js` ROUTE_META, `public/sitemap.xml` (pri 0.7), `PublicLayout.jsx` nav + footer. Prerendered to `dist/faq/index.html` (24,220-char body, 1× FAQPage JSON-LD / 13 Questions, own title + self-canonical/og:url `/faq`). **Blog DEFERRED** per user "FAQ only" — top GEO item for a dedicated next session. +277 / 6 files. Frontend HEAD `4312d90` → `7519362`. Backend HEAD `9f746a6` (unchanged). DOCUMENTATION.md v5.33 → v5.34. Earlier in session 7 — **GEO discoverability pass (frontend P0/P1/P2, no version bump at the time).** Made the public site discoverable + citable via three frontend build-config + meta layers (P0 head/discovery meta + robots/sitemap/llms.txt/vercel.json `a46f14d`→`3287374`; P1 body prerendering of `/`, `/marketplace`, `/reports` via Vite-SSR `prerender.js` `92e4674`; P2 per-route head meta override fixing a duplicate-canonical bug `9b6b13d`→`b6999fe`). Full block below. Earlier this session (session 2 baseline) — **Invite-email delivery fix + persona-specific lead framing (backend ship).** Shipped `2f5d11d` (backend, deployed to prod via Railway `71d22fcb`, ACTIVE/live). When someone invites a startup that is NOT yet a registered OpenI user, the magic-link signup email now reads as a REAL opportunity instead of a generic invite. `emailService.pendingInviteEmail` (L498-539) builds an `inviterLine` (name + org) and a per-`inviterRole` pitch: corporate = "a real lead… looking for solution providers", investor = "investment opportunities", government = "grant / funding opportunity", with incubator/accelerator/researcher/mentor variants; plus a per-`entityType` noun/lead/cta map (challenge/watchlist/collaboration/ipr) and a role-specific subject line. `pendingInviteService.createPendingInvite` now forwards `inviterRole`; callers wired (watchlistController selects `role`→`inviterRole`, inviteController, collaboratorController, iprController). No schema or frontend change. **Verification delegated to the testing team** (Path A: invite a registered user → standard challenge-invite email; Path B: invite an unregistered email from each inviter role → persona-framed magic-link signup email). Backend HEAD `0439bbc` → `2f5d11d`. Frontend HEAD `19ea8cb` (unchanged). DOCUMENTATION.md v5.32 → v5.33.

> **Correction of record (23 Jun 2026, session 23 — P2 mobile-audit MEDIUM batch SE1 + PROF7, frontend HEAD `e5b27d5` → `baf92f1` / backend HEAD `1ec7fc6` (untouched), NO version bump — CSS/layout-only):** Two of three MEDIUM mobile-audit items from `MOBILE_AUDIT_2026-05-27.md` shipped; the third needed no change. Single frontend commit `baf92f1` (2 files, +41/-5). **No JS logic, no API surface, no schema, no migration — purely responsive CSS/layout** → no version bump (consistent with the session-13/14/15 CSS-only precedent). **(SE1) `StartupEvaluation.jsx` vector-card criterion rows.** The fixed `grid-template-columns:'1fr auto auto'` crushed the 5 score buttons + the chat-toggle icon below ~375px. Converted each criterion row to `flex flex-col sm:grid sm:items-center` and wrapped the score buttons + `MessageSquare` toggle in `<div className="flex items-center justify-between sm:contents">` — the `sm:contents` (`display:contents` at the `sm` breakpoint) flattens the wrapper so its children become direct grid cells again at desktop, preserving the original multi-column grid with **no dual render branch**. The column-header bar moved from `grid` to `hidden sm:grid` (hidden on mobile where the row stacks). **(PROF7) `MyProfile.jsx` child-table accordion discovery.** The 8 child-table accordions (team → acquisitions) were slow to find on mobile. Each `ProfileSection` outer div gained `id={`profsec-${section}`}` + `scrollMarginTop:'16px'`; added a `md:hidden` horizontally-scrollable chip strip (8 chips, sand `#F5F3EF` bg / navy `#152838` text) whose onClick does `document.getElementById(`profsec-${section}`)?.scrollIntoView({behavior:'smooth',block:'start'})`. Desktop is untouched (`md:hidden`). **(EV3) `EventsRepository.jsx` modal — NO CHANGE NEEDED.** Already mobile-safe from the earlier "Mobile Ship 5" refactor (`hidden md:block` table + `md:hidden` card fallback; single-column `maxWidth:900` detail). Re-verified, no edit. **Banked invariant — `sm:contents` grid-flatten:** to keep a desktop CSS grid intact while stacking on mobile WITHOUT a second render branch, wrap the would-be grid-cell children in a flex container for mobile and set that wrapper to `sm:contents` so it disappears from layout (children re-promote to direct cells) at `sm`+. **DON'T-REVERT:** the `StartupEvaluation.jsx` `flex flex-col sm:grid` rows + `sm:contents` button wrapper + `hidden sm:grid` header; the `MyProfile.jsx` `profsec-${section}` anchor ids + `scrollMarginTop` + the `md:hidden` chip strip. **Verification delegated to the testing team** (at 375px: StartupEvaluation criterion score buttons + chat icon wrap cleanly, no horizontal scroll; MyProfile chip strip scrolls and each chip smooth-scrolls to its accordion; desktop unchanged at ≥`sm`/`md`). Frontend repo clean + synced `0 0`, Vercel auto-deployed; backend untouched (`1ec7fc6`). Frontend HEAD `e5b27d5` → `baf92f1`. DOCUMENTATION.md stays v5.40.
>
> **Correction of record (19 Jun 2026, session 20 — post-session-19 batch: DeepTech assessment data-isolation fix + reports sector/technology filtering + register persona-group alignment, backend HEAD `fe4525b` → `4f2e7e7` / frontend HEAD `e330285` → `30b9caa`, NO version bump):** Five undocumented commits (3 backend + 2 frontend, captured retroactively at session close) plus one prod DB-only rename. All bug/UX/display — **no new API surface, no schema, no migration** → no version bump (consistent with the session-13/14/15 precedent). **(A) DeepTech data isolation (BE `4f2e7e7`, `deeptechController.js` +11/-1).** `list` adds `da.assessed_by = $N` for non-admins (admins keep full visibility), and `getOne` returns **404 (not 403)** to a non-owner so the existence of another user's assessment can't be probed — mirroring the `exportPdf`/`createShare`/`listShares`/`revokeShare` owner gate already in the file. The testing-team "still an issue" report was proven **NOT a bug**: both demo rows (id 1 score 77, id 2 score 72) are owned by user id 7, so they correctly co-appear for that account. `create` already stamps `assessed_by = req.user.id`. **(B) DB-only rename (no commit).** `deeptech_assessments` row id 1 (`assessed_by=7`, score 77) was renamed `'Test Submit'` → `'Amber Kinetics'` via a single-row prod `UPDATE … WHERE id=1` so both of that account's assessments read consistently; verified live (id 1 + id 2 both now `'Amber Kinetics'`). This was a data edit, not code. **(C) Reports surface (3 commits).** BE `8042a10` — reports list now sorts `createdAt:desc` (newest first). BE `f31c0d0` (`publicController.js` +38/-5) — source the per-report technology tags from the backend lookup table (Strapi is prod-locked, so the tags are resolved server-side, not from the CMS). FE `50c4875` (`PublicReports.jsx` +83/-25) — add sector + technology filtering and map BFSI/ESG/Tamil-Nadu report icons + colours. FE `9eeb22c` (`PublicReports.jsx`) — fix the report-open link by dropping the download-attribute + `target=_blank` combination that opened a blank tab instead of the file. FE `95af76b` — add the static PDF "The AI Data Quality Crisis in BFSI" at `public/reports/the-ai-data-quality-in-bfsi.pdf`. **(D) Register persona picker (2 commits).** FE `d4d1f8e` (`Register.jsx` +73/-47) — split the persona picker into provider/seeker groups and sharpen the Startup-vs-Service-Provider copy (fixes the reported "Dentsu mis-pick" where an agency self-selected the wrong persona). FE `30b9caa` (`Register.jsx` +19/-11) — align the persona group labels with the homepage Innovation Providers / Innovation Seekers framing so registration and marketing copy match. **DON'T-REVERT:** the `deeptechController.js` `list` + `getOne` owner gate (404-on-non-owner); the `publicController.js` reports sort + technology-tag lookup; the `PublicReports.jsx` sector/technology filter + icon/colour map + report-open link fix; the `Register.jsx` provider/seeker persona grouping. **Verification delegated to the testing team** (non-admin sees only own DeepTech assessments + 404 on another user's assessment id; admin sees all; reports filter by sector + technology, newest first, and the BFSI PDF opens in the same tab; register persona groups match the homepage Provider/Seeker split). Both repos clean + synced `0 0`, Vercel + Railway auto-deployed. Backend HEAD `fe4525b` → `4f2e7e7`; frontend HEAD `e330285` → `30b9caa`. DOCUMENTATION.md stays v5.40.
>
> **Correction of record (18 Jun 2026, session 19 — Bug #2a duplicate-org guard (409 recourse) + FULL org-persona claim/merge subsystem, backend HEAD `7275d21` → `fe4525b` / frontend HEAD `c83419d` → `e330285`, version bump v5.39 → v5.40):** Two coordinated ships extend the existing adopt-merge machinery (built for startup personas) to organizations, plus a hard duplicate-org guard at creation. One schema migration RAN + verified live. Backend `fe4525b` (3 files, +456/-11: `claimController.js`, `orgController.js`, `migrate-phase53-claims.js`); frontend `e330285` (2 files, +73/-3: `OrgAdmin.jsx`, `api.js`). **(A) Bug #2a duplicate-org guard (BE `orgController.js`).** `createOrg` now runs a pre-insert duplicate check on `name_normalized OR LOWER(domain)`; on a hit it returns **409** via `buildDuplicateRecourse(matchOrg)` returning `{error, recourse:'claim'|'join', org:{id,name,slug,domain}}`. `recourse` = `'claim'` iff the matched org's admin user is `is_imported===true && claimed_at==null` (an unclaimed imported placeholder the caller can adopt), else `'join'` (a live org the caller should request to join). The INSERT now also writes `name_normalized` via a JS `normalizeOrgName(name)` = `name.trim().toLowerCase().replace(/\s+/g,' ')` that mirrors the SQL normalization. The 23505 unique-violation handler discriminates by constraint name: `organizations_name_normalized_unique` / `organizations_domain_unique` → 409 recourse; slug collision → 400. **(B) Org claim/merge subsystem (BE `claimController.js`).** `CLAIM_ELIGIBLE_ROLES` gains 6 org roles (corporate, government, investor, lab, incubator, accelerator → 9 total). `requestClaim` now accepts EITHER `target_startup_user_id` OR `target_org_id` (exactly one — XOR, else 400); a `target_org_id` routes to the new `requestOrgClaim`. `findOrgClaimCandidates(domain)` matches `organizations JOIN users ON admin_user_id WHERE is_imported AND claimed_at IS NULL` with confidence 1.0 (exact domain) / 0.7 / 0.5. `executeOrgMerge(claim)`: snapshot the placeholder org + admin → adopt `ORG_MERGE_FIELDS` onto the claimer's org → reassign 8 FK reference sets inside per-set SAVEPOINTs (conflictSafe tables use `ON CONFLICT DO NOTHING` then DELETE the orphaned dup) → delete the placeholder org + the imported admin user → stamp the claimer's `claimed_at`. The verify/approve path branches on `target_org_id`; `rollbackClaim` recreates the imported admin user + the placeholder org from the snapshot (which is why `target_org_id` is `ON DELETE SET NULL`, not CASCADE). `resolveOrgMergeIds` derives the claimer-org id + the placeholder-admin id. **(C) FE recourse CTA (FE `OrgAdmin.jsx` + `api.js`).** `handleCreateOrg`'s catch detects `err.status===409 && err.recourse && err.org` → `setCreateRecourse(...)` → renders a branching CTA card: `recourse==='claim'` calls `claimAPI.request({ target_org_id: org.id, evidence })`; `recourse==='join'` calls `orgAPI.requestJoin({ org_id: org.id })`. `src/services/api.js` line-75 generic error path now attaches `err.status` / `err.recourse` / `err.org` / `err.error` from the 409 response body so the component can read them. **(D) Migration (BE `migrate-phase53-claims.js` step-2b).** `ALTER TABLE profile_claims ADD COLUMN IF NOT EXISTS target_org_id INTEGER REFERENCES organizations(id) ON DELETE SET NULL` + `CREATE INDEX IF NOT EXISTS idx_pc_target_org`. **Migrations are NOT auto-run in prod** — applied this session via `railway ssh` → the standalone `migrate-phase53-claims.js` and **VERIFIED LIVE** (`information_schema.columns` probe: `profile_claims` now has 19 columns, `target_org_id` integer = col 19; `idx_pc_target_org` present). **The org claim/merge API surface is the v5.40 bump driver** (`requestClaim` accepting `target_org_id` + the structured 409 recourse contract on `createOrg`), mirroring the session-17 `DELETE /investor/deal-requests/:id` and session-11 `PUT /events/:id` precedents. **Banked invariants:** `ORG_MERGE_FIELDS` = `name`/`domain`/`industry`/`about`/`website`/`headquarters`/`size_band`/`founded_year`/`type`/`logo_url` (`name_normalized` deliberately excluded — it is re-derived from the adopted `name`). `ORG_REASSIGN_REFERENCES` (8 sets) = `org_members.org_id` (conflictSafe), `users.org_id`, `organization_personas.org_id` (conflictSafe), `challenges.org_id`, `challenge_applications.applicant_org_id`, `events.org_id`, `ipr_records.org_id`, `documents.org_id`. 409 recourse payload shape = `{error, recourse:'claim'|'join', org:{id,name,slug,domain}}`. **DON'T-REVERT:** the `orgController.js` dup guard + `buildDuplicateRecourse` + `name_normalized` INSERT + the 23505 constraint discrimination; the `claimController.js` org-claim subsystem (the 9-role set, `requestOrgClaim` / `findOrgClaimCandidates` / `executeOrgMerge` / `resolveOrgMergeIds` + the verify/approve/rollback org branches); `migrate-phase53-claims.js` step-2b; the `OrgAdmin.jsx` recourse CTA; the `api.js` line-75 409-body attach. **Open follow-up (offered, NOT requested):** mirror step-2b's `target_org_id` ALTER into `src/startup.js#runMigrations` so a fresh-DB boot gets the column (it currently lives only in the standalone script; prod is already migrated, so this is low-priority). **No destructive prod change beyond the additive nullable column.** **Verification delegated to the testing team** (create an org whose name/domain collides with an existing one → 409 + a claim-or-join CTA card; run the org-claim happy path → `requestOrgClaim` → admin verify/approve → `executeOrgMerge` reassigns all 8 FK sets, deletes the placeholder org + imported admin, stamps the claimer's `claimed_at`, and populates `target_org_id`; roll a claim back → the imported admin + placeholder org are recreated from the snapshot; submit `requestClaim` with both/neither target id → 400). Both repos clean + synced `0 0`, Vercel + Railway auto-deployed. Backend HEAD `cd0d53e`/`7275d21` → `fe4525b`; frontend HEAD `2eb09aa`/`c83419d` → `e330285`. DOCUMENTATION.md v5.39 → v5.40.
>
> **Correction of record (17 Jun 2026, session 17 — Bug 6: investor deal-request invite + delete, backend HEAD `bdf49a4` → `cd0d53e` / frontend HEAD `91c0da2`/`a0b295f` → `2eb09aa`, version bump v5.38 → v5.39):** Testing-team report — an investor could neither invite an innovation provider to a deal nor delete a deal request. Two blockers fixed, no migration (all tables/columns already exist). **(A) Invite by email (BE `collaboratorController.inviteByEmail`).** The registered-user branch INSERT into `entity_collaborators` referenced a non-existent column (`invited_by_user_id`) and an illegal status (`'active'` is not in the table's CHECK list), so every invite of an already-registered user silently 500'd. Re-aligned to the LIVE schema: `INSERT INTO entity_collaborators (entity_type, entity_id, user_id, role, status, invited_by, accepted_at) VALUES ($1,$2,$3,$4,'accepted',$5,NOW()) ON CONFLICT (entity_type,entity_id,user_id) DO NOTHING` (params `[entity_type, entity_id, existing.id, role||'viewer', req.user.id]`). The non-registered-email path (pending invite + magic-link email) was already correct and untouched. **(B) Delete deal request (BE `investorController.deleteDealRequest` + route).** The endpoint was absent at every layer. Added a transactional controller (`BEGIN/COMMIT`, ownership-gated `WHERE id=$1 AND investor_id=$2` → 404 on mismatch) that prunes `deal_request_applications`, `entity_collaborators`, and `entity_reviews` before deleting the `investor_deal_requests` row; exported it; and registered `DELETE /investor/deal-requests/:id` behind `authMiddleware` in `routes/index.js`. **Frontend wiring (`2eb09aa`).** `src/services/api.js` adds `investorAPI.deleteDealRequest(id)` + `collabAPI.inviteByEmail(payload)`; `src/pages/dashboard/InvestorDealRequests.jsx` adds a confirm-gated Trash2 Delete button (list + detail) calling `deleteRequest()`; `src/components/CollaboratorsPanel.jsx` routes `submitInvite` through `collabAPI.inviteByEmail({ entity_type, entity_id, emails:[email], role })`. **The new DELETE route is the v5.39 bump driver** (new API surface, mirroring the session-11 `PUT /events/:id` precedent). **Banked schema invariant:** `entity_collaborators` uses `invited_by` (NOT `invited_by_user_id`); status CHECK = `'invited'`/`'accepted'`/`'declined'`/`'removed'` (NO `'active'`); has `accepted_at`; UNIQUE on (entity_type, entity_id, user_id); `deal_request` is a supported collab entity type and the owner's `my_role` resolves to `'editor'`. **DON'T-REVERT:** the `collaboratorController.inviteByEmail` INSERT, the `investorController.deleteDealRequest` controller + route, and the FE `api.js` / `InvestorDealRequests.jsx` / `CollaboratorsPanel.jsx` wiring. **No migration. Verification delegated to the testing team** (invite a registered innovation provider to a deal → added immediately; delete a deal request as its owning investor → row + dependents gone, 404 for a non-owner). Both repos clean + synced `0 0`, Vercel + Railway auto-deployed. Backend HEAD `bdf49a4` → `cd0d53e`; frontend HEAD `91c0da2`/`a0b295f` → `2eb09aa`. DOCUMENTATION.md v5.38 → v5.39.
>
> **Correction of record (16 Jun 2026, session 16 — OpenI Brand Kit standardized + applied platform-wide (UI + email assets), backend HEAD `bdf49a4` / frontend HEAD `91c0da2`, version bump v5.37 → v5.38):** The canonical brand palette + typography were locked into a NEW `OPENI_BRAND_KIT.md` committed to BOTH repo roots (`91c0da2` / `bdf49a4`) as the single source of truth, then swept across the React UI and the backend email accents. **Canonical palette:** OpenI Gold (primary) `#D0A848` (RGB 208,168,72); Deep Navy `#152838`; Charcoal `#3A3A3A`; Slate `#585858`; Light Sand `#F5F3EF`; White `#FFFFFF`; accent blue `#6EC1E4`; darkest bg `#111E21`. **Deprecated (do NOT reintroduce):** old gold `#D5AA5B`, old gold-dark `#CFA745`, old navy `#252147`. **Typography:** Lexend only (no Lora `.ttf` bundled — hierarchy via colour/weight/size/banding). **Ship 1 — FE brand apply (`91c0da2`, 105 files, +324/-246).** `tailwind.config.js` ramps retuned (`primary.500`=`#D0A848`, `primary.600`=`#b8923a`, `primary.700`=`#94742d`, `dark.900`=`#152838`, `dark.950`=`#111E21`, `accent.400`=`#6EC1E4`, `navy`=`#152838`) + `fontFamily.sans`/`display`=`['Lexend','sans-serif']`, plus a 102-file `src/**` inline-literal sweep replacing legacy gold `#D5AA5B`→`#D0A848` / old navy `#252147`→`#152838`. **Ship 2 — BE email-accent retune (`bdf49a4`, 4 files, +84/-6).** Gold accent literals in `emailService.js` / `orgController.js` / `emailVerificationController.js` moved to `#D0A848`; the 4th file is `OPENI_BRAND_KIT.md` (the BE copy). **GST tax-invoice PDF LEFT UNTOUCHED** — `pdfService.js` GST field layout is legally constrained (CGST/SGST/IGST, HSN/SAC, place of supply, total-in-words); colour-only changes are permitted but the layout must not change, and a domestic + an export invoice must be re-verified after any palette change. **Cosmetic/brand only — no schema, no new API surface, no migration.** Version bumped for the documented platform-wide brand-*posture* change (consistent with the session-12 security-posture bump precedent: "no new API/schema/migration → bump for posture"). **DON'T-REVERT:** the `tailwind.config.js` ramp values, the `src/**` literal sweep, the BE email-accent literals, and either copy of `OPENI_BRAND_KIT.md`. **Verification delegated to the testing team** (live UI shows new gold/navy + Lexend; NO deprecated `#D5AA5B`/`#252147` survives a hard-reload; domestic + export GST invoices still render correctly). Both repos clean + synced `0 0`, Vercel + Railway auto-deployed. Backend HEAD `1dc6434` → `bdf49a4`; frontend HEAD `21d33f2` → `91c0da2`. DOCUMENTATION.md v5.37 → v5.38.
>
> **Correction of record (15 Jun 2026, session 15 continued — student/academia/mentor bug batch: profile-photo persistence + .docx uploads + Register Step-2 prefill + discovery card-click modal, backend HEAD `ef349e4` / frontend HEAD `21d33f2`, no version bump):** Five ships (2 backend + 3 frontend incl. 1 chore), one schema migration ALREADY applied + VERIFIED live in prod. Backend lineage `1dc6434` → `c320a8d` → `ef349e4`; frontend lineage `863b0c5` → `1542a85` → `77b7027` → `21d33f2`. **Ship 1 — student/academia/mentor profile-photo (`logo_url`) persistence (BE `ef349e4`, two-layer).** The Profile Photo upload silently dropped for student/academia/mentor personas: two gaps — (a) `logo_url` was missing from the `ALLOWED_COLUMNS` whitelist for those tables in `profileController.updateMyProfile()` (non-whitelisted columns are silently dropped), and (b) the column didn't exist on `student_profiles`/`academia_profiles`/`mentor_profiles`. Fix: add `logo_url` to all three whitelists (+3) AND an idempotent `ALTER TABLE … ADD COLUMN IF NOT EXISTS logo_url VARCHAR(500)` migration block in `src/startup.js#runMigrations` (+15). **Migrations are NOT auto-run in prod** — the migration was applied via `railway ssh → node src/scripts/migrate-bootstrap.js` last session and is **VERIFIED LIVE** (an `information_schema.columns` probe this session confirms `logo_url` exists on all three tables). **Ship 2 — .docx/.doc uploads for portfolios/resumes/pitch-decks (BE `c320a8d`, +11/-4).** `src/utils/cloudinary.js` switched the `portfolios`/`resumes`/`pitch_decks` (incl. the `pitch-decks` hyphen variant) upload presets from Cloudinary `resource_type:'image'` to `'auto'` — `'image'` rasterizes a PDF (thumbnail) but REJECTS a `.docx`/`.doc`; `'auto'` stores a PDF as `image` and a Word doc as `raw`. `logos`/`org-logos`/`register-uploads` stay `'image'`. No schema. **Ship 3 — Register Step-2 prefill for Student/Academia (FE `77b7027`, +41/-7).** When a registrant matches an existing-domain org, Step-2 now prefills `research_areas`/`bio`: `src/config/personas.js` maps student/academia `industry`→`research_areas` and `description`→`bio` (PERSONA_INDUSTRY_FIELD / PERSONA_DESCRIPTION_FIELD); `src/pages/auth/Register.jsx` adds a module-scope pure `coerceForField(fields, fieldName, value)` helper (wraps a string in an array for tags-type fields like `research_areas`), applied at both prefill sites (the Phase-117 org-match `useEffect` and the "Create my own" onClick), and memoises `profileFields` with `useMemo([personaType])`. **Ship 4 — discovery card-click profile modal (FE `21d33f2`, +182/-8).** Student/Academia discovery cards had `cursor:pointer` but no `onClick` → clicks were no-ops. Fix mirrors the Mentors modal pattern: `src/pages/dashboard/StudentDiscovery.jsx` + `AcademiaDiscovery.jsx` add a `selected` state set on card click and render a detail modal off the ALREADY-FETCHED record (no new route, no new component, no extra backend call), closing on overlay/X with inner `stopPropagation`. **Ship 5 — chore (FE `1542a85`).** Empty commit to re-trigger a Vercel build after a transient Sentry 504 on release finalize; no code. **DON'T-REVERT:** the BE `profileController.js` student/academia/mentor `logo_url` whitelist entries + the `startup.js` `logo_url` ALTER block; the BE `cloudinary.js` `portfolios`/`resumes`/`pitch_decks` `resource_type:'auto'`; the FE `personas.js` industry→research_areas / description→bio maps + `Register.jsx` `coerceForField` helper; the FE `StudentDiscovery.jsx` / `AcademiaDiscovery.jsx` card-click modal. **No new API surface → no version bump** (consistent with session-13/14/15 precedent; the schema add is a single nullable column on existing tables, not a new product surface). **Verification delegated to the testing team** (upload a profile photo as student/academia/mentor → persists across reload; upload a `.docx` portfolio/resume/pitch-deck → accepted; register matching an existing org domain → Step-2 research_areas/bio prefilled; click a Student/Academia discovery card → detail modal opens). Both repos clean + synced `0 0`, Vercel + Railway auto-deployed. Backend HEAD `1dc6434` → `ef349e4`; frontend HEAD `863b0c5` → `21d33f2`.
>
> **Correction of record (15 Jun 2026, session 15 — testing-team bug batch: admin delete-user FK cascade closeout + analytics 500 fixes + profile-score cap fix + frontend safeStorage crash-guard, backend HEAD `1dc6434` / frontend HEAD `863b0c5`, no version bump):** Four ships (3 backend bug fixes + 1 frontend crash-guard), no migration — all FK fixes are app-layer. Session-15 lineage `533660e` → `1b6ac2e` → `b9f0616`+`660fb91` → `c04031f` → `1dc6434`. **Ship 1 — admin delete-user FK cascade closeout (BE `1b6ac2e` → `1dc6434`, USER-CONFIRMED working in prod).** `adminController.deleteUser()` 500'd on FK violations (23503) because `users(id)` is referenced by ~48 child FK columns, many declared `NO ACTION` (`pg_constraint.confdeltype='a'`). The fix lives inside the existing `BEGIN/COMMIT` txn as two arrays — `nullableFkCleanups` (`UPDATE ${t} SET ${col}=NULL WHERE ${col}=$1`, for nullable attribution cols) and `notNullFkDeletes` (`DELETE FROM ${t} WHERE ${col}=$1`, for NOT-NULL / owned-row dependents) — run before `DELETE FROM users WHERE id=$1`. `1b6ac2e` re-aligned both arrays to the LIVE prod schema; `1dc6434` added all 14 remaining `confdeltype='a'` FKs found via a prod `pg_constraint` enumeration query (9 nullable → SET NULL, 5 NOT-NULL → DELETE). This is the same FK-violation class as session-13 bug #1; the comprehensive `pg_constraint` enumeration ended the whack-a-mole cycle. User tested → **"delete is working."** **Ship 2 — analytics `featureAdoption` 500 fixes (BE `b9f0616` + `660fb91`).** `analyticsController.featureAdoption` 500'd querying a wrong/non-existent column: `b9f0616` counts challenges via `challenges.corporate_id`; `660fb91` counts via `incubator_programs.incubator_id`. **Ship 3 — profile-score 98% cap fix (BE `c04031f`, +31/-4).** `profileScoreService` ALIAS_FIELDS now treats `founded_year` ≡ `incorporation_date` as one filled field, lifting the startup ~98% completeness cap. **Ship 4 — frontend safeStorage crash-guard (FE `863b0c5`, NEW `src/utils/safeStorage.js`).** In-app WebViews / private mode / hardened third-party-storage blocking expose `window.localStorage` but throw `SecurityError` (DOMException 18) on property access → an unguarded boot read crashed the React render. The wrapper returns `null` on getItem failure and no-ops setItem/removeItem. **DON'T-REVERT:** the `adminController.deleteUser` FK-cleanup arrays, the `analyticsController.featureAdoption` column fixes, the `profileScoreService` ALIAS_FIELDS, and `src/utils/safeStorage.js`. **No new API surface, no schema, no migration → no version bump** (consistent with session-13/14 precedent for backend/bug-only ships). **Verification delegated to the testing team** (Ship 1 already USER-CONFIRMED). Both repos clean + synced `0 0`, Vercel + Railway auto-deployed. Backend HEAD `533660e` → `1dc6434`; frontend HEAD `453caa2` → `863b0c5`.
>
> **Correction of record (13 Jun 2026, session 14 — transactional `deleteChallenge` + GEO/SEO GSC diagnosis, backend HEAD `c2d3ad5` / frontend HEAD `453caa2`, no version bump):** Two threads, no migration. **Thread 1 — orphan-notification leak on challenge delete (BE `c2d3ad5`).** `corporateController.deleteChallenge` deleted the challenge row and pruned its `user_notifications` as separate statements; an error between them stranded orphan notification rows (the same stale-FK class the `prune-stale-notifications.js` backfill exists to mop up). Fix: wrap the challenge delete + notification-prune in a single `BEGIN/COMMIT` transaction with `ROLLBACK` on error — either both succeed or neither does, so no orphans can be created going forward. No new API surface, no schema, no migration. **DON'T-REVERT** the txn wrapper in `corporateController.deleteChallenge`. **Thread 2 — GEO/SEO Google Search Console indexing report (no code change, "Option A").** The GSC indexed-coverage warnings for property `openi.ai` trace to dead legacy WordPress asset URLs ending in file extensions (`.css/.js/.png/.pdf/.xml`) — they have no `redirects` rule in `vercel.json` AND are deliberately exempted by the SPA `rewrites` catch-all (`/((?!… | .*\.[a-zA-Z0-9]+$).*)` excludes extension paths) → they fall through to the Vercel static handler → 403. Self-healing: Google drops the ghost URLs on re-crawl; no live URL is affected. User chose **Option A — do nothing**; `vercel.json` (incl. the session-12 security headers) left untouched. **No version bump** — Thread 1 is a no-new-API bug fix and Thread 2 shipped no code. Both repos clean + synced `0 0`. Backend HEAD `533660e` → `c2d3ad5`; frontend HEAD `453caa2` (unchanged).

> **Correction of record (11 Jun 2026, session 12 — ISO 27001 pre-audit OWASP security hardening, backend HEAD `9af20d6` / frontend HEAD `095dff5`, version bump v5.36 → v5.37):** Pre-penetration-test remediation across the OWASP Top 10; 7 findings fixed, no migration, prod-probe GREEN. **A01 Broken Access Control / IDOR** — **H1** (BE `startupController.js`) startups update/remove ownership-gated (only `created_by` or admin, else 403); **M1** (BE `corporateController.js`) milestone/task update+delete gated on `corporate_id` via `SELECT id FROM collaborations WHERE id=$1 AND corporate_id=$2` (404 on mismatch); **LM1** (BE `watchlistController.js`) `getOne`+`exportPdf` gated via `canWatchlistRead(pool, req.user.id, watchlistId)` (admin-bypass, else 403). **A05 Security Misconfig** — **L1** (BE `server.js`) localhost CORS origins gated `if (process.env.NODE_ENV !== 'production')`; **L2** (BE `server.js`) error handler returns generic message in prod (`message: isProd ? 'Internal server error' : err.message`), full detail logged server-side; **M2** (FE `vercel.json`) global `/(.*)` headers block — nosniff / `X-Frame-Options DENY` / Referrer-Policy strict-origin-when-cross-origin / Permissions-Policy camera+microphone+geolocation=() / HSTS max-age 63072000 includeSubDomains preload / CSP (`self` + `unsafe-inline`/`unsafe-eval` for the Vite bundle, Sentry, Google Fonts; `connect-src` limited to `api.openi.ai`/Sentry; `frame-ancestors 'none'`; `object-src 'none'`); `/assets` + `/index.html` Cache-Control preserved. **A02 Cryptographic Failures** — **L3** (BE) JWT pinned `algorithms:['HS256']` on every verify + `algorithm:'HS256'` on signing across `authController.js`, `middleware/auth.js` (authMiddleware + optionalAuth), `emailVerificationController.js`, `mfaController.js` — closes algorithm-confusion. Admin pattern throughout: `req.user?.role==='admin' || (req.user?.roles||[]).includes('admin')`. `npm audit fix` → backend 0 vulns; frontend remaining advisories build-time-only devDeps (react-snap/vite) — accepted. **Findings + remediation report** = Google Doc `16bqv-jzxpz2qVIK1-IZS9Xob2smKIxQFSjbqxG7vrG8` (private in Drive root — share manually). Non-destructive prod probe GREEN (6 headers live on `https://www.openi.ai`, backend health 200). **DON'T-REVERT:** H1/M1/LM1 guards, L1 CORS prod-gate, L2 prod error branch, L3 HS256 pins, `vercel.json` headers. No new API surface / schema / migration → bump for security posture. Both repos clean + synced `0 0`. Backend HEAD `5012b07` → `9af20d6`; frontend HEAD `d82d961` → `095dff5`.

> **Correction of record (10 Jun 2026, session 11 — testing-team bug batch #1–#5, backend HEAD `5012b07` / frontend HEAD `d82d961`, version bump v5.35 → v5.36):** Five reported bugs fixed across both repos, no migration. **#1** (`2b0c8e0` backend) — applying to a Marketplace challenge now auto-accepts a matching pending invite (was leaving the invite stuck pending). **#2** (`be4ea2d` frontend) — DeepTech "Recent Assessments" rows are clickable through to the assessment + the Avg Score tile no longer shows `NaN` when an assessment has no percentage. **#3** (`fc23f26` frontend) — the 8-Vector Evaluation page lists Recent Assessments date-wise (newest first). **#4** (`6607e4c` frontend) — the public DeepTech share page renders full question text + section grouping + per-question weights (matched to the PDF detail shipped session 9). **#5** (`5012b07` backend + `d82d961` frontend) — admins can now Edit AND Delete events they did not create, plus the full event **Edit** feature: NEW `PUT /events/:id` (`eventController.update` — validation mirrors `create()`; auth gate = event creator OR active same-org member via `org_members` self-JOIN OR admin `(req.user?.roles||[]).includes('admin') || req.user?.role==='admin'`; UPDATEs only the user-editable columns RETURNING *; leaves visibility / published_at / registrations / created_by untouched) wired in `routes/index.js` behind `authMiddleware, requireRole(...event.CREATE_ROLES), auditLog('update_event','event')`; frontend `eventAPI.update(id, data)`, `isAdmin`-widened Edit/Delete gate in `EventDetail`, Create modal doubles as the Edit modal. **#5 is the v5.36 bump driver** (new API surface `PUT /events/:id`). **DON'T-REVERT:** the BE `eventController.update` auth gate + the PUT route, and the FE `isAdmin` Edit/Delete gate. **Verification delegated to the testing team.** Both repos clean + synced `0 0`, no migration. Backend HEAD `2afeb70` → `2b0c8e0` → `5012b07`; frontend HEAD `ef82aee` → `be4ea2d` → `fc23f26` → `6607e4c` → `d82d961`.
>
> **Correction of record (9 Jun 2026, session 10 — Login-gated startup-profile share + collab-key fix, backend HEAD `2afeb70` / frontend HEAD `ef82aee`, version bump v5.34 → v5.35):** Two ships this session. **Login-gated share** (BE `2afeb70` + FE `ef82aee`) — any logged-in member can mint a share link for a startup profile; the link always carries the FULL profile (the earlier redaction model was DROPPED per the locked design). An unauthenticated visitor who opens a share link gets a register/login CTA that carries `?redirect=<share URL>`, and lands on the full profile after auth. Backend `2afeb70` adds the share-invite endpoint + the redirect-preserving public route guard; frontend `ef82aee` wires the Share modal (Link / Email tabs) off `startup.user_id` ownership so the tabs render for the profile owner and members. This is the v5.35 bump driver (new public route + share-invite API surface). **Collab-key fix** (BE `83c1717` + FE `d9a4eaf`) — `createCollab` / `startCollab` now key off `startup.user_id` (was a wrong id, causing the Collab action on Discover-Startups cards to mis-target). **DON'T-REVERT:** the login-gated share route guard + `?redirect=` preservation, and the `startup.user_id` collab keying. **Verification delegated to the testing team.** Both repos clean + synced `0 0`, no migration. Backend HEAD `83c1717` → `2afeb70`; frontend HEAD `d9a4eaf` → `ef82aee`.
>
> **Correction of record (5 Jun 2026, session 9 — sequential bug-fix batch Tasks A–F, backend HEAD `ac39389` / frontend HEAD `0b9efcb`, no version bump at the time → folded into v5.35):** Six display / PDF-detail / wiring fixes on existing surfaces — no schema, no new API. **Task A** (`7b01614` backend) — 8-vector assessment PDF now renders every criterion (not just vector totals). **Task B** (`22d11e5` frontend) — Watchlist row-click opens the startup profile. **Task C** (`ac39389` backend) — DeepTech assessment PDF now includes full question text + section grouping + per-question weights. **Task D** (`088da40` frontend) — browser-tab favicon (OpenI "oi" mark) wired (was missing). **Task E** (`31b5598` frontend) — bookmark icon opens the real `watchlistAPI`-backed picker modal instead of a no-op. **Task F** (`0b9efcb` frontend) — Share Startup Profile link ownership derived from `startup.user_id` so the Link/Email tabs render. **Task G** (notification bell) closed as a NON-BUG after live read-only inspection (`GET /my/notifications` → 200, unread-count = 0, badge correctly hides at 0; system never self-notifies) — working as designed, do NOT re-investigate. No schema / new-API change at the time → these stayed v5.34 and are folded into the v5.35 bump.
>
> **Correction of record (4 Jun 2026, session 8 — Bing/IndexNow indexing closeout, frontend HEAD `b5b4df3` / backend HEAD `edb847e` unchanged, no version bump):** Two frontend ships, both SEO / build-infra on the existing public site — no schema, no new API, no migration. **IndexNow** (`0b18d57`) — new `indexnow-submit.js` added as the last `postbuild` step (after `gen-version.js` + `prerender.js`): reads `public/sitemap.xml`, extracts every `<loc>`, dedupes, and POSTs the list to `https://api.indexnow.org/indexnow` so Bing/Yandex recrawl changed pages quickly (the Bing analogue of Google's recrawl, ~10k URLs/day quota). SAFE BY DESIGN — no-ops unless `VERCEL_ENV==='production'` (skips local + preview builds), never throws (any network/parse error logged + swallowed so a flaky endpoint can't fail a Vercel deploy, exits 0); the public key-file `public/9694112683b05c9da88fc8c7d674717f.txt` proves ownership. **SEO meta-trim** (`b5b4df3`) — `index.html` L10 meta-description shortened 244→152 chars to fit the SERP snippet length. **Verified post-deploy:** build log `[indexnow] submitted 7 URLs — HTTP 202`; key-file live `https://www.openi.ai/9694112683b05c9da88fc8c7d674717f.txt` → 200. All 4 Bing goals complete (key URLs submitted, coverage read, IndexNow live, sitemap confirmed). **DON'T-REVERT:** `indexnow-submit.js`, the key-file, the `package.json` postbuild chain. No schema / new-API change → stays v5.34.
>
> **Correction of record (3 Jun 2026, session 7 — Phase 99m/99n/99o/99p + Option B cache-busting, backend HEAD `edb847e` / frontend HEAD `c4bf182`, no version bump):** Five tail ships, all display / RBAC / PDF-layout / data-hygiene / build-infra on existing surfaces — no schema, no new API. **99m** (`ea46e6e` frontend) — gate the invite "Revoke" control to the original inviter only (co-inviter could previously revoke another inviter's pending-email invite). **99n** (`19f2479` backend) — allow `viewer`-role challenge members to Export PDF (was owner/editor-only; read-capable roles should be able to export). **99o** (`50b39b1` backend) — fix challenge-report PDF text overlap by replacing auto-flow with manual-y `ensureSpace` pagination. **99p** (`edb847e` backend) — hide U+FFFD-corrupted startup names from the public list + search (filter on the replacement char). **Option B** (`c4bf182` frontend) — runtime stale-bundle detector: `__APP_VERSION__` baked into the bundle by Vite (`define`, source = `VERCEL_GIT_COMMIT_SHA`), `dist/version.json` written at postbuild (`gen-version.js`), `useVersionCheck` re-checks on window focus / visibility (60s rate-limit, no-store fetch) and shows a persistent Reload/Later toast on mismatch; closes the recurring "stale Vercel bundle indistinguishable from a broken feature" bug class. No schema / new-API change → stays v5.34.
>
> **Correction of record (3 Jun 2026, session 7 — Phase 99l, backend HEAD `9f746a6` / frontend HEAD `da5c558`, no version bump):** Two corporate challenge-invite-modal bugs fixed (testing-team report). **Bug #1** (`9f746a6` backend, +37/-2) — the Invited list showed only registered-user invites; `listInvitesForChallenge` INNER-JOINed `users` on `invited_user_id`, structurally excluding `pending_email_invites` rows (NULL `invited_user_id`). Fixed with a `UNION ALL` second branch (`source='pending_email'`, `pei.created_at AS invited_at`, `pei.email` as both name + email) + a `source`-aware `revokeInvite` that routes pending-email rows to `pendingInviteService.revokePendingInvite`. **Bug #2** (`da5c558` frontend) — invite-by-email dropped the 3rd+ chip; the `onKeyDown`/`onBlur` dedupe read a render-time stale-closure `inviteEmails`. Fixed by moving dedupe INSIDE the functional updater (`setInviteEmails(prev => prev.includes(em) ? prev : [...prev, em])`); `api.js revokeInvite(cid,iid,source)` forwards `source` as a query param. No schema / new-API change → stays v5.33.
>
> **Correction of record (3 Jun 2026, session 7 — GEO discoverability pass, frontend HEAD `da5c558` → `b6999fe`, no version bump):** Made the public site discoverable + citable by crawlers and AI answer engines (ChatGPT / Claude / Perplexity / Gemini). Three layers, all frontend build-config + meta only (no schema, no API, no migration). **P0** (`a46f14d` → `3287374`) — static head/discovery meta baked into `index.html` (title, description, canonical, Open Graph, Twitter card, 2× JSON-LD: Organization + SoftwareApplication) plus `robots.txt`, `sitemap.xml`, `llms.txt`, and `vercel.json` rewrites so the static assets serve. **P1** (`92e4674`) — body prerendering: a `postbuild` step (`prerender.js`) Vite-bundles `src/entry-prerender.jsx` (SSR), renders the public leaf pages (`/`, `/marketplace`, `/reports`) to static HTML via `react-dom/server`, and injects it into each route's `dist/<route>/index.html` so raw HTML carries real body content before JS runs (no Chromium — react-snap removed). **P2** (`9b6b13d` doc → `b6999fe`) — per-route head meta override in `prerender.js` (`ROUTE_META` + `applyRouteMeta`) so `/marketplace` and `/reports` advertise their OWN title / description / canonical / og:url instead of inheriting the homepage's (fixes a duplicate-canonical SEO bug + strengthens the per-page signal for answer engines). No schema / new-API change → stays v5.33.
>
> **Correction of record (3 Jun 2026, session 6 — backend HEAD `579a989`, no version bump):** Phases 99j (`61ea1cf`) and 99k (`579a989`) NARROWED the `2f5d11d` persona "lead" framing described above. Only `challenge` (apply) invites still read as a REAL lead. `challenge_reviewer` (99j → `isCollabInvite`), plus `watchlist` / `ipr` / `collaboration` (99k → `NON_LEAD_PITCH` map + `isNonLeadInvite`) now read as collaboration / opportunity asks instead. The gate is `isNonLeadInvite = Object.prototype.hasOwnProperty.call(NON_LEAD_PITCH, entityType)` — when present, `entityType` OVERRIDES `inviterRole`, so a corporate inviting a reviewer no longer mis-frames the email as "a real lead… looking for solution providers." Also in session 6: 99f (`b2b04c0`) reviewer-invite-by-email + 99g role-aware challenge access (`challenge_members` owner/editor/reviewer/viewer), 99h (`cd2b41f`) challenge-invite emails not sending (PG client use-after-release re-order), 99i (`dc52588`) Postgres 23514 on over-escaped `pending_email_invites.status` CHECK. No schema / new-API change → DOCUMENTATION.md stays v5.33.

**Pending verification:** **Session 11** — bug batch #1–#5: **#1** (`2b0c8e0` BE) applying to a Marketplace challenge auto-accepts a matching pending invite (was stuck pending); **#2** (`be4ea2d` FE) DeepTech Recent-Assessments rows click through + Avg Score tile no longer shows `NaN` on a no-percentage assessment; **#3** (`fc23f26` FE) 8-Vector Evaluation page lists Recent Assessments newest-first; **#4** (`6607e4c` FE) public DeepTech share page renders full question text + sections + per-question weights; **#5** (`5012b07` BE + `d82d961` FE) an admin can Edit AND Delete an event they did not create (Edit button next to Delete on every event; the new modal prefills + `PUT /events/:id` saves; visibility/registrations untouched). **Session 10** — login-gated share (`2afeb70` + `ef82aee`): open a share link logged-OUT → register/login CTA carries `?redirect=<share URL>` → land on the FULL profile after auth; a logged-in member mints a share link + the Email tab renders; the share link always carries the full profile (no redaction). Collab-key fix (`83c1717` + `d9a4eaf`): the Collab button on a Discover-Startups card targets the right startup (keys off `startup.user_id`). **Session 9** — Tasks A–F display/PDF/wiring (`7b01614`/`ac39389` BE, `22d11e5`/`088da40`/`31b5598`/`0b9efcb` FE): 8-vector PDF criterion detail, watchlist row-click→profile, DeepTech PDF question/section/weight detail, favicon, bookmark→real watchlist picker, Share-link tabs render. **Carried** — Invite-email persona framing (`2f5d11d`) + 99j/99k COPY narrowing (REAL external gmail: Path A registered → standard challenge-invite; Path B unregistered per inviter role → persona-framed magic-link; reviewer/watchlist/IPR/collaboration read as collaboration NOT "lead"). 99g role-aware challenge access (reviewer can open+evaluate, cannot mutate). Tours fire on the moved mount (`085cc07`). 401/idle banners in prod (`4f73c0b`). Phase 122 + 123 mobile dashboard layout on real device. Phase 120 mobile Back button.

### 10 Jun ships (session 11 — bug batch #1–#5)

| # | Commit | Fix | Notes |
|---|---|---|---|
| 1 | `2b0c8e0` (BE) | fix(invite) — applying to a Marketplace challenge auto-accepts a matching pending invite | Was leaving the invite stuck pending. |
| 2 | `be4ea2d` (FE) | fix(deeptech) — Recent-Assessments rows clickable + Avg Score `NaN` fix | Rows click through to the assessment; the Avg Score tile no longer shows `NaN` when an assessment has no percentage. |
| 3 | `fc23f26` (FE) | fix(8-vector) — Recent Assessments date-wise (newest first) | 8-Vector Evaluation page ordering. |
| 4 | `6607e4c` (FE) | fix(share) — public DeepTech share page full Q text + sections + per-question weights | Matched to the assessment-PDF detail shipped session 9 (`ac39389`). `SharedDeepTech.jsx` renders `DEEPTECH_SECTIONS`/`DEEPTECH_OPTIONS`-grouped answers. |
| 5 | `5012b07` (BE) + `d82d961` (FE) | feat(events) — admin Edit/Delete + full event Edit feature | NEW `PUT /events/:id` (`eventController.update`; validation mirrors `create()`; auth gate = creator OR active same-org member via `org_members` self-JOIN OR admin; UPDATEs only user-editable cols RETURNING *; leaves visibility/published_at/registrations/created_by untouched) behind `authMiddleware, requireRole(...event.CREATE_ROLES), auditLog('update_event','event')`. FE `eventAPI.update`, `isAdmin`-widened Edit/Delete gate in `EventDetail`, Create modal doubles as Edit modal. **v5.36 bump driver** (new API surface). DON'T-REVERT the BE auth gate + PUT route and the FE `isAdmin` gate. |

### 9 Jun ships (session 10 — login-gated share + collab-key)

| # | Commit | Fix | Notes |
|---|---|---|---|
| 1 | `2afeb70` | feat(share) — login-gated startup-profile share invite (backend) | New share-invite endpoint + redirect-preserving public route guard. Unauthenticated visitor on a share link gets a register/login CTA carrying `?redirect=<share URL>`; lands on the FULL profile after auth (redaction model dropped per locked design). v5.35 bump driver (new public route + share-invite API surface). DON'T-REVERT the route guard + `?redirect=` preservation. |
| 2 | `ef82aee` | feat(share) — Share modal Link/Email tabs (frontend) | Share modal wired off `startup.user_id` ownership so the Link/Email tabs render for the profile owner and members. Pairs with `2afeb70`. |
| 3 | `83c1717` | fix(collab) — key `createCollab` off `startup.user_id` (backend) | Was a wrong id, causing the Collab action on Discover-Startups cards to mis-target. |
| 4 | `d9a4eaf` | fix(collab) — `startCollab` keys off `startup.user_id` (frontend) | `CorporateStartupSearch.jsx` `startCollab` passes `startup_user_id: startup.user_id`. Pairs with `83c1717`. DON'T-REVERT the `startup.user_id` keying. |

### 5 Jun ships (session 9 — Tasks A–F, folded into v5.35)

| # | Commit | Fix | Notes |
|---|---|---|---|
| A | `7b01614` | fix(pdf) — 8-vector assessment PDF full criterion detail (backend) | Renders every criterion, not just vector totals. |
| B | `22d11e5` | fix(watchlist) — row-click opens startup profile (frontend) | |
| C | `ac39389` | fix(pdf) — DeepTech assessment PDF full question text + sections + per-question weights (backend) | |
| D | `088da40` | fix(favicon) — browser-tab favicon (OpenI "oi" mark) wired (frontend) | Was missing. |
| E | `31b5598` | fix(bookmark) — bookmark icon opens real `watchlistAPI` picker modal (frontend) | Was a no-op. |
| F | `0b9efcb` | fix(share) — Share Startup Profile link ownership off `startup.user_id` (frontend) | Link/Email tabs render. Precursor to session-10 share work. |

> Task G (notification bell) closed as a NON-BUG after live read-only inspection (`GET /my/notifications` → 200, unread-count = 0, badge correctly hides at 0; system never self-notifies). Working as designed — do NOT re-investigate.

### 1 Jun ships (session 2 — invite-email)

| # | Commit | Fix | Notes |
|---|---|---|---|
| 1 | `2f5d11d` | fix(invite-email) — persona-specific lead framing for pending invites | Magic-link signup email to non-OpenI users now frames the lead as REAL and persona-specific by inviter `users.role` + `entityType`. `pendingInviteEmail` builds `inviterLine` + per-role pitch + per-entity noun/lead/cta + role-specific subject. `createPendingInvite` forwards `inviterRole`; 4 callers wired. No schema/frontend change. Deployed via Railway `71d22fcb` (ACTIVE). |

---

**Previous Version:** 5.35
**Previous Updated:** 9 Jun 2026 (session 10) — **Login-gated startup-profile share + collaboration-key fix (backend HEAD `2afeb70` / frontend HEAD `ef82aee`).** Two fixes across both repos. (1) Collab keyed on wrong id (FE `d9a4eaf` + BE `83c1717`): collaborations are now stored/joined by `startup_user_id` (not the legacy `startups.id`), fixing an FK collision; the Discover "Collab" button now sends the correct `startup_user_id`. (2) Login-gated share (FE `ef82aee` + BE `2afeb70`): any logged-in member can mint a share link at `/share/startup/:token`; the link always carries the FULL profile (the earlier redaction was dropped); unauthenticated visitors get a register/login CTA carrying `?redirect=<share URL>` (open-redirect guard accepts internal single-`/` paths only) and land on the profile after auth. Login route is `/dashboard/login` (no plain `/login`). Known limitation by design: `redirect` is not threaded through the email-verify hop. DON'T-REVERT the intentional mods to `SharedStartupProfile.jsx`, `Login.jsx`, `CorporateStartupSearch.jsx`. Both repos clean + synced `0 0`. DOCUMENTATION.md v5.34 → v5.35.

**Pending verification:** Spot-check tours fire on the moved mount (one corporate dashboard + one persona dashboard + one admin route) since the mount relocated. 401/idle banners in prod now that redirect targets `/dashboard/login` (clear `openi_token` → trigger authed call → expect redirect + amber banner). Phase 122 + 123 mobile dashboard layout on real device. Phase 120 mobile Back button (challenge detail → list, not Home).

### 1 Jun ships (post-hang recovery)

| # | Commit | Fix | Notes |
|---|---|---|---|
| 1 | `4f73c0b` | fix(auth) — correct login redirect path so session banners render | Redirects pointed at `/login?reason=expired` but real route is `/dashboard/login`; auth guard stripped the query string before the banner could read it. Re-pointed all 3 redirects + pathname guards. Closes the carried "401 interceptor pending verification" item. |
| 2 | `085cc07` | refactor(tours) — hoist TourWrapper to DashboardLayout | Single mount `{navRole && <TourWrapper role={navRole} />}` replaces per-page mounts in Corporate + Persona dashboards. Covers all routes incl. admin sub-routes. Effectively closes Tour Batches 6/7/8 (3 P0s). Build 0 errors. |

---

**Previous Version:** 5.31
**Previous Updated:** 31 May 2026 — **Vercel stale-bundle root-cause fix + retroactive token-rotation documentation.** Shipped `9bc03fa` fix(vercel): added a `headers` block to `vercel.json` so content-hashed `/assets/*` are served `Cache-Control: public, max-age=31536000, immutable` while the `/index.html` shell is `public, max-age=0, must-revalidate`. Root cause of the recurring stale-bundle `ae` TDZ false-alarm: Vercel's default served BOTH the shell and the hashed assets with `must-revalidate`, allowing stale bundles through. Verified live via deployment `dpl_91dTWrik3VxpKbQ967cyVcvb6D5p` @ `9bc03fa` (state READY) — curl confirmed shell `/` → `max-age=0, must-revalidate` and hashed asset `/assets/index-D_qB7PVq.js` → `max-age=31536000, immutable`, with the asset hash changed (`index-CvSeNjHY.js` → `index-D_qB7PVq.js`) confirming a fresh build. Also documented retroactively: backend git auth + GitHub token rotation (done a prior session, never written to memory) — the exposed over-scoped classic PAT was rotated to a fine-grained repo-scoped PAT (Contents r/w) and the old classic PAT deleted; backend fetch/push verified working. Backend HEAD `0439bbc` (unchanged). Frontend HEAD `9bc03fa` → doc commit `e8090ff`. DOCUMENTATION.md v5.30 → v5.31.

---

**Previous Version:** 5.30
**Previous Updated:** 30 May 2026 — **Auth/onboarding bug-trio session (Tyler Woolsey report) + 401 interceptor completion.** Cohort member Tyler Woolsey couldn't complete a Dentsu challenge apply — expired JWT produced a silent "0 of 0 steps" onboarding dead-end plus a Settings page crash. Root-caused and shipped a 4-commit fix: (1) `0ff280b` fix(settings) — TDZ crash on `/dashboard/settings`; (2) `150287a` fix(onboarding) — de-silenced the `getStatus` catch + added error/retry UI (was failing silently to "0 of 0 steps"); (3) `6e62a6f` feat(api) — global 401 interceptor in `request()`: expired/invalid JWT now `removeToken()` + redirect `/login?reason=expired` with amber banner in Login.jsx (permanent fix for the dead-end); (4) `d321b26` fix(api) — ported the same 401 interceptor branch into `blobRequest()` (PDF-download wrapper), closing the gap where an expired token mid-download left a dead-end + dispatches `openi:unauthorized`. Tyler recovery email drafted + handed to marketing to send (NOT sent by Claude). Backend HEAD `0439bbc` (unchanged — frontend-only session). Frontend HEAD `d321b26`. DOCUMENTATION.md v5.29 → v5.30.
...
1. **Bug #3 — onBlur email auto-promote** (`8878707` frontend). Cohort reported Send Invites button stays disabled when user types email but doesn't press Enter. Fix: added `onBlur` handler to email input mirroring existing `onKeyDown` validate-and-promote logic. Email auto-promotes to chip on focus loss → button enables → send proceeds.

2. **Bug #1 — Step 2 Register prefill from email domain** (`7f8febd` backend + `397ac42` frontend). Cohort reported Step 2 fields blank when user picks "Create my own" from Phase 113 ladder card. Backend: widened `orgByDomain` SELECT to LEFT JOIN admin's persona profile across 8 seeker tables (corporate/government/investor/mentor/lab/incubator/accelerator/service_provider), exposing `admin_persona`, `industry`, `description`. Removed `LIMIT 1` to support multi-org-per-domain. Frontend: wrapped `setStep(2)` button onClick with prefill handler that auto-fills Company Name + Industry + About from matched org. Live verification: `karuna@tata.com` → Tata Innovation Corp prefill works end-to-end.

3. **Bug #2 — Reviewer challenge visibility** (`be2da35` backend + `beacc87` Side A + `03930fc` Side B frontend). Cohort reported reviewers added to Challenge Team don't see the challenge. Schema migration: `challenge_members_role_check` extended from 3 ('owner','editor','viewer') to 4 ('owner','editor','reviewer','viewer'). New `corporateController.myReviewerQueue` handler + `GET /challenges/my-reviewer-queue` route (literal segment before `/challenges/:id` wildcard per Phase 94 ordering rule). Notification fire on `addMember` (kind `challenge_reviewer_invite`, fire-and-forget pattern). New `ChallengesToReview.jsx` page + `WORKSPACE_ITEMS.challengesToReview` nav entry (visible across all 11 personas) + Team UI in `CorporateChallenges.jsx` (invite-by-email form + role dropdown + members list + edit/remove).

4. **A.5 INR — Renewal CTA → Annual upgrade path** (`2f0870d` backend + `33591be` frontend + transactional DB UPDATE). Karuna's mid-June renewal flow. Pricing UPDATE on prod: Pro Plan ₹2,499/mo → ₹23,990/yr (20% off, saves ₹5,998), Enterprise ₹9,999/mo → ₹95,990/yr (20% off, saves ₹23,998). NEW `changeBillingCycle` handler with Razorpay proration for mid-period upgrade. `createOrder` + `verifyPayment` already had cycle branching (memory audit). Phase 113 renewal email annual nudge for monthly subscribers (conditional gold tip card). Frontend Monthly/Annual toggle pill above plan grid, plan card cycle-aware (yearly + "≈ ₹X/mo equivalent" + "Save ₹X/year" badge), Upgrade button shows "Pro (Annual)" / "Pro (Monthly)", URL `?cycle=annual` deep-link from email auto-selects Annual on mount.

5. **A.5 USD — Export buyer pricing path** (`fc70fe2` backend + `7105825` frontend + transactional DB UPDATE). Pricing UPDATE on prod: Pro $29.93/mo / $287.31/yr, Enterprise $119.75/mo / $1,149.58/yr (auto-derived via USD_INR_RATE=83.50). `subscription_plans` schema: ADD COLUMN `price_monthly_usd` + `price_yearly_usd`. `createOrder` + `verifyPayment` + `changeBillingCycle` detect `isExport` from `billing_address.country`, branch basePrice between INR/USD columns, Razorpay order currency = USD vs INR. Phase 113 renewal email USD/INR copy branching (savings text + plan-specific amounts). Frontend `displayCurrency` derived state from billing_address, plan card branches `$` + en-US locale (USD) vs `₹` + en-IN locale (INR). USD path renders "IGST 0% (Export under LUT)" — no GST math, Phase 60.11 LUT compliance. Karuna (India) unchanged INR path. First export buyer gets full USD checkout end-to-end.

6. **A — Admin Platform-Health Dashboard** (`7caed9c` backend + `cc973bf` hotfix v2 + `b1d4c9f` frontend + `6c5bbca` icon fix). User explicit ask 26 May. Closes 4th leg of operator observability stack (cost + uptime + errors + NOW user-growth/revenue/churn). NEW `adminDashboardController.platformHealth` returns single JSON via Promise.all of 8 parallel SQL queries: user KPIs, signups timeline, persona breakdown, paid count, revenue MTD/YTD/12mo (per-currency with gross + net), churn early warning. Hotfix v2: split KPIs into "Real Signups" (excludes 575K bulk-imported s39 directory substrate) + "Directory" (all users) after discovery that initial query showed signups_30d=560,201 due to s39 being counted. NEW `AdminPlatformHealth.jsx` page with 12 KPI tiles + 3 recharts (signups LineChart 90d, revenue BarChart 12mo stacked INR+USD, persona PieChart donut). Admin sidebar entry "Platform Health" with TrendingUp icon. Live at `/dashboard/admin/platform-health`. Icon fix: IndianRupee for INR tiles, DollarSign for USD tiles.

7. **B / Ship C — Finance paid-customer notification** (`62162b6` backend). Closes "how do I find out about new paid customers" gap. NEW `paidCustomerFinanceNotificationEmail()` template (~50 lines, HTML table with customer + plan + amount + GST + total + currency + Razorpay payment_id + order_id + invoice number + payment date IST + deep-link to admin user profile). Subject: `[OpenI Paid] {customerName} - {planDisplayName} - {currency} {totalAmount}`. TO recipients vanessa+rajeev (NOT BCC — for "to me" inbox filter visibility). Fire-and-forget hook in `subscriptionController.verifyPayment` after existing `paymentConfirmationEmail` send. Try/catch wrapper guarantees zero impact on payment flow. First signal Vanessa + Rajeev get on next paid event.

**Pricing source-of-truth on prod after today (both currencies, set via railway ssh transactional UPDATE):**

| Plan | INR Monthly | INR Yearly | USD Monthly | USD Yearly |
|---|---|---|---|---|
| Pro | ₹2,499 | ₹23,990 (20% off, saves ₹5,998) | $29.93 | $287.31 (20% off, saves ~$70) |
| Enterprise | ₹9,999 | ₹95,990 (20% off, saves ₹23,998) | $119.75 | $1,149.58 (20% off, saves ~$287) |

**Operational signals from Platform Health (first live observation):**
- Directory claim rate: **4 of 575,111 = 0.0007%** — founder outreach + claim flow are the levers to move this
- Active 30d (logged in): 21 of 62 real signups — `last_login` column may not be uniformly populated by all auth flows
- Revenue MTD/YTD: ₹5,447.82 INR (Karuna's 2 captured payments) + $0 USD (no export buyers yet — A.5 USD path armed for first one)

**A.5 USD note on backward compat:** A.5 INR (commit 2f0870d) shipped earlier in session — backend cycle branching was already present from prior phases, just the `changeBillingCycle` handler was new. A.5 USD (commit fc70fe2) extended that to currency-aware. No behavioral regression for Karuna or any INR customer; Karuna's path unchanged.

**Decisions locked this session (do not re-litigate):**
- Annual discount: 20% off (Indian SaaS standard)
- USD pricing: auto-derived via USD_INR_RATE=83.50, round to 2 decimals
- Currency detection: from `billing_address.country` (India → INR, else → USD)
- Tax handling: INR shows 18% GST math, USD shows "IGST 0% (Export under LUT)"
- Revenue dashboard: Gross AND Net side-by-side (net = captured - refunded)
- Active user definition: logged in within last 30 days
- Persona scope: canonical 11 only (no admin/evaluator/legacy roles)
- Churn window: 30d forward + 30d backward

**Lessons banked this session (added to CLAUDE.md Don'ts):**
1. Single-quoted heredoc `<<'EOSCRIPT'` is paste-safe up to ~100 lines for apply-scripts. Bracketed-paste-mode failures only at extreme sizes.
2. macOS TextEdit auto-appends `.txt` extension despite explicit `.py` filename — skip TextEdit, use heredoc instead.
3. Apply-script transport reliability order: single-quoted heredoc > base64 > TextEdit.
4. Stuck heredoc recovery: Ctrl+C aborts cleanly. Verify with `ls -la <path> && echo exists || echo not_written`.
5. Anchor-string whitespace matters for apply-script matching. Column-alignment patterns (`removeMember:      (cid, uid)      => del(...)`) need exact-multi-space match.
6. Apply-script `[ok]` per substitution + final summary print is the right format. Mid-script abort visibility critical.
7. Pre-flight schema probe via `railway ssh + node + pg` before any SQL-touching feature.
8. The "$" lucide icon next to "Rs X" tiles looks like currency mismatch even when decorative. Use `IndianRupee` for INR + `DollarSign` for USD.

---


### What's New in v5.7 - Phase 104 ladder + Phase 106 sourcemaps (23 May 2026)

**6 commits, full Phase 104 ladder shipped + Phase 106 Sentry sourcemap upload live on prod.**

#### The session in one line

Sentry alerted on a frontend crash at `/dashboard/documents` (Phase 104b useMemo placed after `if (loading) return ...` early-return -> "Rendered more hooks than during the previous render" -> ErrorBoundary). Fixed the crash, then built a permanent guardrail (ESLint rules-of-hooks at pre-commit + Vercel build), then surfaced 32 latent errors across the codebase the chain caught on its first run, then closed the loop by wiring `@sentry/vite-plugin` so future Sentry triages show real `file:line` instead of minified `xv:1:42`.

#### Six commits on the ladder

| Phase | Commit | What |
|---|---|---|
| 104c | `4b26ff2` | useMemo moved above `if (loading) return ...` in DocumentRepository.jsx. Fixes the Sentry-reported crash that was breaking `/dashboard/documents` for every visitor since Phase 104b ship (22 May late evening). |
| 104d | `ab31cdb` | ESLint 8 + Standard React preset + react-hooks plugin + husky v9 + lint-staged + Vercel build gate. `rules-of-hooks: error` + `exhaustive-deps: warn`. Pre-commit hook lints staged files only. Vercel build runs `npm run lint && vite build`. |
| 104e+f | `909ba68` | Resolves the 32 errors the new lint chain surfaced. Real bugs found: DashboardHome.jsx had 7 hooks declared AFTER 3 early-return redirects (same trap class as 104b -- would have crashed any user whose persona triggered a redirect on second render). DashboardLayout.jsx had a duplicate `onClick` that made notification mark-read never call the backend (Phase 101 Sub-D regression) AND a duplicate `cursor:"pointer"` that made non-link notification rows misleadingly clickable-looking. 13 files touched. |
| 106 | `1b40985` -> `422d555` | `@sentry/vite-plugin@^2.22.0` wired into `vite.config.js`. `build.sourcemap: true` + plugin runs at build end: creates Sentry release tagged with `VERCEL_GIT_COMMIT_SHA`, uploads ~15 sourcemap files, deletes them from `dist/` before Vercel ships to CDN. Public bundle stays minified for users; Sentry has the maps. |
| Sentry archive | (UI action) | The "Rendered more hooks" issue archived in next-release mode -- will auto-escalate if regression detected. |

#### Two silent bugs caught by the lint chain on first run

The lint chain's actual gold:

1. **DashboardHome.jsx hooks after early return.** Phase 20 redirect chain at lines 65-73 returned `<Navigate to=...>` for new users + corporate persona + persona roles. All 7 `useState` + 1 `useEffect` declared at lines 77-93 BELOW the redirects. Same crash class as Phase 104b. Hadn't fired in Sentry yet because the redirect-triggering personas happened to hydrate auth state synchronously most of the time -- but any race condition flipping the `effectiveRole` value between two renders would crash. Phase 104e moved all hooks above the redirect block.
2. **DashboardLayout.jsx notification dropdown.** Two duplicate keys / props in the same JSX element. Outer `<div>` at line 593 had `onClick` calling `notificationAPI.markRead(id)` + navigate. A SECOND `onClick` at line 612 (~20 lines later in the same JSX element) overrode it with a local-only `setNotifications` update. Notifications appeared marked-read in the dropdown but the backend `read_at` column never updated. Silent regression since Phase 101 Sub-D (22 May early evening). Also: `cursor:"pointer"` hardcoded on line 608 overriding `cursor: n.link ? "pointer" : "default"` on line 606 -- non-link notification rows showed pointer cursor (misleading affordance). Phase 104e dedupe.

#### Vercel env vars added for Phase 106

- `SENTRY_AUTH_TOKEN` (Sensitive, Production-only) - new org token with `project:read` + `project:write` + `project:releases` scopes
- `SENTRY_ORG_SLUG=openi-partners-llp` (all envs, NOTE: corrected from a one-character typo `openi-partner-llp` that wasted ~30 min during ship)
- `SENTRY_PROJECT_SLUG=openi-hub-frontend` (all envs)
- `VITE_SENTRY_RELEASE` - deleted (Vercel env var Value fields don't expand `$VAR`; plugin auto-detects `VERCEL_GIT_COMMIT_SHA` at build time instead)

#### Verified end-to-end on prod

- Vercel deployment `dpl_3Y4xT8DicYEytGiPJRvFoqmXaRUo` READY (commit `422d555`)
- Sentry issue archived in next-release mode (7 events / 13h fingerprint, last seen 13h ago, zero events post-104c)
- Future Sentry stack traces will show real `file.jsx:LINE:COL` instead of minified `xv:1:42` -- next triage session will be ~5 minutes faster

#### Lessons banked (4 new, added to CLAUDE.md Don'ts)

- **Hook-after-early-return is a recurring trap.** Phase 104b (DocumentRepository) and Phase 104e (DashboardHome) both shipped with hooks declared AFTER guard returns. Permanent fix: `react-hooks/rules-of-hooks: error` in `.eslintrc.cjs` + Vercel build gate. Three layers of defense now: ESLint pre-commit, Vercel build, Sentry crash detection.
- **Vercel env var Value fields do NOT expand `$VAR` references.** They're literal strings. Setting `VITE_SENTRY_RELEASE=$VERCEL_GIT_COMMIT_SHA` resulted in `sentry-cli releases new $vercel_git_commit_sha` (literal). Always use the system var directly in code (`process.env.VERCEL_GIT_COMMIT_SHA`).
- **Always verify org/project slugs via the Sentry API curl, not from memory.** The typo `openi-partner-llp` (missing `s`) wasted ~30 min. Verify via `curl https://sentry.io/api/0/projects/<org>/<project>/` -> 200 = correct. Vercel reports "Project not found" without telling you WHICH slug is wrong, costing triage time.
- **Vercel "Redeploy" UI button rebuilds the ORIGINAL deployment's commit + env-var snapshot.** It's not a fresh CI run. To rebuild with NEW env vars, push a new commit (even empty) to force a fresh CI build. This bit us when env var typo was fixed but UI Redeploy of `909ba68` succeeded (because that commit doesn't have Phase 106 code -- so the wrong env var was never read).

#### Lint chain summary (now live + non-negotiable)

| Layer | Tool | Scope | Behavior |
|---|---|---|---|
| Pre-commit (local) | husky v9 + lint-staged | Staged `.js`/`.jsx` only | Aborts commit if any rules-of-hooks error. Existing files in unchanged dirs untouched. Bypass via `--no-verify` (use sparingly). |
| CI build (Vercel) | `npm run lint && vite build` | Full `src/` tree | Fails build if any rules-of-hooks error. Warnings allowed up to `--max-warnings=9999`. |
| Runtime | Sentry @sentry/react ErrorBoundary | Production users | Catches React errors that ESLint missed (e.g. dynamic rendering bugs). Now with real file:line via Phase 106 sourcemaps. |

Three layers + Phase 102 watchdog dashboard (cost / uptime / errors) = the operational safety net is fully assembled.
