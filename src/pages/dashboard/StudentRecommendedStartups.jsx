/**
 * P1.4 (s32) — Student Recommended Startups
 *
 * Consumes GET /api/student/recommended-startups (student persona only).
 * The backend uses cluster-bridge logic: student's research_areas/skills →
 * student-cluster anchors → bridged startup-cluster IDs → boosted top-10.
 *
 * Each row carries:
 *   - cluster_boost (0 or 8): whether this startup got the cluster-bridge boost
 *   - boost_lift (int): rank delta vs no-boost (>0 means lifted UP by cluster)
...
export default function StudentRecommendedStartups() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const data = await studentEnhAPI.recommendedStartups();
    ...
    <div style={{ padding: 24, maxWidth: 1100, margin: '0 auto' }}>
      <div id="tour-page-recs-header" style={{ marginBottom: 20 }}>
        ...
        <p style={{ fontSize: 12, color: '#888', margin: '4px 0 0' }}>
          Startups matching your research areas and skills, ranked by topical alignment.
        </p>
      </div>
          ...
          {startups.map((s, i) => {
            ...
            return (
              ...
              >
                ...
                {/* Match meta: sector + match_score + cluster boost badges */}
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center', marginTop: 8 }}>
                  ...
                  {(() => {
                    ...
                    let tail = '';
                    if (ts > 0) tail = `${ts} employees`;
                    else if (fr >= 10000000) tail = `₹${(fr / 10000000).toFixed(fr >= 100000000 ? 0 : 1)}Cr`;
